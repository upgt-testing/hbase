...
public class TestClientOperationTimeout_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestClientOperationTimeout_RestartInjected.class);
    ...
    public void testGetTimeout() throws Exception {
        DELAY_GET = 600;
        RestartFramework.at("before_get_timeout_test").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
    ...
    public void testPutTimeout() throws Exception {
        DELAY_MUTATE = 600;
        RestartFramework.at("before_put_timeout_test").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Put put = new Put(ROW);
     ...
     */
    @Test
    public void testMultiTimeout() throws Exception {
        DELAY_BATCH = 600;
        ...
        gets.add(get1);
        RestartFramework.at("before_batch_timeout_test").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        gets.add(get2);
    ...
    private void assertMultiException(ThrowingRunnable runnable) {
        ...
        Assert.assertTrue("Expected SocketTimeoutException or RetriesExhaustedWithDetailsException" + " but was " + e.getClass(), e instanceof RetriesExhaustedWithDetailsException);
     ...
     */
    @Test
    public void testMultiGetMetaTimeout() throws Exception {
        Configuration conf = new Configuration(UTIL.getConfiguration());
        ...
        try (Connection specialConnection = ConnectionFactory.createConnection(conf);
            ...
            RestartFramework.at("after_connection_create_meta_timeout").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
    @Test
    public void testMultiGetRetryTimeout() throws Exception {
        Configuration conf = new Configuration(UTIL.getConfiguration());
        ...
        try (Connection specialConnection = ConnectionFactory.createConnection(conf);
            Table specialTable = specialConnection.getTable(TABLE_NAME)) {
            RestartFramework.at("after_connection_create_retry_timeout").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            MetricsConnection metrics = ((ConnectionImplementation) specialConnection).getConnectionMetrics();
            ...
            try {
                ...
                Assert.assertEquals(metaCacheNumClearServerPreFailure + 1, metaCacheNumClearServerPostFailure);
                for (Throwable cause : expected.getCauses()) {
                    Assert.assertTrue(cause instanceof OperationTimeoutExceededException);
     ...
     */
    @Test
    public void testMultiOperationTimeoutWithLocationError() throws Exception {
        ...
        try (Connection specialConnection = ConnectionFactory.createConnection(conf);
            Table specialTable = specialConnection.getTable(TABLE_NAME)) {
            RestartFramework.at("after_connection_create_location_error").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testScanTimeout() throws Exception {
        RestartFramework.at("before_scan_timeout_test").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DELAY_SCAN = 600;
        try {
