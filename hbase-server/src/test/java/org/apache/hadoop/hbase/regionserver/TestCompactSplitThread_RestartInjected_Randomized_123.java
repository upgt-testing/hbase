...
public class TestCompactSplitThread_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCompactSplitThread_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestCompactSplitThread_RestartInjected.class);
    ...
    public void testThreadPoolSizeTuning() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_config_update_bigger").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_config_update_smaller").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // check again after online update
    ...
    public void testFlushWithTableCompactionDisabled() throws Exception {
        ...
        RestartFramework.at("after_flush_loop").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assert (hfiles.size() > blockingStoreFiles + 1);
    }

    @Test
    public void testFlushWithRegionReplicas() throws Exception {
        TableDescriptor htd = TableDescriptorBuilder.newBuilder(tableName).setRegionReplication(2).build();
        TEST_UTIL.createTable(htd, new byte[][] { family }, null);
        RestartFramework.at("after_flush_loop_with_replicas").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // load the table
        for (int i = 0; i < blockingStoreFiles + 1; i++) {
            TEST_UTIL.loadTable(TEST_UTIL.getConnection().getTable(tableName), family);
            TEST_UTIL.flush(tableName);
        }
        RestartFramework.at("after_table_create_with_replicas").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // One region split should have taken place, because the primary replica gets split, and not the
        // secondary replica.
        assertEquals(1, TEST_UTIL.getRSForFirstRegionInTable(tableName).getCompactSplitThread().getSubmittedSplitsCount());
    }
}
