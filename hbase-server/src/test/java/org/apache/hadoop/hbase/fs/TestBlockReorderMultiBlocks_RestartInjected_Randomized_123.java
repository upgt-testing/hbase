...
public class TestBlockReorderMultiBlocks_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestBlockReorderMultiBlocks_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestBlockReorderMultiBlocks_RestartInjected.class);
    ...
    public void testHBaseCluster() throws Exception {
        ...
        RestartFramework.at("after_cluster_start").on(hbm).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_table_create").on(hbm).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        while (nbTest < 10) {
            ...
            RestartFramework.at("after_wal_roll").on(hbm).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            h.put(p);
            RestartFramework.at("after_put").on(hbm).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testFromDFS(DistributedFileSystem dfs, String src, int repCount, String localhost) throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            for (int y = 0; y < lbs.getLocatedBlocks().size() && done; y++) {
                Assert.assertEquals(localhost, getLocatedBlockLocations(lbs.get(y))[repCount - 1].getHostName());
            }
        }
    }
