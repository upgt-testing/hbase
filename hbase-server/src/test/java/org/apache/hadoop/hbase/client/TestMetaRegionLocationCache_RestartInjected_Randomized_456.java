...
public class TestMetaRegionLocationCache_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMetaRegionLocationCache_RestartInjected.class);
    ...
    private void verifyCachedMetaLocations(HMaster master) throws Exception {
        ...
        retries = 0;
        while (master.getMetaRegionLocationCache().getMetaRegionLocations().get().size() != metaZnodes.size()) {
            Thread.sleep(1000);
            if (++retries == 10) {
                break;
            }
        }
        List<HRegionLocation> metaHRLs = master.getMetaRegionLocationCache().getMetaRegionLocations().get();
    ...
    public void testInitialMetaLocations() throws Exception {
        ...
        RestartFramework.at("after_cache_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void testStandByMetaLocations() throws Exception {
        RestartFramework.at("after_standby_master_start").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_standby_cache_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        standBy = TEST_UTIL.getMiniHBaseCluster().getMaster(1) != null ? TEST_UTIL.getMiniHBaseCluster().getMaster(1) : // Refresh after master restart
   ...
   */
    ...
    public void testMetaLocationsChange() throws Exception {
        List<HRegionLocation> currentMetaLocs = getCurrentMetaLocations(TEST_UTIL.getMiniHBaseCluster().getMaster().getZooKeeper());
        ...
        RegionReplicaTestHelper.waitUntilAllMetaReplicasAreReady(TEST_UTIL, REGISTRY);
        RestartFramework.at("after_meta_replica_move").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_meta_replicas_ready").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMetaRegionLocationCache() throws Exception {
        ...
        try (ZKWatcher zkWatcher = new ZKWatcher(conf, null, null, true)) {
            ...
            try {
                ...
                RestartFramework.at("after_meta_locations_set").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                // Wait until the meta cache is populated.
                int iters = 0;
                while (iters++ < 10) {
                    if (metaCache.getMetaRegionLocations().isPresent() && metaCache.getMetaRegionLocations().get().size() == 3) {
                        break;
                    }
                    Thread.sleep(1000);
                }
