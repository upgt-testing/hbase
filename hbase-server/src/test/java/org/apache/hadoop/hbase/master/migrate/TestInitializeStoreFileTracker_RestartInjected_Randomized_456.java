...
public class TestInitializeStoreFileTracker_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestInitializeStoreFileTracker_RestartInjected.class);
    ...
    public void setUp() throws Exception {
        ...
        RestartFramework.at("after_cluster_start").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMigrateStoreFileTracker() throws IOException, InterruptedException {
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            TableDescriptor tdRemovedSFT = TableDescriptorBuilder.newBuilder(tdAfterCreated).removeValue(StoreFileTrackerFactory.TRACKER_IMPL).build();
        ...
        RestartFramework.at("after_tracker_removed_from_descriptors").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_tables_created").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_master_restart_ready").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_migration_completed").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
