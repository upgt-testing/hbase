...
public class TestBlockReorderBlockLocation_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestBlockReorderBlockLocation_RestartInjected.class);
    ...
    public void testBlockLocation() throws Exception {
        ...
        RestartFramework.at("after_file_create").on(hbm).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        for (int i = 0; i < 10; i++) {
            ...
            do {
                ...
                Assert.assertTrue("Expecting " + repCount + " , got " + getLocatedBlockLocations(lbs.get(0)).length, EnvironmentEdgeManager.currentTime() < max);
            ...
            Assert.assertEquals(host1, getLocatedBlockLocations(lbs.get(0))[2].getHostName());
            if (i == 0) {
                RestartFramework.at("during_block_reorder_check").on(hbm).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }
}
