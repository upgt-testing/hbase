...
public class TestScannerTimeoutHandling_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestScannerTimeoutHandling_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestScannerTimeoutHandling_RestartInjected.class);
    ...
    public void testExceededClientDeadline() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            table.put(new Put(Bytes.toBytes(i)).addColumn(new byte[] { '0' }, new byte[] { '0' }, new byte[] { '0' }));
        }
        ...
        try {
            ...
            METRICS_ASSERT.assertCounterGt("exceptions.callTimedOut", 0, TEST_UTIL.getHBaseCluster().getRegionServer(0).getRpcServer().getMetrics().getMetricsSource());
        }
    }
    ...
    private static class RSRpcServicesWithScanTimeout extends RSRpcServices {
        ...
        @Override
        public ClientProtos.ScanResponse scan(final RpcController controller, final ClientProtos.ScanRequest request) throws ServiceException {
