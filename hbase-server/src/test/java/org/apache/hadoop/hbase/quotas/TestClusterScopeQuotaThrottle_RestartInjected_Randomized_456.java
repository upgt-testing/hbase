...
public class TestClusterScopeQuotaThrottle_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestClusterScopeQuotaThrottle_RestartInjected.class);
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        for (int i = 0; i < TABLE_NAMES.length; ++i) {
            ...
            tables[i] = TEST_UTIL.getConnection().getTableBuilder(TABLE_NAMES[i], null).setOperationTimeout(10000).build();
        }
        ...
        table = TEST_UTIL.getConnection().getTableBuilder(TABLE_NAME, null).setOperationTimeout(10000).build();
    }
    ...
    public void testNamespaceClusterScopeQuota() throws Exception {
        ...
        admin.setQuota(QuotaSettingsFactory.throttleNamespace(NAMESPACE, ThrottleType.WRITE_NUMBER, 10, TimeUnit.MINUTES, QuotaScope.CLUSTER));
        // Add 6req/min limit for read request in machine scope
        admin.setQuota(QuotaSettingsFactory.throttleNamespace(NAMESPACE, ThrottleType.READ_NUMBER, 6, TimeUnit.MINUTES, QuotaScope.MACHINE));
        ...
        RestartFramework.at("after_operations").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testTableClusterScopeQuota() throws Exception {
        final Admin admin = TEST_UTIL.getAdmin();
        admin.setQuota(QuotaSettingsFactory.throttleTable(TABLE_NAME, ThrottleType.READ_NUMBER, 20, TimeUnit.HOURS, QuotaScope.CLUSTER));
        RestartFramework.at("after_operations").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testUserClusterScopeQuota() throws Exception {
        ...
        admin.setQuota(QuotaSettingsFactory.throttleUser(userName, ThrottleType.READ_NUMBER, 6, TimeUnit.MINUTES, QuotaScope.CLUSTER));
    ...
    // Spews the log w/ triggering of scheduler? HBASE-24035
    @Test
    public void testUserNamespaceClusterScopeQuota() throws Exception {
        ...
        admin.setQuota(QuotaSettingsFactory.throttleUser(userName, namespace, ThrottleType.READ_NUMBER, 10, TimeUnit.MINUTES, QuotaScope.CLUSTER));
        // Add 6req/min limit for write request in machine scope
    ...
    public void testUserTableClusterScopeQuota() throws Exception {
        ...
        final String userName = User.getCurrent().getShortName();
        RestartFramework.at("after_quota_set").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.setQuota(QuotaSettingsFactory.throttleUser(userName, TABLE_NAME, ThrottleType.READ_NUMBER, 20, TimeUnit.HOURS, QuotaScope.CLUSTER));
