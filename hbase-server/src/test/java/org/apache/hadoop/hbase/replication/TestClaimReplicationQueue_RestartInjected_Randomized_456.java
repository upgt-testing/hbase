...
public class TestClaimReplicationQueue_RestartInjected_Randomized_456 extends TestReplicationBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestClaimReplicationQueue_RestartInjected.class);
    ...
    public void testClaim() throws Exception {
        ...
        RestartFramework.at("after_stop_rs").on(UTIL1.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_procedure_waiting_timeout").on(UTIL1.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_start_new_rs").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_scp_completion").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        final HMaster masterAfterRestart = UTIL1.getMiniHBaseCluster().getMaster();
        RestartFramework.at("after_enable_peers").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hbaseAdmin.enableReplicationPeer(PEER_ID2);
        hbaseAdmin.enableReplicationPeer(PEER_ID3);
        RestartFramework.at("after_load_tables").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        EMPTY = false;
        RestartFramework.at("after_disable_peers").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // wait until the SCP finished, ClaimReplicationQueuesProcedure is a sub procedure of SCP
        UTIL1.waitFor(30000, () -> masterAfterRestart.getProcedures().stream().filter(p -> p instanceof ServerCrashProcedure).allMatch(Procedure::isSuccess));
