...
public class TestRegionAssignedToMultipleRegionServers_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionAssignedToMultipleRegionServers_RestartInjected.class);
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_sleep_reproduce").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_abort_master").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_arrive_await").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_set_kill_flag").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        KILL = true;
        RestartFramework.at("after_halt_false").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HMaster activeMaster = UTIL.getMiniHBaseCluster().getMaster();
        activeMaster.abort("For testing");
        RestartFramework.at("after_verify_region_assignment").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        activeMaster.join();
        KILL = false;
