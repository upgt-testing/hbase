...
public class TestReplicationStatusAfterLagging_RestartInjected_Randomized_456 extends TestReplicationBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationStatusAfterLagging_RestartInjected.class);
    ...
    public void testReplicationStatusAfterLagging() throws Exception {
        ...
        RestartFramework.at("after_target_cluster_start").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ReplicationLoadSource loadSource = loadSources.get(0);
        assertTrue(loadSource.hasEditsSinceRestart());
        RestartFramework.at("after_data_insert").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
