...
public class TestMetaShutdownHandler_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestMetaShutdownHandler_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMetaShutdownHandler_RestartInjected.class);
    ...
    public static void setUpBeforeClass() throws Exception {
        StartMiniClusterOption option = StartMiniClusterOption.builder().numRegionServers(3).rsClass(MyRegionServer.class).numDataNodes(3).build();
    ...
    public void testExpireMetaRegionServer() throws Exception {
        ...
        if (master.getServerName().equals(metaServerName) || metaServerName == null || !metaServerName.equals(cluster.getServerHoldingMeta())) {
            ...
            RestartFramework.at("after_move_meta").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
        }
        ...
        RestartFramework.at("after_no_rit").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Refresh after master restart
        master = cluster.getMaster();
        RestartFramework.at("after_server_offline").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_get_meta_state").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        regionStates = master.getAssignmentManager().getRegionStates();
        ...
        RestartFramework.at("after_meta_reassigned").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final ServerName priorMetaServerName = metaServerName;
        TEST_UTIL.waitFor(60000, 100, () -> metaRegionServer.isStopped());
        cluster = TEST_UTIL.getHBaseCluster();
        RestartFramework.at("after_rs_stopped").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = cluster.getMaster();
        // Refresh after master restart
        ...
        TEST_UTIL.waitFor(120000, 200, new Waiter.Predicate<Exception>() {
            ...
            public boolean evaluate() throws Exception {
                return !finalServerManager.isServerOnline(priorMetaServerName) && !finalServerManager.areDeadServersInProgress();
            }
        });
        LOG.info("Past wait on RIT");
        TEST_UTIL.waitUntilNoRegionsInTransition(60000);
        RestartFramework.at("after_get_meta_server").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_delete_zk_node").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
