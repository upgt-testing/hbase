...
public class TestRpcConnectionRegistry_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRpcConnectionRegistry_RestartInjected.class);
    ...
    private void setMaxNodeCount(int count) {
        UTIL.getMiniHBaseCluster().getMasterThreads().stream().map(t -> t.getMaster().getConfiguration()).forEach(conf -> conf.setInt(RSRpcServices.CLIENT_BOOTSTRAP_NODE_LIMIT, count));
        UTIL.getMiniHBaseCluster().getRegionServerThreads().stream().map(t -> t.getRegionServer().getConfiguration()).forEach(conf -> conf.setInt(RSRpcServices.CLIENT_BOOTSTRAP_NODE_LIMIT, count));
    }
    ...
    public void testRegistryRPCs() throws Exception {
        ...
        assertThat(initialParsedServers, hasItem(ServerName.valueOf(activeMaster.getServerName().getHostname(), activeMaster.getServerName().getPort(), -1)));
        // Since our initial delay is 1 second, finally we should have refreshed the endpoints
        UTIL.waitFor(5000, () -> registry.getParsedServers().contains(activeMaster.getServerManager().getOnlineServersList().get(0)));
        Set<ServerName> parsedServers = registry.getParsedServers();
        assertThat(parsedServers, hasSize(activeMaster.getServerManager().getOnlineServersList().size()));
        assertThat(parsedServers, hasItems(activeMaster.getServerManager().getOnlineServersList().toArray(new ServerName[0])));
        ...
        RestartFramework.at("after_metadata_verify").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        setMaxNodeCount(1);
        RestartFramework.at("after_meta_replicas_ready").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_endpoints_refresh").on(UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBrokenBootstrapNodes() throws Exception {
        ...
        conf.set(RpcConnectionRegistry.BOOTSTRAP_NODES, badServer.toShortString() + ", " + currentMasterAddrs);
