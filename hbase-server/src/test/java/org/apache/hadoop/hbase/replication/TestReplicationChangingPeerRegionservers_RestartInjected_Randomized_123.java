...
public class TestReplicationChangingPeerRegionservers_RestartInjected_Randomized_123 extends TestReplicationBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationChangingPeerRegionservers_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestReplicationChangingPeerRegionservers_RestartInjected.class);
    ...
    public void testChangingNumberOfPeerRegionServers() throws IOException, InterruptedException {
        ...
        RestartFramework.at("after_third_put").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_peer_rs_stop").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        int rsToStop = peerCluster.getServerWithMeta() == 0 ? 1 : 0;
        RestartFramework.at("after_peer_rs_restart").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_put").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        doPutTest(Bytes.toBytes(2));
        RestartFramework.at("after_second_put").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_peer_rs_start").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        peerCluster.startRegionServer();
        // Sanity check
