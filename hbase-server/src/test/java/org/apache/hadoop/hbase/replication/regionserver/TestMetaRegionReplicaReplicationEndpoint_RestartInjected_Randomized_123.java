...
public class TestMetaRegionReplicaReplicationEndpoint_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMetaRegionReplicaReplicationEndpoint_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestMetaRegionReplicaReplicationEndpoint_RestartInjected.class);
    ...
    public void before() throws Exception {
        ...
        RestartFramework.at("after_cluster_start_and_meta_replicas_ready").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        HTU.waitFor(30000, () -> HTU.getMiniHBaseCluster().getRegions(TableName.META_TABLE_NAME).size() >= numOfMetaReplica);
    }
    ...
    private void testHBaseMetaReplicatesOneRow(int i) throws Exception {
        ...
        try (Table table = HTU.createTable(TableName.valueOf(this.name.getMethodName() + "_" + i), HConstants.CATALOG_FAMILY)) {
    ...
    public void testHBaseMetaReplicates() throws Exception {
        try (Table table = HTU.createTable(TableName.valueOf(this.name.getMethodName() + "_0"), HConstants.CATALOG_FAMILY, Arrays.copyOfRange(HBaseTestingUtility.KEYS, 1, HBaseTestingUtility.KEYS.length))) {
            verifyReplication(TableName.META_TABLE_NAME, numOfMetaReplica, getMetaCells(table.getName()));
        }
        try (Table table = HTU.createTable(TableName.valueOf(this.name.getMethodName() + "_1"), HConstants.CATALOG_FAMILY, Arrays.copyOfRange(HBaseTestingUtility.KEYS, 1, HBaseTestingUtility.KEYS.length))) {
            verifyReplication(TableName.META_TABLE_NAME, numOfMetaReplica, getMetaCells(table.getName()));
            // Try delete.
    ...
    public void testCatalogReplicaReplicationWALRolledAndDeleted() throws Exception {
        ...
        try (Connection connection = ConnectionFactory.createConnection(HTU.getConfiguration());
            ...
            HTU.waitFor(2000, 100, true, () -> {
                ...
                source.logQueue.getQueues().keySet().forEach(w -> readerWaiting.setValue(readerWaiting.getValue() && source.workerThreads.get(w).entryReader.waitingPeerEnabled.get()));
            ...
            HTU.getHBaseCluster().getMaster().getLogCleaner().triggerCleanerNow().get(1, TimeUnit.SECONDS);
    ...
    protected void verifyReplication(TableName tableName, int regionReplication, final int startRow, final int endRow, final byte[] family) throws Exception {
    ...
    private void waitForMetaReplicasToOnline() throws IOException {
        final RegionLocator regionLocator = HTU.getConnection().getRegionLocator(TableName.META_TABLE_NAME);
        HTU.waitFor(10000, // getRegionLocations returns an entry for each replica but if unassigned, entry is null.
        // Pass reload to force us to skip cache else it just keeps returning default.
        () -> regionLocator.getRegionLocations(HConstants.EMPTY_START_ROW, true).stream().filter(Objects::nonNull).count() >= numOfMetaReplica);
    ...
    public void testHBaseMetaReplicaGets() throws Exception {
        ...
        try (Table table = HTU.createTable(tn, HConstants.CATALOG_FAMILY, Arrays.copyOfRange(HBaseTestingUtility.KEYS, 1, HBaseTestingUtility.KEYS.length))) {
            verifyReplication(TableName.META_TABLE_NAME, numOfMetaReplica, getMetaCells(table.getName()));
            ...
            primaryIncreaseReplicaIncrease(readReqsForMetaReplicasAfterGet, readReqsForMetaReplicasAfterGetAllLocations);
            ...
            primaryIncreaseReplicaNoChange(readReqsForMetaReplicasAfterGetAllLocations, readReqsForMetaReplicasAfterMove);
            // Move region again.
            ...
            primaryMayIncreaseReplicaNoChange(readReqsForMetaReplicasAfterMove, readReqsForMetaReplicasAfterSecondMove);
            doNGets(tableForGet, getRows);
            ...
            // There are read requests increase for primary meta replica.
            // For rest of meta replicas, there is no change.
            primaryIncreaseReplicaNoChange(readReqsForMetaReplicasAfterSecondMove, readReqsForMetaReplicasAfterThirdGet);
        }
    }
}
