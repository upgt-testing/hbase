 ...
 */
public abstract class WALCorruptionDueToDanglingByteBufferTestBase_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(WALCorruptionDueToDanglingByteBufferTestBase_RestartInjected.class);
    ...
    public void test() throws Exception {
        ...
        try (Table table = UTIL.getConnection().getTable(TABLE_NAME)) {
            ...
            for (int i = 0; i < 100; i++) {
                table.batch(Arrays.asList(new Put(getBytes("row", i)).addColumn(CF, CQ, getBytes("value", i)).setDurability(Durability.ASYNC_WAL)), new Object[1]);
            }
            RestartFramework.at("after_async_puts").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ARRIVE.await();
            ARRIVE = null;
