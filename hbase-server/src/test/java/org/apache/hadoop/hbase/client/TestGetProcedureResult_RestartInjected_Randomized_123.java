...
public class TestGetProcedureResult_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestGetProcedureResult_RestartInjected.class);
    ...
    public void testRace() throws Exception {
        ...
        RestartFramework.at("after_failure_set").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DummyProcedure p = new DummyProcedure();
        long procId = executor.submitProcedure(p);
        RestartFramework.at("after_rollback_allowed").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
