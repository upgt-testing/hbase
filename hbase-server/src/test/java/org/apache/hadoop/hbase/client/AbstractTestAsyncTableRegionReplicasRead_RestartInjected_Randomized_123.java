...
public abstract class AbstractTestAsyncTableRegionReplicasRead_RestartInjected_Randomized_123 {
    ...
    public static List<Object[]> params() {
        return Arrays.asList(new Supplier<?>[] { AbstractTestAsyncTableRegionReplicasRead_RestartInjected::getRawTable }, new Supplier<?>[] { AbstractTestAsyncTableRegionReplicasRead_RestartInjected::getTable });
    }
    ...
    protected static int getSecondaryGetCount() {
        return REPLICA_ID_TO_COUNT.entrySet().stream().filter(e -> e.getKey().intValue() != RegionReplicaUtil.DEFAULT_REPLICA_ID).mapToInt(e -> e.getValue().get()).sum();
    }
    ...
    public void testNoReplicaRead() throws Exception {
        ...
        Thread.sleep(5000);
        RestartFramework.at("after_read_check").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReplicaRead() throws Exception {
        ...
        REPLICA_ID_TO_COUNT.clear();
        RestartFramework.at("after_replica_read").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReadSpecificReplica() throws Exception {
        ...
        RestartFramework.at("after_specific_replica_reads").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
}
