...
public class TestRegionsOnMasterOptions_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionsOnMasterOptions_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRegionsOnMasterOptions_RestartInjected.class);
    ...
    // ns and meta -- no acl unless enabled.
    ...
    private void checkBalance(int masterCount, int rsCount) throws Exception {
        StartMiniClusterOption option = StartMiniClusterOption.builder().numMasters(MASTERS).numRegionServers(SLAVES).numDataNodes(SLAVES).build();
        ...
        try {
            ...
            RestartFramework.at("after_master_failover").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void checkCount(int actual, int expected) {
        assertTrue("Actual=" + actual + ", expected=" + expected, // Lots of slop +/- 2
