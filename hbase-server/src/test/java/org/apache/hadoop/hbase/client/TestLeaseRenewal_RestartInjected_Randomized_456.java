...
public class TestLeaseRenewal_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestLeaseRenewal_RestartInjected.class);
    ...
    public void testLeaseRenewal() throws Exception {
        ...
        RestartFramework.at("after_scanner_open").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        table.put(p);
        ...
        assertTrue(rs.renewLease());
        Thread.sleep(leaseTimeout / 2);
        RestartFramework.at("after_lease_renewal").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(rs.renewLease());
        ...
        assertFalse(rs.renewLease());
        RestartFramework.at("after_read_all_data").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
