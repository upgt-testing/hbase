...
public class TestRetainAssignmentOnRestart_RestartInjected_Randomized_456 extends AbstractTestRestartCluster {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRetainAssignmentOnRestart_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRetainAssignmentOnRestart_RestartInjected.class);
    ...
    public void testRetainAssignmentOnClusterRestart() throws Exception {
        ...
        LOG.info("\n\nSleeping a bit");
        RestartFramework.at("after_snapshot_init").on(cluster).restart("regionserver").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(2000);
        ...
        RestartFramework.at("after_setup_cluster").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void setupCluster() throws Exception, IOException, InterruptedException {
        ...
        UTIL.startMiniCluster(StartMiniClusterOption.builder().masterClass(HMasterForTest.class).numRegionServers(NUM_OF_RS).build());
        // Turn off balancer
