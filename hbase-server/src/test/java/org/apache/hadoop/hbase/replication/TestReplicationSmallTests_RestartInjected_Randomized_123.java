...
public class TestReplicationSmallTests_RestartInjected_Randomized_123 extends TestReplicationBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationSmallTests_RestartInjected.class);
    ...
    public void testDeleteTypes() throws Exception {
        ...
        Put put = new Put(row);
        RestartFramework.at("after_version_delete").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        htable1.delete(d);
        RestartFramework.at("after_column_delete").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        get = new Get(row);
        get.readAllVersions();
        for (int i = 0; i < NB_RETRIES; i++) {
            if (i == NB_RETRIES - 1) {
        ...
        // at the replica
        get = new Get(row);
        RestartFramework.at("after_three_version_puts").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < NB_RETRIES; i++) {
            if (i == NB_RETRIES - 1) {
    ...
    public void testDisableEnable() throws Exception {
        ...
        RestartFramework.at("after_disable_peer").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Put put = new Put(rowkey);
        put.addColumn(famName, row, row);
        RestartFramework.at("after_enable_peer").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        htable1.put(put);
    ...
    public void testAddAndRemoveClusters() throws Exception {
        ...
        RestartFramework.at("after_remove_peer").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(SLEEP_TIME);
        ...
        RestartFramework.at("after_add_peer").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        for (int i = 0; i < retries; i++) {
            ...
            if (res.length != NB_ROWS_IN_BIG_BATCH) {
                if (i == retries - 1) {
                    ...
                    fail("Waited too much time for normal batch replication, " + res.length + " instead of " + NB_ROWS_IN_BIG_BATCH + "; waited=" + (EnvironmentEdgeManager.currentTime() - start) + "ms");
                } else {
    ...
    public void testVerifyListReplicatedTable() throws Exception {
        ...
        RestartFramework.at("after_tables_create").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReplicationInReplay() throws Exception {
        ...
        RestartFramework.at("after_wal_edit_replay").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetReplicationPeerState() throws Exception {
        RestartFramework.at("after_disable_check_state").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_enable_check_state").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
