...
public class TestMasterFileSystem_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterFileSystem_RestartInjected.class);

    @Rule
    public TestName name = new TestName();

    private static final Logger LOG = LoggerFactory.getLogger(TestMasterFileSystem_RestartInjected.class);
    ...
    public void testFsUriSetProperly() throws Exception {
        ...
        RestartFramework.at("after_get_master_filesystem").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckNoTempDir() throws Exception {
        ...
        RestartFramework.at("after_load_table").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
