...
public class TestScannerBlockSizeLimits_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestScannerBlockSizeLimits_RestartInjected.class);
    ...
    public void testSingleBlock() throws IOException {
        ...
        ResultScanner scanner = table.getScanner(getBaseScan().withStartRow(Bytes.toBytes(1)).withStopRow(Bytes.toBytes(2)).addColumn(FAMILY1, COLUMN1).addColumn(FAMILY1, COLUMN2).setReadType(Scan.ReadType.STREAM));
        ...
        RestartFramework.at("after_scan_complete").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckLimitAfterFilterRowKey() throws IOException {
        Table table = TEST_UTIL.getConnection().getTable(TABLE);
        ResultScanner scanner = table.getScanner(getBaseScan().addColumn(FAMILY1, COLUMN1).addColumn(FAMILY1, COLUMN2).addColumn(FAMILY1, COLUMN3).addFamily(FAMILY2).setFilter(new RowFilter(CompareOperator.NOT_EQUAL, new BinaryComparator(Bytes.toBytes(2)))));
        RestartFramework.at("after_filter_row_key_scan").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ScanMetrics metrics = scanner.getScanMetrics();
        ...
        for (Result result : scanner) {
            ...
            if (rows.contains(3)) {
                assertFalse("expected row3 to come all in one result, but found it in two results", foundRow3);
    ...
    public void testCheckLimitAfterFilteringRowCellsDueToFilterRow() throws IOException {
        Table table = TEST_UTIL.getConnection().getTable(TABLE);
        ResultScanner scanner = table.getScanner(getBaseScan().withStartRow(Bytes.toBytes(1), true).addColumn(FAMILY1, COLUMN1).addColumn(FAMILY1, COLUMN2).setReadType(Scan.ReadType.STREAM).setFilter(new SkipFilter(new QualifierFilter(CompareOperator.EQUAL, new BinaryComparator(Bytes.toBytes("dasfasf"))))));
        // Our filter doesn't end up matching any real columns, so expect only cursors
        for (Result result : scanner) {
            assertTrue(result.isCursor());
        }
        RestartFramework.at("after_filter_row_scan").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ScanMetrics metrics = scanner.getScanMetrics();
    ...
    public void testCheckLimitAfterFilteringCell() throws IOException {
        Table table = TEST_UTIL.getConnection().getTable(TABLE);
        ResultScanner scanner = table.getScanner(getBaseScan().setCaching(1).setFilter(new QualifierFilter(CompareOperator.EQUAL, new BinaryComparator(COLUMN2))));
        int cursors = 0;
        for (Result result : scanner) {
            if (result.isCursor()) {
                cursors++;
        ...
        RestartFramework.at("after_filter_cell_scan").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckLimitAfterFilteringRowCells() throws IOException {
        Table table = TEST_UTIL.getConnection().getTable(TABLE);
        ResultScanner scanner = table.getScanner(getBaseScan().withStartRow(Bytes.toBytes(1), true).addColumn(FAMILY1, COLUMN1).setReadType(Scan.ReadType.STREAM).setFilter(new SingleColumnValueExcludeFilter(FAMILY1, COLUMN1, CompareOperator.EQUAL, new BinaryComparator(DATA))));
        ...
        RestartFramework.at("after_filter_row_cells_scan").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ScanMetrics metrics = scanner.getScanMetrics();
    ...
    public void testSeekNextUsingHint() throws IOException {
        Table table = TEST_UTIL.getConnection().getTable(TABLE);
        RestartFramework.at("after_seek_scan").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ResultScanner scanner = table.getScanner(getBaseScan().addFamily(FAMILY1).setFilter(new ColumnPaginationFilter(1, COLUMN5)));
        scanner.next(100);
        ScanMetrics metrics = scanner.getScanMetrics();
