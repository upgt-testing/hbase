...
public class TestRollbackSCP_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRollbackSCP_RestartInjected.class);
    ...
    private static final class AssignmentManagerForTest extends AssignmentManager {
        ...
        void persistToMeta(RegionStateNode regionNode) throws IOException {
            ...
            if (!regionNode.getRegionInfo().isMetaRegion() && proc.hasParent()) {
                Procedure<?> p = getMaster().getMasterProcedureExecutor().getProcedure(proc.getRootProcId());
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        RestartFramework.at("after_cluster_start").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.waitTableAvailable(TABLE_NAME);
        UTIL.getAdmin().balance(BalanceRequest.newBuilder().setIgnoreRegionsInTransition(true).build());
        UTIL.waitUntilNoRegionsInTransition();
        RestartFramework.at("after_balance").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private ServerCrashProcedure getSCPForServer(ServerName serverName) throws IOException {
        return UTIL.getMiniHBaseCluster().getMaster().getProcedures().stream().filter(p -> p instanceof ServerCrashProcedure).map(p -> (ServerCrashProcedure) p).filter(p -> p.getServerName().equals(serverName)).findFirst().orElse(null);
    }
    ...
    private void dumpAllProcedures(String label, ProcedureExecutor<MasterProcedureEnv> procExec) {
        ...
        procExec.getProcedures().forEach(p -> {
            System.out.println("DEBUG-GROUP55: [" + label + "] Proc: procId=" + p.getProcId() + ", class=" + p.getClass().getSimpleName() + ", state=" + p.getState() + ", hasParent=" + p.hasParent() + ", parentProcId=" + (p.hasParent() ? p.getParentProcId() : -1) + ", rootProcId=" + p.getRootProcId() + ", isFinished=" + p.isFinished() + ", wasExecuted=" + p.wasExecuted());
        });
    ...
    public void testFailAndRollback() throws Exception {
        ...
        RestartFramework.at("after_rs_kill").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        System.out.println("DEBUG-GROUP55: SCP created with procId=" + scp.getProcId());
        dumpAllProcedures("AFTER_SCP_CREATED", procExec);
        RestartFramework.at("after_verify_exception").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_verify_subprocs_cleaned").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_rs_with_meta").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        int iteration = 0;
        while (scp.getState() != ProcedureState.FAILED || !currentProcExec.isRunning()) {
            ...
            System.out.println("DEBUG-GROUP55: SCP state=" + scp.getState() + ", procExec.isRunning=" + currentProcExec.isRunning());
            ...
            RestartFramework.at("after_procedure_restart").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Refresh after master restart
            currentProcExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
            ProcedureTestingUtility.waitProcedure(currentProcExec, scp);
        }
        RestartFramework.at("after_scp_failed").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_scp_detected").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(scp.getState(), ProcedureState.FAILED);
        RestartFramework.at("after_procedure_stop").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
