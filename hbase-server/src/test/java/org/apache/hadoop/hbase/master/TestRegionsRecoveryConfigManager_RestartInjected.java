...
public class TestRegionsRecoveryConfigManager_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestRegionsRecoveryConfigManager_RestartInjected.class);
  ...
  public void testChoreSchedule() throws Exception {
    ...
    RestartFramework.at("after_get_master")
        .on(cluster)
        .restart("master")
        .withIndex(0)
    ...
    RestartFramework.at("after_create_config_manager")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_first_chore_check")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_first_config_change")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    assertFalse(
      hMaster.getChoreService().isChoreScheduled(regionsRecoveryConfigManager.getChore()));
    RestartFramework.at("after_second_chore_check")
        .on(cluster)
    ...
    this.regionsRecoveryConfigManager.onConfigurationChange(conf);
    RestartFramework.at("after_set_check_interval")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    assertFalse(
      hMaster.getChoreService().isChoreScheduled(regionsRecoveryConfigManager.getChore()));
    RestartFramework.at("after_third_chore_check")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    this.regionsRecoveryConfigManager.onConfigurationChange(conf);
    RestartFramework.at("after_set_store_file_ref_count")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_chore_scheduled")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    this.regionsRecoveryConfigManager.onConfigurationChange(conf);
    RestartFramework.at("after_change_store_file_ref_count_to_20")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    // chore re-scheduled
    assertTrue(hMaster.getChoreService().isChoreScheduled(regionsRecoveryConfigManager.getChore()));
    RestartFramework.at("after_chore_rescheduled")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    conf.setInt("hbase.regions.recovery.store.file.ref.count", 20);
    this.regionsRecoveryConfigManager.onConfigurationChange(conf);
    RestartFramework.at("after_config_change_same_value")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    assertTrue(hMaster.getChoreService().isChoreScheduled(regionsRecoveryConfigManager.getChore()));
    RestartFramework.at("after_chore_untouched_check")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    conf.unset("hbase.regions.recovery.store.file.ref.count");
    this.regionsRecoveryConfigManager.onConfigurationChange(conf);
    RestartFramework.at("after_unset_config")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    assertFalse(
      hMaster.getChoreService().isChoreScheduled(regionsRecoveryConfigManager.getChore()));
    RestartFramework.at("after_chore_unscheduled")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
