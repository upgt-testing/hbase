...
public class TestHbckMetricsResource_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestHbckMetricsResource_RestartInjected.class);
    ...
    private static final String hbckOrphanRegionOnFS = quote + "hbck_orphan_regions_on_fs" + quoteColon;

    private static final String hbckOrphanRegionOnRS = quote + "hbck_orphan_regions_on_rs" + quoteColon;

    private static final String hbckInconsistentRegion = quote + "hbck_inconsistent_regions" + quoteColon;
    ...
    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestHbckMetricsResource_RestartInjected.class);
    ...
    private static final class ClassSetup extends ExternalResource {
        ...
        public ClassSetup(final Supplier<AsyncConnection> connectionSupplier) {
            ...
            tableName = TableName.valueOf(TestHbckMetricsResource_RestartInjected.class.getSimpleName());
        }
        ...
        protected void before() throws Throwable {
            RestartFramework.at("after_table_create").on(miniClusterRule.getTestingUtility().getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final AsyncConnection conn = connectionSupplier.get();
            admin = conn.getAdmin();
            final TableDescriptor tableDescriptor = TableDescriptorBuilder.newBuilder(tableName).setColumnFamily(ColumnFamilyDescriptorBuilder.newBuilder(Bytes.toBytes("c")).build()).setDurability(Durability.SKIP_WAL).build();
            ...
            // Test data for Mock HBCK Report
            ServerName server1 = ServerName.valueOf(localhost1, Integer.parseInt(port), Integer.parseInt(hostStartCode));
            ServerName server2 = ServerName.valueOf(localhost2, Integer.parseInt(port), Integer.parseInt(hostStartCode));
            ...
            RestartFramework.at("after_mock_setup").on(miniClusterRule.getTestingUtility().getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            when(hbckReport.getCheckingStartTimestamp()).thenReturn(Instant.ofEpochMilli(reportStartTime));
            ...
            admin = conn.getAdmin();
            final String baseUrl = admin.getMaster().thenApply(ServerName::getHostname).thenCombine(admin.getMasterInfoPort(), (hostName, infoPort) -> "http://" + hostName + ":" + infoPort).get();
        ...
        protected void after() {
            ...
            try {
                admin.tableExists(tableName).thenCompose(val -> {
                    if (val) {
                        return admin.disableTable(tableName).thenCompose(ignored -> admin.deleteTable(tableName));
                    } else {
    ...
    @ClassRule
    public static RuleChain ruleChain = RuleChain.outerRule(miniClusterRule).around(connectionRule).around(classRule);

    @Test
    public void testGetRoot() {
        final String response = classRule.getTarget().request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK JSON Response : " + response);
        assertThat(response, allOf(containsString(hbckReportStartTime), containsString(hbckReportEndTime), containsString(hbckOrphanRegionOnFS), containsString(hbckOrphanRegionOnRS), containsString(hbckInconsistentRegion), containsString(hbckHoles), containsString(hbckOverlaps), containsString(hbckUnknownServers), containsString(hbckEmptyRegionInfo), containsString(Objects.toString(reportStartTime)), containsString(Objects.toString(reportEndTime))));
    }

    @Test
    public void testGetRootHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }

    @Test
    public void testGetOrphanRegionOnFS() {
        final String response = classRule.getTarget().path("orphan-regions-on-fs").request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK Response for resource orphan-regions-on-fs : " + response);
        assertThat(response, allOf(startsWith(dataStartsWith), endsWith(dataEndsWith), containsString(regionId), containsString(regionHdfsPath), containsString(regionId1), containsString(regionId2), containsString(path1), containsString(path2)));
    }

    @Test
    public void testGetOrphanRegionOnFSHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().path("orphan-regions-on-fs").request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }

    @Test
    public void testGetOrphanRegionOnRS() {
        final String response = classRule.getTarget().path("orphan-regions-on-rs").request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK Response for resource orphan-regions-on-rs : " + response);
        assertThat(response, allOf(startsWith(dataStartsWith), endsWith(dataEndsWith), containsString(regionId), containsString(rsName), containsString(hostName), containsString(hostPort), containsString(startCode), containsString(regionId1), containsString(regionId2), containsString(localhost1), containsString(localhost2), containsString(port), containsString(hostStartCode)));
    }

    @Test
    public void testGetOrphanRegionOnRSHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().path("orphan-regions-on-rs").request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }

    @Test
    public void testGetInconsistentRegions() {
        final String response = classRule.getTarget().path("inconsistent-regions").request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK Response for resource inconsistent-regions : " + response);
        assertThat(response, allOf(startsWith(dataStartsWith), endsWith(dataEndsWith), containsString(hostName), containsString(hostPort), containsString(startCode), containsString(listOfServers), containsString(regionId1), containsString(regionId2), containsString(regionId), containsString(serverNameInMeta), containsString(localhost1), containsString(localhost2), containsString(port), containsString(hostStartCode)));
    }

    @Test
    public void testGetInconsistentRegionsHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().path("inconsistent-regions").request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }

    @Test
    public void testGetRegionHoles() {
        final String response = classRule.getTarget().path("region-holes").request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK Response for resource region-holes : " + response);
        assertThat(response, allOf(startsWith(dataStartsWith), endsWith(dataEndsWith), containsString(region1Info), containsString(region2Info), containsString(regionId), containsString(tableName), containsString(metaRegionID), containsString(metaTableName)));
    }

    @Test
    public void testGetRegionHolesHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().path("region-holes").request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }

    @Test
    public void testGetRegionOverlaps() {
        final String response = classRule.getTarget().path("region-overlaps").request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK Response for resource region-overlaps : " + response);
        assertThat(response, allOf(startsWith(dataStartsWith), endsWith(dataEndsWith), containsString(regionId), containsString(tableName), containsString(region2Info), containsString(region2Info), containsString(metaRegionID), containsString(metaTableName)));
    }

    @Test
    public void testGetRegionOverlapsHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().path("region-overlaps").request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }

    @Test
    public void testGetUnkownServers() {
        final String response = classRule.getTarget().path("unknown-servers").request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK Response for resource unknown-servers : " + response);
        assertThat(response, allOf(startsWith(dataStartsWith), endsWith(dataEndsWith), containsString(regionInfo), containsString(regionId), containsString(tableName), containsString(serverName), containsString(serverName), containsString(port), containsString(startCode), containsString(metaRegionID), containsString(metaTableName), containsString(localhost1), containsString(localhost2), containsString(port), containsString(startCode)));
    }

    @Test
    public void testGetUnkownServersHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().path("unknown-servers").request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }

    @Test
    public void testGetEmptyRegionInfo() {
        final String response = classRule.getTarget().path("empty-regioninfo").request(MediaType.APPLICATION_JSON_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class);
        LOG.info("HBCK Response for resource empty-regioninfo : " + response);
        assertThat(response, allOf(startsWith(dataStartsWith), endsWith(dataEndsWith), containsString(regionInfo), containsString(regionId1), containsString(regionId2)));
    }

    @Test
    public void testGetEmptyRegionInfoHtml() {
        assertThrows(NotAcceptableException.class, () -> classRule.getTarget().path("empty-regioninfo").request(MediaType.TEXT_HTML_TYPE).header("X-Jersey-Tracing-Accept", true).get(String.class));
    }
}
