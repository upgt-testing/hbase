...
public class TestRaceBetweenSCPAndTRSP_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRaceBetweenSCPAndTRSP_RestartInjected.class);
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_region_opening_arrive").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        // Kill the region server and trigger a SCP
        UTIL.getMiniHBaseCluster().killRegionServer(sn);
        RestartFramework.at("after_get_scp_procid").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        // Wait until the SCP reaches the getRegionsOnServer call
        arriveGetRegionsOnServer.await();
        RestartFramework.at("after_resume_region_opening").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_scp_finished").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_move_future_get").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RSProcedureDispatcher remoteDispatcher = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor().getEnvironment().getRemoteDispatcher();
        ...
        RestartFramework.at("after_wait_remote_dispatcher").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        moveFuture.get();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        ...
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        RestartFramework.at("after_move_async").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
