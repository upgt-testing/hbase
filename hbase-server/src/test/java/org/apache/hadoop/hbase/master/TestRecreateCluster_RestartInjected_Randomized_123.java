...
public class TestRecreateCluster_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRecreateCluster_RestartInjected.class);
    ...
    public void setup() throws Exception {
        ...
        TEST_UTIL.startMiniCluster(StartMiniClusterOption.builder().numRegionServers(NUM_RS).numDataNodes(NUM_RS).createWALDir(true).build());
    }
    ...
    private void validateRecreateClusterWithUserDisabled(boolean cleanupWALs, boolean cleanUpZNodes) throws Exception {
        ...
        RestartFramework.at("after_table_enable").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TEST_UTIL.getAdmin().disableTable(tableName);
        RestartFramework.at("after_prepare_data").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void validateRecreateClusterWithUserTableEnabled(boolean cleanupWALs, boolean cleanUpZNodes) throws Exception {
        ...
        prepareDataBeforeRecreate(TEST_UTIL, tableName);
        restartHBaseCluster(cleanupWALs, cleanUpZNodes);
        RestartFramework.at("after_prepare_data_enabled").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        validateDataAfterRecreate(TEST_UTIL, tableName);
    }

    private void restartHBaseCluster(boolean cleanUpWALs, boolean cleanUpZnodes) throws Exception {
        RestartFramework.at("after_procedures_done").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        List<ServerName> oldServers = TEST_UTIL.getHBaseCluster().getMaster().getServerManager().getOnlineServersList();
        ...
        TEST_UTIL.waitFor(TIMEOUT_MS, () -> TEST_UTIL.getMiniHBaseCluster().getNumLiveRegionServers() == NUM_RS);
        // make sure we have a new set of region servers with different hostnames and ports
        List<ServerName> newServers = TEST_UTIL.getHBaseCluster().getMaster().getServerManager().getOnlineServersList();
        assertFalse(newServers.stream().filter(newServer -> oldServers.contains(newServer)).findAny().isPresent());
    }
    ...
    private void ensureTableNotColocatedWithSystemTable(TableName userTable, TableName systemTable) throws IOException, InterruptedException {
        ...
        TEST_UTIL.moveRegionAndWait(TEST_UTIL.getAdmin().getRegions(userTable).get(0), destServer.getServerName());
    ...
    private void validateDataAfterRecreate(HBaseTestingUtility testUtil, TableName tableName) throws Exception {
        ...
        assertEquals("v", Bytes.toString(cell.getValueArray(), cell.getValueOffset(), cell.getValueLength()));
