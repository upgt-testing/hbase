...
public class TestAdmin1_RestartInjected_Randomized_123 extends TestAdminBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAdmin1_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestAdmin1_RestartInjected.class);

    @Test
    public void testCompactRegionWithTableName() throws Exception {
        TableName tableName = TableName.valueOf(name.getMethodName());
        try {
            TableDescriptor htd = TableDescriptorBuilder.newBuilder(tableName).setColumnFamily(ColumnFamilyDescriptorBuilder.of("fam1")).build();
            ...
            RestartFramework.at("after_compact_attempt").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCompactATableWithSuperLongTableName() throws Exception {
        ...
        try {
            ADMIN.createTable(htd);
            RestartFramework.at("after_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertThrows(IllegalArgumentException.class, () -> ADMIN.majorCompactRegion(tableName.getName()));
            assertThrows(IllegalArgumentException.class, () -> ADMIN.majorCompactRegion(Bytes.toBytes("abcd")));
        } finally {
            ADMIN.disableTable(tableName);
            ADMIN.deleteTable(tableName);
        }
    ...
    private void splitTest(byte[] splitPoint, byte[][] familyNames, int[] rowCounts, int numVersions, int blockSize, boolean async) throws Exception {
        ...
        try (final Table table = TEST_UTIL.createTable(tableName, familyNames, numVersions, blockSize);
            ...
            try (ResultScanner scanner = table.getScanner(scan)) {
                ...
                RestartFramework.at("after_region_split").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                // Verify row count
                // We counted one row above.
                rows = 1 + count(scanner);
            }
            assertEquals(rowCount, rows);
            List<HRegionLocation> regions = null;
            try {
            ...
            if (splitPoint != null) {
                ...
                assertEquals(Bytes.toString(splitPoint), Bytes.toString(regions.get(0).getRegion().getEndKey()));
                assertEquals(Bytes.toString(splitPoint), Bytes.toString(regions.get(1).getRegion().getStartKey()));
                ...
                if (familyNames.length > 1) {
                    ...
                    LOG.debug("SplitKey=" + splitKey + "&deltaForLargestFamily=" + deltaForLargestFamily + ", r=" + regions.get(0).getRegion());
                    for (int index = 0; index < familyNames.length; index++) {
    ...
    public void testSplitAndMergeWithReplicaTable() throws Exception {
        ...
        try {
            TEST_UTIL.getHBaseAdmin().splitRegionAsync(regions.get(1).getFirst(), new byte[] { (byte) '1' });
        ...
        try {
            TEST_UTIL.getAdmin().splitRegionAsync(regions.get(1).getFirst().getRegionName(), new byte[] { (byte) '1' }).get();
        } catch (IllegalArgumentException ex) {
            gotException = true;
        }
        RestartFramework.at("after_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            TEST_UTIL.getAdmin().mergeRegionsAsync(regions.get(1).getFirst().getEncodedNameAsBytes(), regions.get(2).getFirst().getEncodedNameAsBytes(), true).get();
    ...
    public void testMergeRegions() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_first_merge").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            tableRegions = ADMIN.getRegions(tableName);
            ...
            RestartFramework.at("after_second_merge").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMergeRegionsInvalidRegionCount() throws IOException, InterruptedException, ExecutionException {
        ...
        try {
            ...
            // 1
            try {
                ADMIN.mergeRegionsAsync(new byte[][] { tableRegions.get(0).getEncodedNameAsBytes() }, false).get();
                fail();
            } catch (IllegalArgumentException e) {
                // expected
            }
    ...
    public void testSplitShouldNotHappenIfSplitIsDisabledForTable() throws Exception {
        RestartFramework.at("after_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final TableName tableName = TableName.valueOf(name.getMethodName());
        TableDescriptor htd = TableDescriptorBuilder.newBuilder(tableName).setRegionSplitPolicyClassName(DisabledRegionSplitPolicy.class.getName()).setColumnFamily(ColumnFamilyDescriptorBuilder.of("f")).build();
        Table table = TEST_UTIL.createTable(htd, null);
        for (int i = 0; i < 10; i++) {
        ...
        List<RegionInfo> allRegions = MetaTableAccessor.getTableRegions(ADMIN.getConnection(), tableName, true);
    ...
    public void testTruncateRegions() throws Exception {
        ...
        final TableName tableName = TableName.valueOf(name.getMethodName());
        final byte[][] splitKeys = new byte[][] { Bytes.toBytes("30"), Bytes.toBytes("60"), Bytes.toBytes("90") };
        String family1 = "f1";
        String family2 = "f2";
        ...
        try {
            ...
            RestartFramework.at("after_inserts").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            List<RegionInfo> tableRegions = ADMIN.getRegions(tableName);
            ...
            RestartFramework.at("after_truncate").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testTruncateReplicaRegionNotAllowed() throws Exception {
        ...
        try {
            ...
            try {
                ...
                // Assert
                assertEquals("Expected message is different", "Can't truncate replicas directly.Replicas are auto-truncated " + "when their primary is truncated.", e.getMessage());
            }
        } finally {
            ADMIN.disableTable(tableName);
            ADMIN.deleteTable(tableName);
    ...
    public void testTruncateRegionMetaTableRegionsNotAllowed() throws Exception {
        ...
        try {
            ...
            String expectedErrorMessage = "Invalid region: " + Bytes.toStringBinary(regionToBeTruncated.getRegionName());
