...
public class TestAdmin2_RestartInjected_Randomized_456 extends TestAdminBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAdmin2_RestartInjected.class);
    ...
    public void testCreateBadTables() throws IOException {
        ...
        RestartFramework.at("after_concurrent_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < count; i++) {
    ...
    public void testCreateTableRPCTimeOut() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_large_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        for (RegionInfo regionInfo : onlineRegions) {
            if (!regionInfo.isMetaRegion()) {
                if (regionInfo.getRegionNameAsString().contains(name)) {
                    info = regionInfo;
                    assertThrows(UnknownRegionException.class, () -> ADMIN.unassign(Bytes.toBytes("test,,1358563771069.acc1ad1b7962564fc3a43e5907e8db33."), true));
                }
            }
        }
        RestartFramework.at("after_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetTableRegions() throws IOException {
        ...
        RestartFramework.at("after_multi_region_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWALRollWriting() throws Exception {
        ...
        RestartFramework.at("after_region_flush").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        while (v.length() < 1000) {
            v.append(className);
        }
        RestartFramework.at("after_wal_writes").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckHBaseAvailableWithoutCluster() {
        ...
        conf.set(HConstants.CLIENT_CONNECTION_REGISTRY_IMPL_CONF_KEY, HConstants.ZK_CONNECTION_REGISTRY_CLASS);
     ...
     */
    private void testGetWithEncodedRegionName(ClusterConnection conn, RegionInfo inputRI) throws IOException {
        RegionInfo ri = ProtobufUtil.getRegionInfo(null, conn.getAdmin(TEST_UTIL.getMiniHBaseCluster().getMaster().getServerName()), inputRI.getEncodedNameAsBytes());
        assertEquals(inputRI, ri);
    }

    private void testGetWithRegionName(ClusterConnection conn, RegionInfo inputRI) throws IOException {
        RegionInfo ri = ProtobufUtil.getRegionInfo(null, conn.getAdmin(TEST_UTIL.getMiniHBaseCluster().getMaster().getServerName()), inputRI.getRegionName());
        assertEquals(inputRI, ri);
    }
    ...
    public void testTableMergeFollowedByModify() throws Exception {
        final TableName tableName = TableName.valueOf(name.getMethodName());
        TEST_UTIL.createTable(tableName, new byte[][] { Bytes.toBytes("f") }, new byte[][] { Bytes.toBytes(3) });
        RestartFramework.at("after_table_modify").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // assert we have at least 2 regions in the table
        List<RegionInfo> regions = ADMIN.getRegions(tableName);
        int originalCount = regions.size();
        RestartFramework.at("after_region_merge").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSlowLogResponses() throws Exception {
        ...
        List<LogEntry> onlineLogRecords = ADMIN.getLogEntries(new HashSet<>(serverNames), "SLOW_LOG", ServerType.REGION_SERVER, 100, null);
        ...
        List<LogEntry> balancerDecisionRecords = ADMIN.getLogEntries(null, "BALANCER_DECISION", ServerType.MASTER, 100, null);
    ...
    private static void waitForServerCommissioned(ServerName excludeServer, boolean anyServerDecommissioned) {
        TEST_UTIL.waitFor(3000, () -> {
            try {
                ...
                if (anyServerDecommissioned) {
                    return decomServers.size() == 1 && decomServers.get(0).equals(excludeServer);
                } else {
