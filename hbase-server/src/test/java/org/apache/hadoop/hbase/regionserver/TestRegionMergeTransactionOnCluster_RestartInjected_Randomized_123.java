...
public class TestRegionMergeTransactionOnCluster_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionMergeTransactionOnCluster_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRegionMergeTransactionOnCluster_RestartInjected.class);
    ...
    public static void beforeAllTests() throws Exception {
        // Start a cluster
        StartMiniClusterOption option = StartMiniClusterOption.builder().masterClass(MyMaster.class).numRegionServers(NB_SERVERS).numDataNodes(NB_SERVERS).build();
    ...
    public void testWholesomeMerge() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_table_create_and_load").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Merge 1st and 2nd region
            mergeRegionsAndVerifyRegionNum(MASTER, tableName, 0, 1, INITIAL_REGION_NUM - 1);
            RestartFramework.at("after_first_merge").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_merge").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verifyRowCount(table, ROWSIZE);
            RestartFramework.at("after_row_verification").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_assign_attempt").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // We should not be able to unassign it either
            am.unassign(hri);
            assertFalse("Merged region can't be unassigned", regionStates.isRegionInTransition(hri));
            RestartFramework.at("after_unassign_attempt").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            table.close();
        } finally {
    ...
    public void testMergeAndRestartingMaster() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_merge_with_restart_simulation").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            table.close();
        } finally {
            TEST_UTIL.deleteTable(tableName);
        }
    ...
    public void testCleanMergeReference() throws Exception {
        ...
        try {
            ...
            // Merge 1st and 2nd region
            mergeRegionsAndVerifyRegionNum(MASTER, tableName, 0, 1, INITIAL_REGION_NUM - 1);
            RestartFramework.at("after_merge").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verifyRowCount(table, ROWSIZE);
            RestartFramework.at("after_row_verification").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            table.close();
            ...
            Result mergedRegionResult = MetaTableAccessor.getRegionResult(MASTER.getConnection(), mergedRegionInfo);
            ...
            RestartFramework.at("after_compaction").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cleaner_chore").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            while (EnvironmentEdgeManager.currentTime() < timeout) {
                int newcount1 = 0;
                for (ColumnFamilyDescriptor colFamily : columnFamilies) {
            ...
            RestartFramework.at("after_catalog_janitor").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Wait around a bit to give stuff a chance to complete.
            while (true) {
                mergedRegionResult = MetaTableAccessor.getRegionResult(TEST_UTIL.getConnection(), mergedRegionInfo);
                if (MetaTableAccessor.hasMergeRegions(mergedRegionResult.rawCells())) {
                    LOG.info("Waiting on cleanup of merge columns {}", Arrays.asList(mergedRegionResult.rawCells()).stream().map(c -> c.toString()).collect(Collectors.joining(",")));
                    Threads.sleep(50);
                } else {
                    break;
                }
    ...
    public void testMerge() throws Exception {
        ...
        try {
            ...
            am.unassign(b);
            am.offlineRegion(b);
            RestartFramework.at("after_region_offline").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_merge_attempts").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin = TEST_UTIL.getAdmin();
            table.close();
        } finally {
            TEST_UTIL.deleteTable(tableName);
    ...
    public void testMergeWithReplicas() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_table_create_with_replicas").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_initial_region_collection").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            // Merge 1st and 2nd region
            PairOfSameType<RegionInfo> mergedRegions = mergeRegionsAndVerifyRegionNum(MASTER, tableName, 0, 2, 5 * 2 - 2);
            RestartFramework.at("after_merge_replicas").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            ...
            RestartFramework.at("after_replica_verification").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            table.close();
        } finally {
            TEST_UTIL.deleteTable(tableName);
    ...
    private PairOfSameType<RegionInfo> mergeRegionsAndVerifyRegionNum(HMaster master, TableName tablename, int regionAnum, int regionBnum, int expectedRegionNum) throws Exception {
        PairOfSameType<RegionInfo> mergedRegions = requestMergeRegion(master, tablename, regionAnum, regionBnum);
    ...
    private PairOfSameType<RegionInfo> requestMergeRegion(HMaster master, TableName tablename, int regionAnum, int regionBnum) throws Exception {
        List<Pair<RegionInfo, ServerName>> tableRegions = MetaTableAccessor.getTableRegionsAndLocations(TEST_UTIL.getConnection(), tablename);
    ...
    private void waitAndVerifyRegionNum(HMaster master, TableName tablename, int expectedRegionNum) throws Exception {
        ...
        while (EnvironmentEdgeManager.currentTime() < timeout) {
            ...
            if (tableRegionsInMetaSize == expectedRegionNum && tableRegionsInMasterSize == expectedRegionNum) {
                break;
            }
            Thread.sleep(250);
        }
    ...
    private Table createTableAndLoadData(HMaster master, TableName tablename, int numRegions, int replication) throws Exception {
        ...
        assertEquals("Wrong number of regions in table " + tablename, numRegions * replication, tableRegions.size());
