...
public abstract class AbstractTestRegionLocator_RestartInjected_Randomized_456 {
    ...
    protected static void startClusterAndCreateTable() throws Exception {
        ...
        TableDescriptor td = TableDescriptorBuilder.newBuilder(TABLE_NAME).setRegionReplication(REGION_REPLICATION).setColumnFamily(ColumnFamilyDescriptorBuilder.of(FAMILY)).build();
        ...
        try (ConnectionRegistry registry = ConnectionRegistryFactory.getRegistry(UTIL.getConfiguration(), User.getCurrent())) {
            RegionReplicaTestHelper.waitUntilAllMetaReplicasAreReady(UTIL, registry);
        }
        RestartFramework.at("after_meta_replicas_ready").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetRegionLocation() throws IOException {
        for (int i = 0; i <= SPLIT_KEYS.length; i++) {
            for (int replicaId = 0; replicaId < REGION_REPLICATION; replicaId++) {
                assertRegionLocation(getRegionLocation(TABLE_NAME, getStartKey(i), replicaId), i, replicaId);
            }
            if (i == SPLIT_KEYS.length / 2) {
                RestartFramework.at("during_region_location_lookup").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }
    ...
    public void testGetRegionLocations() throws IOException {
        for (int i = 0; i <= SPLIT_KEYS.length; i++) {
            ...
            if (i == SPLIT_KEYS.length / 2) {
                RestartFramework.at("during_region_locations_lookup").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }
    ...
    public void testGetAllRegionLocations() throws IOException {
        RestartFramework.at("after_get_all_locations").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void assertMetaRegionLocation(HRegionLocation loc, int replicaId) {
        ...
        assertEquals(replicaId, region.getReplicaId());
        ServerName expected = findRegionLocation(TableName.META_TABLE_NAME, region.getStartKey(), replicaId);
        assertEquals(expected, loc.getServerName());
    }
    ...
    public void testMeta() throws IOException {
        ...
        RestartFramework.at("after_meta_start_end_keys").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int replicaId = 0; replicaId < REGION_REPLICATION; replicaId++) {
            assertMetaRegionLocation(getRegionLocation(TableName.META_TABLE_NAME, HConstants.EMPTY_START_ROW, replicaId), replicaId);
        }
        RestartFramework.at("after_meta_region_locations").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    protected abstract HRegionLocation getRegionLocation(TableName tableName, byte[] row, int replicaId) throws IOException;

    protected abstract List<HRegionLocation> getRegionLocations(TableName tableName, byte[] row) throws IOException;
