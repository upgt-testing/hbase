...
public class TestMasterCoprocessorExceptionWithRemove_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterCoprocessorExceptionWithRemove_RestartInjected.class);
    ...
    public void testExceptionFromCoprocessorWhenCreatingTable() throws IOException {
        ...
        RestartFramework.at("after_get_coprocessor").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_zk_tracker_setup").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_exception_handling").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
