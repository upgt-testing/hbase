...
public class TestMobCompactionWithDefaults_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestMobCompactionWithDefaults_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMobCompactionWithDefaults_RestartInjected.class);
    ...
    protected final static byte[] mobVal = Bytes.toBytes("01234567890123456789012345678901234567890123456789012345678901234567890123456789");
    ...
    public TestMobCompactionWithDefaults_RestartInjected_Randomized_456(Boolean useFileBasedSFT) {
    ...
    public void baseTestMobFileCompaction() throws InterruptedException, IOException {
        ...
        RestartFramework.at("after_mob_compact").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        mobCompact(tableDescriptor, familyDescriptor);
        assertEquals("Should have 4 MOB files per region due to 3xflush + compaction.", numRegions * 4, getNumberOfMobFiles(table, famStr));
        cleanupAndVerifyCounts(table, famStr, 3 * rows);
        RestartFramework.at("after_load_three_times").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cleanup").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMobFileCompactionAfterSnapshotClone() throws InterruptedException, IOException {
        RestartFramework.at("after_snapshot").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_mob_compact_clone").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_hlinks_check").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_clone_snapshot").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mob_file_check_clone").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.cloneSnapshot(TestMobUtils.getTableName(test), clone);
        RestartFramework.at("after_cleanup_clone").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals("Should have 3 hlinks per region in MOB area from snapshot clone", 3 * numRegions, getNumberOfMobFiles(clone, famStr));
        mobCompact(admin.getDescriptor(clone), familyDescriptor);
        assertEquals("Should have 3 hlinks + 1 MOB file per region due to clone + compact", 4 * numRegions, getNumberOfMobFiles(clone, famStr));
        RestartFramework.at("after_load_three_times_clone").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMobFileCompactionAfterSnapshotCloneAndFlush() throws InterruptedException, IOException {
        ...
        loadAndFlushThreeTimes(rows, table, famStr);
        RestartFramework.at("after_mob_compact_flush_test").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_snapshot_flush_test").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        LOG.debug("Taking snapshot and cloning table {}", table);
        admin.snapshot(TestMobUtils.getTableName(test), table);
        admin.cloneSnapshot(TestMobUtils.getTableName(test), clone);
        assertEquals("Should have 3 hlinks per region in MOB area from snapshot clone", 3 * numRegions, getNumberOfMobFiles(clone, famStr));
        loadAndFlushThreeTimes(rows, clone, famStr);
        RestartFramework.at("after_first_load_flush").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_load_flush").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        mobCompact(admin.getDescriptor(clone), familyDescriptor);
        assertEquals("Should have 7 MOB file per region due to clone + 3xflush + compact", 7 * numRegions, getNumberOfMobFiles(clone, famStr));
        RestartFramework.at("after_hlinks_check_flush").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_clone_snapshot_flush_test").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cleanup_flush").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mob_file_check_flush").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    protected void loadAndFlushThreeTimes(int rows, TableName table, String family) throws IOException {
        ...
        loadData(table, rows);
        loadData(table, rows);
        assertEquals("Should have 3 more mob files per region from flushing.", start + numRegions * 3, getNumberOfMobFiles(table, family));
    }
    ...
    protected void enableCompactions() throws IOException {
        final List<String> serverList = admin.getRegionServers().stream().map(sn -> sn.getServerName()).collect(Collectors.toList());
    ...
    protected void mobCompact(TableDescriptor tableDescriptor, ColumnFamilyDescriptor familyDescriptor) throws IOException, InterruptedException {
        ...
        RestartFramework.at("after_mob_compact_impl").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        waitUntilCompactionIsComplete(tableDescriptor.getTableName());
        RestartFramework.at("after_enable_compactions").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        disableCompactions();
    }
    ...
    protected void cleanupAndVerifyCounts(TableName table, String family, int rows) throws InterruptedException, IOException {
        ...
        assertEquals("After cleaning, we should have 1 MOB file per region based on size.", numRegions, getNumberOfMobFiles(table, family));
