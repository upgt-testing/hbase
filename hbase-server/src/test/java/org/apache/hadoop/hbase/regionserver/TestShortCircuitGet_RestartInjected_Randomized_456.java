...
public class TestShortCircuitGet_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestShortCircuitGet_RestartInjected.class);
    ...
    static final TableName tableName = TableName.valueOf("TestShortCircuitGet_RestartInjected");
    ...
    public void testScannerCloseWhenScanAndGetInCP() throws Exception {
        Table table = null;
        try {
            table = TEST_UTIL.createTable(tableName, new byte[][] { FAMILY }, 1, HConstants.DEFAULT_BLOCKSIZE, MyScanObserver.class.getName());
        ...
        try {
            ...
            RestartFramework.at("before_scan").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private static class MyRSRpcServices extends RSRpcServices {
        ...
        public ScanResponse scan(RpcController controller, ScanRequest request) throws ServiceException {
            try {
                ...
                if (region != null && TableName.isMetaTableName(region.getTableDescriptor().getTableName())) {
                    return super.scan(controller, request);
                }
                assertTrue(!RpcServer.getCurrentCall().isPresent());
                return super.scan(controller, request);
    ...
    public static class MyScanObserver implements RegionCoprocessor, RegionObserver {
        ...
        @Override
        public RegionScanner postScannerOpen(final ObserverContext<RegionCoprocessorEnvironment> observerContext, final Scan scan, final RegionScanner regionScanner) throws IOException {
            if (inCP) {
            ...
            Get get2 = new Get(r2);
            inCP = true;
            try (Connection connection = observerContext.getEnvironment().createConnection(observerContext.getEnvironment().getConfiguration())) {
                try {
