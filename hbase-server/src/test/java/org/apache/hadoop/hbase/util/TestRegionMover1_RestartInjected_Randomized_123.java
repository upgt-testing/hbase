...
public class TestRegionMover1_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionMover1_RestartInjected.class);

    @Rule
    public TestName name = new TestName();

    private static final Logger LOG = LoggerFactory.getLogger(TestRegionMover1_RestartInjected.class);
    ...
    public void testWithAck() throws Exception {
        ...
        try (RegionMover rm = rmBuilder.build()) {
            ...
            rm.unload();
            RestartFramework.at("after_unload").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(0, regionServer.getNumberOfOnlineRegions());
            LOG.info("Successfully Unloaded\nNow Loading");
            rm.load();
            RestartFramework.at("after_load").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testExclude() throws Exception {
        ...
        try (RegionMover rm = rmBuilder.build()) {
            rm.unload();
            RestartFramework.at("after_unload_with_exclude").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDesignatedFile() throws Exception {
        ...
        try (RegionMover rm = rmBuilder.build()) {
            ...
            rm.unload();
            RestartFramework.at("after_unload_with_designated").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(0, regionServer.getNumberOfOnlineRegions());
    ...
    public void testLoadMetaRegion() throws Exception {
        ...
        try (RegionMover rm = new RegionMoverBuilder(rsName, TEST_UTIL.getConfiguration()).ack(true).build()) {
            ...
            rm.unload();
            RestartFramework.at("after_meta_unload").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(0, rsWithMeta.getNumberOfOnlineRegions());
            LOG.info("Loading " + rsWithMeta.getServerName());
            rm.load();
            RestartFramework.at("after_meta_load").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testTargetServerDeadWhenLoading() throws Exception {
        ...
        try (RegionMover rm = new RegionMoverBuilder(rsName, conf).filename(filename).ack(true).build()) {
            LOG.info("Unloading " + rs.getServerName());
            rm.unload();
            assertEquals(0, rs.getNumberOfOnlineRegions());
        }
        String inexistRsName = "whatever:123";
        try (RegionMover rm = new RegionMoverBuilder(inexistRsName, conf).filename(filename).ack(true).build()) {
    ...
    public void testDecomServerExclusionWithAck() throws Exception {
        ...
        RestartFramework.at("after_server_decom").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        int regionsExcludeServer = excludeServer.getNumberOfOnlineRegions();
        ...
        try (RegionMover regionMover = rmBuilder.build()) {
            ...
            RestartFramework.at("after_unload_decom_excluded").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void waitForServerDecom(HRegionServer excludeServer) {
        TEST_UTIL.waitFor(3000, () -> {
            try {
                ...
                return decomServers.size() == 1 && decomServers.get(0).equals(excludeServer.getServerName());
            } catch (IOException e) {
