...
public class TestAssignmentManagerMetrics_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAssignmentManagerMetrics_RestartInjected.class);
    ...
    public void testRITAssignmentManagerMetrics() throws Exception {
        ...
        try (Table table = TEST_UTIL.createTable(TABLENAME, FAMILY)) {
            ...
            RestartFramework.at("after_first_metrics_check").on(CLUSTER).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_wait_rit_completion").on(CLUSTER).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            METRICS_HELPER.assertCounter(MetricsAssignmentManagerSource.ASSIGN_METRIC_PREFIX + "SubmittedCount", 3, amSource);
        }
    }
}
