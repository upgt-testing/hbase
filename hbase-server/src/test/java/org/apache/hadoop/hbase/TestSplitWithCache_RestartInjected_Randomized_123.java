...
public class TestSplitWithCache_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSplitWithCache_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSplitWithCache_RestartInjected.class);
    ...
    public void testEvictOnSplit() throws Exception {
        doTest("testEvictOnSplit", true, (f, m) -> Waiter.waitFor(UTIL.getConfiguration(), 1000, () -> m.get(f) != null), (f, m) -> Waiter.waitFor(UTIL.getConfiguration(), 1000, () -> m.get(f) == null));
    }

    @Test
    public void testDoesntEvictOnSplit() throws Exception {
        doTest("testDoesntEvictOnSplit", false, (f, m) -> Waiter.waitFor(UTIL.getConfiguration(), 1000, () -> m.get(f) != null), (f, m) -> Waiter.waitFor(UTIL.getConfiguration(), 1000, () -> m.get(f) != null));
    }
    ...
    private void doTest(String table, boolean evictOnSplit, BiConsumer<String, Map<String, Pair<String, Long>>> predicateBeforeSplit, BiConsumer<String, Map<String, Pair<String, Long>>> predicateAfterSplit) throws Exception {
        ...
        try {
            ...
            Collection<HStoreFile> files = UTIL.getMiniHBaseCluster().getRegions(tableName).get(0).getStores().get(0).getStorefiles();
            checkCacheForBlocks(tableName, files, predicateBeforeSplit);
            RestartFramework.at("before_region_split").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            UTIL.getAdmin().split(tableName, Bytes.toBytes("row-500"));
            Waiter.waitFor(UTIL.getConfiguration(), 30000, () -> UTIL.getMiniHBaseCluster().getRegions(tableName).size() == 2);
