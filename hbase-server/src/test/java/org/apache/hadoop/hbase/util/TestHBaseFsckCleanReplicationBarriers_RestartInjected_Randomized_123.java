...
public class TestHBaseFsckCleanReplicationBarriers_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestHBaseFsckCleanReplicationBarriers_RestartInjected.class);
    ...
    public void testCleanReplicationBarrierWithDeletedTable() throws Exception {
        ...
        RestartFramework.at("after_barriers_written").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 110; i++) {
        ...
        RestartFramework.at("after_clean_barriers").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Result result;
        ...
        boolean cleaned = HbckTestingUtil.cleanReplicationBarrier(UTIL.getConfiguration(), tableName);
        assertTrue(cleaned);
        RestartFramework.at("after_barrier_scan").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCleanReplicationBarrierWithExistTable() throws Exception {
        RestartFramework.at("after_barriers_added").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static void createPeer() throws IOException {
        ReplicationPeerConfig rpc = ReplicationPeerConfig.newBuilder().setClusterKey(UTIL.getClusterKey() + "-test").setSerial(true).build();
