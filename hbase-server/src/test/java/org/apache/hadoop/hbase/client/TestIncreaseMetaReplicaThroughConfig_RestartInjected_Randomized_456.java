...
public class TestIncreaseMetaReplicaThroughConfig_RestartInjected_Randomized_456 extends MetaWithReplicasTestBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestIncreaseMetaReplicaThroughConfig_RestartInjected.class);
    ...
    public void testUpgradeAndIncreaseReplicaCount() throws Exception {
        ...
        RestartFramework.at("after_new_master_start").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_replica_count_update").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_master_stop").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        oldMaster = TEST_UTIL.getMiniHBaseCluster().getMaster();
