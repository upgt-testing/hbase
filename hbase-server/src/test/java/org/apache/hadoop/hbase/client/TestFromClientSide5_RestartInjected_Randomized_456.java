...
public class TestFromClientSide5_RestartInjected_Randomized_456 extends FromClientSideBase {

    private static final Logger LOG = LoggerFactory.getLogger(TestFromClientSide5_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFromClientSide5_RestartInjected.class);

    @Rule
    public TableNameTestRule name = new TableNameTestRule();

    // To keep the child classes happy.
    TestFromClientSide5_RestartInjected_Randomized_456() {
    }

    public TestFromClientSide5_RestartInjected_Randomized_456(Class<? extends ConnectionRegistry> registry, int numHedgedReqs) throws Exception {
    ...
    public void testGetClosestRowBefore() throws IOException, InterruptedException {
        ...
        try (Table table = TEST_UTIL.createTable(tableName, new byte[][] { HConstants.CATALOG_FAMILY, Bytes.toBytes("info2") }, 1, 1024);
            ...
            table.put(put3);
            table.put(put4);
            RestartFramework.at("after_put").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            region.flush(true);
    ...
    public void testMultiRowMutation() throws Exception {
        ...
        try (Table t = TEST_UTIL.createTable(tableName, FAMILY)) {
            ...
            RestartFramework.at("after_batch_put").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRowMutations() throws Exception {
        ...
        try (Table t = TEST_UTIL.createTable(tableName, FAMILY)) {
            ...
            RestartFramework.at("after_row_mutations").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBatchAppendWithReturnResultFalse() throws Exception {
        ...
        try (Table table = TEST_UTIL.createTable(tableName, FAMILY)) {
            ...
            RestartFramework.at("after_batch_append").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAppend() throws Exception {
        ...
        try (Table t = TEST_UTIL.createTable(tableName, FAMILY)) {
            ...
            RestartFramework.at("after_append").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            a = new Append(ROW);
    ...
    private List<Result> doAppend(final boolean walUsed) throws IOException {
        ...
        try (Table t = TEST_UTIL.createTable(TABLENAME, FAMILY)) {
            ...
            t.put(put_0);
            t.put(put_1);
            RestartFramework.at("after_append_and_puts").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAppendWithoutWAL() throws Exception {
        ...
        for (int i = 0; i < resultsWithWal.size(); ++i) {
            ...
            for (int j = 0; j < resultWithWal.rawCells().length; ++j) {
                ...
                assertArrayEquals(CellUtil.cloneQualifier(cellWithWal), CellUtil.cloneQualifier(cellWithoutWal));
    ...
    public void testClientPoolRoundRobin() throws IOException {
        ...
        try (Table table = TEST_UTIL.createTable(tableName, new byte[][] { FAMILY }, Integer.MAX_VALUE)) {
            ...
            for (int versions = 1; versions <= numVersions; versions++) {
                ...
                assertEquals("The number of versions of '" + Bytes.toString(FAMILY) + ":" + Bytes.toString(QUALIFIER) + " did not match", versions, navigableMap.size());
    ...
    public void testCheckAndPut() throws IOException {
        ...
        try (Table table = TEST_UTIL.createTable(name.getTableName(), FAMILY)) {
            ...
            assertTrue(ok);
            RestartFramework.at("after_check_and_put").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckAndMutateWithTimeRange() throws IOException {
        try (Table table = TEST_UTIL.createTable(name.getTableName(), FAMILY)) {
            ...
            assertFalse(ok);
            ok = table.checkAndMutate(ROW, FAMILY).qualifier(QUALIFIER).timeRange(TimeRange.from(ts + 10000)).ifEquals(VALUE).thenPut(put);
            assertFalse(ok);
            ok = table.checkAndMutate(ROW, FAMILY).qualifier(QUALIFIER).timeRange(TimeRange.between(ts + 10000, ts + 20000)).ifEquals(VALUE).thenPut(put);
            assertFalse(ok);
            ok = table.checkAndMutate(ROW, FAMILY).qualifier(QUALIFIER).timeRange(TimeRange.until(ts)).ifEquals(VALUE).thenPut(put);
            assertFalse(ok);
            ok = table.checkAndMutate(ROW, FAMILY).qualifier(QUALIFIER).timeRange(TimeRange.at(ts)).ifEquals(VALUE).thenPut(put);
            assertTrue(ok);
            ok = table.checkAndMutate(ROW, FAMILY).qualifier(QUALIFIER).timeRange(TimeRange.from(ts)).ifEquals(VALUE).thenPut(put);
            assertTrue(ok);
            ok = table.checkAndMutate(ROW, FAMILY).qualifier(QUALIFIER).timeRange(TimeRange.between(ts, ts + 20000)).ifEquals(VALUE).thenPut(put);
            assertTrue(ok);
            ok = table.checkAndMutate(ROW, FAMILY).qualifier(QUALIFIER).timeRange(TimeRange.until(ts + 10000)).ifEquals(VALUE).thenPut(put);
            assertTrue(ok);
    ...
    private void waitForStoreFileCount(HStore store, int count, int timeout) throws InterruptedException {
        ...
        while (start + timeout > EnvironmentEdgeManager.currentTime() && store.getStorefilesCount() != count) {
            Thread.sleep(100);
        }
        System.out.println("start=" + start + ", now=" + EnvironmentEdgeManager.currentTime() + ", cur=" + store.getStorefilesCount());
        ...
        try (Table ignored = TEST_UTIL.createTable(tableName, new byte[][] { family1, family2 }, 10);
            ...
            for (int i = 0; i < TEST_UTIL.getMiniHBaseCluster().getLiveRegionServerThreads().size(); i++) {
                ...
                if (addr.getPort() != addrBefore.getPort()) {
                    ...
                    Thread.sleep(5000);
                    RestartFramework.at("after_region_move").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    RestartFramework.at("after_region_move").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    addrAfter = addr;
                    break;
                }
            }
    ...
    public void testNullWithReverseScan() throws Exception {
        ...
        // Use a new table
        try (Table ht = TEST_UTIL.createTable(TableName.valueOf(name.getTableName().toString() + "2"), FAMILY)) {
    ...
    public void testCellSizeLimit() throws IOException {
        final TableName tableName = name.getTableName();
        TableDescriptorBuilder.ModifyableTableDescriptor tableDescriptor = new TableDescriptorBuilder.ModifyableTableDescriptor(tableName).setValue(HRegion.HBASE_MAX_CELL_SIZE_KEY, Integer.toString(10 * 1024));
        ColumnFamilyDescriptor familyDescriptor = new ColumnFamilyDescriptorBuilder.ModifyableColumnFamilyDescriptor(FAMILY);
    ...
    public void testCellSizeNoLimit() throws IOException {
        final TableName tableName = name.getTableName();
        ColumnFamilyDescriptor familyDescriptor = new ColumnFamilyDescriptorBuilder.ModifyableColumnFamilyDescriptor(FAMILY);
        TableDescriptorBuilder.ModifyableTableDescriptor tableDescriptor = new TableDescriptorBuilder.ModifyableTableDescriptor(tableName).setValue(HRegion.HBASE_MAX_CELL_SIZE_KEY, Integer.toString(0));
        tableDescriptor.setColumnFamily(familyDescriptor);
        try (Admin admin = TEST_UTIL.getAdmin()) {
            admin.createTable(tableDescriptor);
        }
        ...
        try (Table ht = TEST_UTIL.getConnection().getTable(tableName)) {
            ht.put(new Put(ROW).addColumn(FAMILY, QUALIFIER, new byte[HRegion.DEFAULT_MAX_CELL_SIZE - 1024]));
    ...
    public void testDeleteLatestVersionOfSpecifiedColumn() throws Exception {
        ...
        try (Table ht = TEST_UTIL.createTable(tableName, FAMILY, 5)) {
            ...
            assertNResult(result, ROW, FAMILY, QUALIFIER, new long[] { ts[0], ts[1], ts[2] }, new byte[][] { VALUES[0], VALUES[1], VALUES[2] }, 0, 2);
            delete = new Delete(ROW);
            ...
            assertNResult(result, ROW, FAMILY, QUALIFIER, new long[] { ts[0] }, new byte[][] { VALUES[0] }, 0, 0);
            put = new Put(ROW);
            ...
            assertNResult(result, ROW, FAMILY, QUALIFIER, new long[] { ts[0], ts[4] }, new byte[][] { VALUES[0], VALUES[4] }, 0, 1);
        }
    }
    ...
    private void createTableAndValidateTableSchemaModification(TableName tableName, boolean modifyTable) throws Exception {
        ...
        TableDescriptor tableDesc = TableDescriptorBuilder.newBuilder(tableName).setColumnFamily(ColumnFamilyDescriptorBuilder.of(cf1)).setColumnFamily(ColumnFamilyDescriptorBuilder.of(cf2)).build();
