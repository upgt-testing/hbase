...
public class TestAssignRegionToUninitializedRegionServer_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAssignRegionToUninitializedRegionServer_RestartInjected.class);
    ...
    public void testMove() throws Exception {
        RestartFramework.at("after_new_rs_online").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.getMiniHBaseCluster().getConfiguration().setClass(HConstants.REGION_SERVER_IMPL, RegionServerForTest.class, HRegionServer.class);
        CountDownLatch arrive = new CountDownLatch(1);
        ARRIVE = arrive;
        RestartFramework.at("before_start_new_rs").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        Future<HRegionServer> regionServerFuture = ForkJoinPool.commonPool().submit(() -> UTIL.getMiniHBaseCluster().startRegionServer().getRegionServer());
        ARRIVE.await();
        RestartFramework.at("after_resume_rs").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_servers").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HRegionServer dst = UTIL.getOtherRegionServer(src);
        src = UTIL.getRSForFirstRegionInTable(NAME);
        try {
            UTIL.getAdmin().move(UTIL.getAdmin().getRegions(NAME).get(0).getEncodedNameAsBytes(), dst.getServerName());
            ...
            RestartFramework.at("after_first_move_attempt").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
        ...
        UTIL.getAdmin().move(UTIL.getAdmin().getRegions(NAME).get(0).getEncodedNameAsBytes(), dst.getServerName());
        src = UTIL.getRSForFirstRegionInTable(NAME);
        RestartFramework.at("after_second_move").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // this time the region should be on the new region server
        assertSame(dst, UTIL.getRSForFirstRegionInTable(NAME));
        RestartFramework.at("after_rs_arrive_before_report").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
