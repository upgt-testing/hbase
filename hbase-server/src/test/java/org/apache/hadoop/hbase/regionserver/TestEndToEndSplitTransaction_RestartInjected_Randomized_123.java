...
public class TestEndToEndSplitTransaction_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestEndToEndSplitTransaction_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestEndToEndSplitTransaction_RestartInjected.class);
    ...
    public void testCanSplitJustAfterASplit() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_get_references").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            compactSplit.setCompactionsEnabled(true);
            for (HRegion region : regions) {
                region.compact(true);
            }
            RestartFramework.at("after_compaction").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFromClientSideWhileSplitting() throws Throwable {
        ...
        RestartFramework.at("after_split_complete").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        regionSplitter.join();
        RestartFramework.at("after_chore_schedule").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        stopper.stop(null);
        ...
        try {
            TEST_UTIL.waitFor(10000, new Waiter.Predicate<Exception>() {
                ...
                public boolean evaluate() throws Exception {
                    return rs.getServerName().equals(MetaTableAccessor.getRegionLocation(admin.getConnection(), regionName).getServerName());
                }
            });
        } catch (Exception e) {
