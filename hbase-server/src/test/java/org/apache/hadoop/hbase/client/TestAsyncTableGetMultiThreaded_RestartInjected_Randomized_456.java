...
public class TestAsyncTableGetMultiThreaded_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAsyncTableGetMultiThreaded_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestAsyncTableGetMultiThreaded_RestartInjected.class);
    ...
    protected static void setUp(MemoryCompactionPolicy memoryCompaction) throws Exception {
        ...
        RestartFramework.at("after_table_create_and_data_load").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void test() throws Exception {
        ...
        for (byte[] splitPoint : SPLIT_KEYS) {
            ...
            RestartFramework.at("after_balance").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
