...
public class TestRaceWhenCreatingReplicationSource_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRaceWhenCreatingReplicationSource_RestartInjected.class);
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        RestartFramework.at("after_add_replication_peer").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRace() throws Exception {
        ...
        RestartFramework.at("after_table_creation").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.waitTableAvailable(TABLE_NAME);
        RestartFramework.at("after_put_operation").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_uuid_change").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_replication_source_initialized").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_validation").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        UTIL.waitFor(30000, new ExplainingPredicate<Exception>() {
            ...
            @Override
            public boolean evaluate() throws Exception {
                try (WALStreamReader reader = WALFactory.createStreamReader(FS, LOG_PATH, UTIL.getConfiguration())) {
