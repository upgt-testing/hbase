...
public class TestMergeTableRegionsProcedure_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestMergeTableRegionsProcedure_RestartInjected.class);
  ...
  public void testMergeTwoRegions() throws Exception {
    ...
    RestartFramework.at("after_create_table_merge_two")
    ...
    RestartFramework.at("after_test_merge_two")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  private void testMerge(TableName tableName, int mergeCount) throws IOException {
    ...
    RestartFramework.at("after_load_rows")
        .on(UTIL.getMiniHBaseCluster())
    ...
    RestartFramework.at("after_verify_region_count")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_enable_table")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_trigger_catalog_janitor")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_wait_cleanup")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMergeTenRegions() throws Exception {
    ...
    RestartFramework.at("after_test_merge_ten")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMergeRegionsConcurrently() throws Exception {
    ...
    // collect AM metrics before test
    collectAssignmentManagerMetrics();
    RestartFramework.at("after_collect_metrics_concurrent")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_submit_concurrent_procs")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_verify_concurrent_merge")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testRecoveryAndDoubleExecution() throws Exception {
    ...
    RestartFramework.at("after_setup_recovery_test")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    long procId = procExec.submitProcedure(
      new MergeTableRegionsProcedure(procExec.getEnvironment(), regionsToMerge, true));
    RestartFramework.at("after_submit_recovery_proc")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_recovery_test")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_verify_recovery")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testRollbackAndDoubleExecution() throws Exception {
    ...
    ProcedureTestingUtility.waitNoProcedureRunning(procExec);
    ProcedureTestingUtility.setKillAndToggleBeforeStoreUpdate(procExec, true);
    RestartFramework.at("after_setup_rollback_test")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    long procId = procExec.submitProcedure(
      new MergeTableRegionsProcedure(procExec.getEnvironment(), regionsToMerge, true));
    RestartFramework.at("after_submit_rollback_proc")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_rollback_test")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_wait_regions_assigned")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_verify_rollback")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMergeWithoutPONR() throws Exception {
    ...
    ProcedureTestingUtility.waitNoProcedureRunning(procExec);
    ProcedureTestingUtility.setKillAndToggleBeforeStoreUpdate(procExec, true);
    RestartFramework.at("after_setup_without_ponr_test")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    long procId = procExec.submitProcedure(
      new MergeTableRegionsProcedure(procExec.getEnvironment(), regionsToMerge, true));
    RestartFramework.at("after_submit_without_ponr_proc")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_recovery_without_ponr")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    ProcedureTestingUtility.waitProcedure(procExec, procId);
    RestartFramework.at("after_wait_without_ponr_proc")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMergingRegionWhileTakingSnapshot() throws Exception {
    ...
    UTIL.getHBaseCluster().getMaster().getSnapshotManager().registerSnapshotProcedure(snapshotProto,
      snapshotProcId);
    RestartFramework.at("after_submit_snapshot_proc")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    long mergeProcId = procExec.submitProcedure(
      new MergeTableRegionsProcedure(procExec.getEnvironment(), regionsToMerge, true));
    RestartFramework.at("after_submit_merge_with_snapshot")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_wait_merge_and_snapshot")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    assertEquals(initialRegionCount, UTIL.getAdmin().getRegions(tableName).size());
    RestartFramework.at("after_verify_snapshot_conflict")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMergeDetectsModifyTableProcedure() throws Exception {
    ...
    RestartFramework.at("after_submit_merge_with_modify")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_wait_modify_proc")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
