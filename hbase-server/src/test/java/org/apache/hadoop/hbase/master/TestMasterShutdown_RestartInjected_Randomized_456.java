...
public class TestMasterShutdown_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestMasterShutdown_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterShutdown_RestartInjected.class);
    ...
    public void testMasterShutdown() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_wait_master_threads").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // find the active master
            final HMaster active = cluster.getMaster();
            assertNotNull(active);
            RestartFramework.at("after_find_active_master").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_verify_backup_masters").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMasterShutdownBeforeStartingAnyRegionServer() throws Exception {
        ...
        try {
            htu = new HBaseTestingUtility(createMasterShutdownBeforeStartingAnyRegionServerConfiguration());
            // configure a cluster with
            final StartMiniClusterOption options = StartMiniClusterOption.builder().numDataNodes(1).numMasters(1).numRegionServers(0).masterClass(HMaster.class).rsClass(MiniHBaseCluster.MiniHBaseClusterRegionServer.class).createRootDir(true).build();
            ...
            RestartFramework.at("after_create_local_cluster").on(hbaseCluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final MasterThread masterThread = hbaseCluster.getMasters().get(0);
            masterThread.start();
            RestartFramework.at("after_start_master_thread").on(hbaseCluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertNotEquals("Timeout waiting for server manager to become available.", -1, Waiter.waitFor(htu.getConfiguration(), timeout, () -> masterThread.getMaster().getServerManager() != null));
            RestartFramework.at("after_server_manager_available").on(hbaseCluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
