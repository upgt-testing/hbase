...
public class TestRegionObserverInterface_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionObserverInterface_RestartInjected.class);
    ...
    public static void setupBeforeClass() throws Exception {
        ...
        conf.setStrings(CoprocessorHost.REGION_COPROCESSOR_CONF_KEY, SimpleRegionObserver.class.getName());
    ...
    public void testRegionObserver() throws IOException {
        ...
        try {
            ...
            get.addColumn(C, C);
            table.get(get);
            RestartFramework.at("after_get").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            table.delete(delete);
            RestartFramework.at("after_delete").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRowMutation() throws IOException {
        ...
        try {
            ...
            arm.add(delete);
            table.mutateRow(arm);
            RestartFramework.at("after_row_mutations").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testIncrementHook() throws IOException {
        ...
        try {
            ...
            table.increment(inc);
            RestartFramework.at("after_increment").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckAndPutHooks() throws IOException {
        ...
        try (Table table = util.createTable(tableName, new byte[][] { A, B, C })) {
            ...
            p.addColumn(A, A, A);
            verifyMethodResult(SimpleRegionObserver.class, new String[] { "getPreCheckAndPut", "getPreCheckAndPutAfterRowLock", "getPostCheckAndPut", "getPreCheckAndPutWithFilter", "getPreCheckAndPutWithFilterAfterRowLock", "getPostCheckAndPutWithFilter", "getPreCheckAndMutate", "getPreCheckAndMutateAfterRowLock", "getPostCheckAndMutate" }, tableName, new Integer[] { 0, 0, 0, 0, 0, 0, 0, 0, 0 });
            table.checkAndMutate(Bytes.toBytes(0), A).qualifier(A).ifEquals(A).thenPut(p);
            RestartFramework.at("after_check_and_put").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verifyMethodResult(SimpleRegionObserver.class, new String[] { "getPreCheckAndPut", "getPreCheckAndPutAfterRowLock", "getPostCheckAndPut", "getPreCheckAndPutWithFilter", "getPreCheckAndPutWithFilterAfterRowLock", "getPostCheckAndPutWithFilter", "getPreCheckAndMutate", "getPreCheckAndMutateAfterRowLock", "getPostCheckAndMutate" }, tableName, new Integer[] { 1, 1, 1, 0, 0, 0, 1, 1, 1 });
            table.checkAndMutate(Bytes.toBytes(0), new SingleColumnValueFilter(A, A, CompareOperator.EQUAL, A)).thenPut(p);
    ...
    public void testCheckAndDeleteHooks() throws IOException {
        ...
        try {
            ...
            table.checkAndMutate(Bytes.toBytes(0), new SingleColumnValueFilter(A, A, CompareOperator.EQUAL, A)).thenDelete(d);
    ...
    public void testCheckAndIncrementHooks() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_check_and_increment").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckAndRowMutationsHooks() throws Exception {
        ...
        try {
            ...
            table.put(p);
            verifyMethodResult(SimpleRegionObserver.class, new String[] { "getPreCheckAndMutate", "getPreCheckAndMutateAfterRowLock", "getPostCheckAndMutate" }, tableName, new Integer[] { 0, 0, 0 });
            table.checkAndMutate(CheckAndMutate.newBuilder(row).ifEquals(A, A, A).build(new RowMutations(row).add((Mutation) new Put(row).addColumn(B, B, B)).add((Mutation) new Delete(row))));
            verifyMethodResult(SimpleRegionObserver.class, new String[] { "getPreCheckAndMutate", "getPreCheckAndMutateAfterRowLock", "getPostCheckAndMutate" }, tableName, new Integer[] { 1, 1, 1 });
            Object[] result = new Object[2];
            table.batch(Arrays.asList(p, CheckAndMutate.newBuilder(row).ifEquals(A, A, A).build(new RowMutations(row).add((Mutation) new Put(row).addColumn(B, B, B)).add((Mutation) new Delete(row)))), result);
            verifyMethodResult(SimpleRegionObserver.class, new String[] { "getPreCheckAndMutate", "getPreCheckAndMutateAfterRowLock", "getPostCheckAndMutate" }, tableName, new Integer[] { 2, 2, 2 });
        } finally {
            util.deleteTable(tableName);
            table.close();
    ...
    @Test
    public // HBase-3758
    void testHBase3758() throws IOException {
        final TableName tableName = TableName.valueOf(name.getMethodName());
        util.createTable(tableName, new byte[][] { A, B, C });
    ...
    private static class PreWALAppendWALActionsListener implements WALActionsListener {
        ...
        public void postAppend(long entryLen, long elapsedTimeMillis, WALKey logKey, WALEdit logEdit) throws IOException {
            for (int k = 0; k < 4; k++) {
                if (!walKeysCorrect[k]) {
                    walKeysCorrect[k] = Arrays.equals(SimpleRegionObserver.WAL_EXTENDED_ATTRIBUTE_BYTES, logKey.getExtendedAttribute(Integer.toString(k + 1)));
                }
            }
        }
