...
public class TestFSErrorsExposed_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFSErrorsExposed_RestartInjected.class);
    ...
    public void testFullSystemBubblesFSErrors() throws Exception {
        ...
        try {
            ...
            try (Table table = util.getConnection().getTable(tableName)) {
                ...
                util.flush();
                RestartFramework.at("after_load_and_flush").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                util.countRows(table);
                RestartFramework.at("after_count_rows").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
