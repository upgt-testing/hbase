...
public class TestZooKeeper_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestZooKeeper_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestZooKeeper_RestartInjected.class);
    ...
    public void testRegionAssignmentAfterMasterRecoveryDueToZKExpiry() throws Exception {
        ...
        try (Admin admin = TEST_UTIL.getAdmin()) {
            ...
            // Master doesn't recover any more
            TEST_UTIL.waitFor(5000, () -> m.isStopped());
            RestartFramework.at("after_master_abort").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
