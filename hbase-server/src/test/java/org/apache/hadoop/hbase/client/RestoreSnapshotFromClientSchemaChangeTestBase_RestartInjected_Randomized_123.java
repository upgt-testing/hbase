...
public class RestoreSnapshotFromClientSchemaChangeTestBase_RestartInjected_Randomized_123 extends RestoreSnapshotFromClientTestBase {
    ...
    public void testRestoreSchemaChange() throws Exception {
        ...
        RestartFramework.at("after_second_enable").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_enable").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_restore").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.enableTable(tableName);
        ...
        admin.enableTable(tableName);
        htd = admin.getDescriptor(tableName);
        RestartFramework.at("after_first_restore").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(2, htd.getColumnFamilyCount());
        assertEquals(2, table.getDescriptor().getColumnFamilyCount());
        assertEquals(500, countRows(table, TEST_FAMILY2));
        assertEquals(snapshot2Rows, countRows(table));
