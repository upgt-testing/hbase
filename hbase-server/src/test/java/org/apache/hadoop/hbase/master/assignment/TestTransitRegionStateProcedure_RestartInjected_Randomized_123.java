...
public class TestTransitRegionStateProcedure_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestTransitRegionStateProcedure_RestartInjected.class);
    ...
    private void resetProcExecutorTestingKillFlag() {
        ProcedureExecutor<MasterProcedureEnv> procExec = UTIL.getHBaseCluster().getMaster().getMasterProcedureExecutor();
    ...
    public void testRecoveryAndDoubleExecutionMove() throws Exception {
        ...
        RestartFramework.at("after_create_move_procedure").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        env = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor().getEnvironment();
        HRegion region = UTIL.getMiniHBaseCluster().getRegions(tableName).get(0);
        RestartFramework.at("after_recovery_and_double_execution").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_get_env").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_region_and_seqnum").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRecoveryAndDoubleExecutionReopen() throws Exception {
        ...
        rs = UTIL.getRSForFirstRegionInTable(tableName);
        RestartFramework.at("after_recovery_and_double_execution").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_update_request_counts").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRecoveryAndDoubleExecutionUnassignAndAssign() throws Exception {
        ...
        RestartFramework.at("after_create_unassign_procedure").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_get_master_and_env").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        ...
        RestartFramework.at("after_verify_region_closed").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_unassign_execution").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        testRecoveryAndDoubleExcution(unassign);
        // Refresh master reference
        master = UTIL.getMiniHBaseCluster().getMaster();
        AssignmentManager am = master.getAssignmentManager();
        ...
        RestartFramework.at("after_assign_execution").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HRegion region2 = UTIL.getMiniHBaseCluster().getRegions(tableName).get(0);
        long openSeqNum2 = region2.getOpenSeqNum();
        // confirm that the region is successfully opened
        assertTrue(openSeqNum2 > openSeqNum);
