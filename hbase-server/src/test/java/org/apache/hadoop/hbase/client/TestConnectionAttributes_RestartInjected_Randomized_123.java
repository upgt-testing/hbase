...
public class TestConnectionAttributes_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestConnectionAttributes_RestartInjected.class);
    ...
    public static void setUp() throws Exception {
        ...
        Table table = TEST_UTIL.createTable(TABLE_NAME, new byte[][] { FAMILY }, 1, HConstants.DEFAULT_BLOCKSIZE, TestConnectionAttributes_RestartInjected.AttributesCoprocessor.class.getName());
    ...
    public void testConnectionHeaderOverwrittenAttributesRemain() throws IOException {
        ...
        try (Connection conn = ConnectionFactory.createConnection(conf, null, AuthUtil.loginClient(conf), CONNECTION_ATTRIBUTES);
            ...
            RestartFramework.at("after_connection_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class AttributesCoprocessor implements RegionObserver, RegionCoprocessor {
        ...
        public void preGetOp(ObserverContext<RegionCoprocessorEnvironment> c, Get get, List<Cell> result) throws IOException {
            ...
            for (Map.Entry<String, byte[]> attr : rpcCall.getConnectionAttributes().entrySet()) {
                result.add(c.getEnvironment().getCellBuilder().clear().setRow(get.getRow()).setFamily(FAMILY).setQualifier(Bytes.toBytes(attr.getKey())).setValue(attr.getValue()).setType(Cell.Type.Put).setTimestamp(1).build());
            }
