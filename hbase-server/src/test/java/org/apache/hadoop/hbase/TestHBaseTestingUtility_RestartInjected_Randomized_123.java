...
public class TestHBaseTestingUtility_RestartInjected_Randomized_123 {
    ...
    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestHBaseTestingUtility_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestHBaseTestingUtility_RestartInjected.class);
     ...
     * ensemble. We then create the same table in both and make sure that what we insert in one place
    ...
    public void testMultiClusters() throws Exception {
        ...
        try {
            MiniHBaseCluster cluster1 = htu1.startMiniCluster();
            MiniHBaseCluster cluster2 = htu2.startMiniCluster();
            ...
            RestartFramework.at("after_table_create").on(cluster1).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Put put = new Put(ROW);
            put.addColumn(FAM_NAME, QUAL_NAME, VALUE);
            table1.put(put);
            RestartFramework.at("after_put").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Get get = new Get(ROW);
    ...
    public void testMiniClusterWithSSLOn() throws Exception {
        final String BASEDIR = System.getProperty("test.build.dir", "target/test-dir") + "/" + TestHBaseTestingUtility_RestartInjected.class.getSimpleName();
        String sslConfDir = KeyStoreTestUtil.getClasspathDir(TestHBaseTestingUtility_RestartInjected.class);
    ...
    public void testMultipleStartStop() throws Exception {
        ...
        MiniHBaseCluster cluster1 = htu1.startMiniCluster();
        RestartFramework.at("after_file_create").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        htu1.getDFSCluster().getFileSystem().create(foo);
        assertTrue(htu1.getDFSCluster().getFileSystem().exists(foo));
        htu1.shutdownMiniCluster();
        MiniHBaseCluster cluster2 = htu1.startMiniCluster();
        assertFalse(htu1.getDFSCluster().getFileSystem().exists(foo));
        htu1.getDFSCluster().getFileSystem().create(foo);
        RestartFramework.at("after_second_file_create").on(cluster2).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(htu1.getDFSCluster().getFileSystem().exists(foo));
        htu1.shutdownMiniCluster();
    }
    ...
    public void testOverridingOfDefaultPorts() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("at_50_percent").on(defaultCluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final String masterHostPort = defaultCluster.getMaster().getServerName().getAddress().toString();
            ...
            assertEquals(masterHostPort, defaultCluster.getConfiguration().get(HConstants.MASTER_ADDRS_KEY));
        } finally {
        ...
        try {
            ...
            RestartFramework.at("at_50_percent_custom").on(customCluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final String masterHostPort = customCluster.getMaster().getServerName().getAddress().toString();
            ...
            assertEquals(masterHostPort, customCluster.getConfiguration().get(HConstants.MASTER_ADDRS_KEY));
        } finally {
            htu.shutdownMiniCluster();
        }
    }
    ...
    public void testKillMiniHBaseCluster() throws Exception {
        ...
        try {
            MiniHBaseCluster cluster = htu.startMiniHBaseCluster();
            TableName tableName;
            byte[] FAM_NAME;
            for (int i = 0; i < NUMTABLES; i++) {
                ...
                try (Table table = htu.createMultiRegionTable(tableName, FAM_NAME, NUMREGIONS)) {
                    ...
                    RestartFramework.at("after_load_data").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
            }
        } finally {
