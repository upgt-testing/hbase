...
public class TestRegionMover2_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionMover2_RestartInjected.class);
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TestRegionMover2_RestartInjected.class);
    ...
    private TableName createTable(String name) throws IOException {
        ...
        TableDescriptor tableDesc = TableDescriptorBuilder.newBuilder(tableName).setColumnFamily(ColumnFamilyDescriptorBuilder.of(CF)).build();
        int startKey = 0;
        int endKey = 80000;
    ...
    public void testWithMergedRegions() throws Exception {
        ...
        try (RegionMover rm = rmBuilder.build()) {
            ...
            rm.unload();
            RestartFramework.at("after_unload").on(cluster).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWithSplitRegions() throws Exception {
        ...
        table.put(puts);
        RestartFramework.at("after_compact").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.flush(tableName);
    ...
    public void testFailedRegionMove() throws Exception {
        ...
        try (RegionMover rm = rmBuilder.build()) {
            ...
            RestartFramework.at("after_offline_region").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testIsolateSingleRegionOnTheSameServer() throws Exception {
        final TableName tableName = TableName.valueOf(name.getMethodName());
        MiniHBaseCluster cluster = TEST_UTIL.getHBaseCluster();
        RestartFramework.at("after_data_load").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testIsolateMetaOnTheSameSever() throws Exception {
        MiniHBaseCluster cluster = TEST_UTIL.getHBaseCluster();
        RestartFramework.at("before_meta_isolate").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private List<Put> createPuts(int count) {
        ...
        for (int i = 0; i < count; i++) {
            puts.add(new Put(Bytes.toBytes("rowkey_" + i)).addColumn(Bytes.toBytes(CF), Bytes.toBytes("q1"), Bytes.toBytes("val_" + i)));
        }
        return puts;
    }
    ...
    public ServerName findSourceServerName(TableName tableName) throws Exception {
        ...
        if (sourceServer == null) {
            throw new Exception("This shouldn't happen, No RS found with more than 2 regions of table : " + tableName);
        }
        return sourceServer;
    }
    ...
    public void regionIsolationOperation(ServerName sourceServerName, ServerName destinationServerName, int numRegionsToIsolate, boolean isolateMetaAlso) throws Exception {
        ...
        List<HRegion> hRegions = sourceRS.getRegions().stream().filter(hRegion -> hRegion.getRegionInfo().getTable().equals(tableName)).collect(Collectors.toList());
        ...
        try (RegionMover rm = rmBuilder.build()) {
            LOG.debug("Unloading {} except regions: {}", destinationRS.getServerName(), listOfRegionIDsToIsolate);
            rm.isolateRegions();
            RestartFramework.at("after_region_isolate").on(cluster).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            for (int i = 0; i < numRegionsToIsolate; i++) {
                Assert.assertTrue(listOfRegionIDsToIsolate.contains(onlineRegions.get(i).getRegionInfo().getEncodedName()));
            }
            LOG.debug("Successfully Isolated {} regions: {} on {}", listOfRegionIDsToIsolate.size(), listOfRegionIDsToIsolate, destinationRS.getServerName());
        } finally {
