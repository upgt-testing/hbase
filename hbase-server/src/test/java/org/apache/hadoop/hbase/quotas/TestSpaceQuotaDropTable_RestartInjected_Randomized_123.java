...
public class TestSpaceQuotaDropTable_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSpaceQuotaDropTable_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSpaceQuotaDropTable_RestartInjected.class);
    ...
    public void testSetQuotaAndThenDropTableWithRegionReport() throws Exception {
        ...
        RestartFramework.at("after_table_drop").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        Waiter.waitFor(TEST_UTIL.getConfiguration(), 30 * 1000, new Waiter.Predicate<Exception>() {
            ...
            public boolean evaluate() throws Exception {
                ...
                List<RegionInfo> tableRegions = MetaTableAccessor.getTableRegions(TEST_UTIL.getConnection(), tn);
        ...
        RestartFramework.at("after_region_report").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void setQuotaAndThenDropTable(SpaceViolationPolicy policy) throws Exception {
        ...
        RestartFramework.at("after_quota_violation").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_table_recreate").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
