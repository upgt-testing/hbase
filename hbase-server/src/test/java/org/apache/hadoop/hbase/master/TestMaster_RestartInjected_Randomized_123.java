...
public class TestMaster_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMaster_RestartInjected.class);

    private static final HBaseTestingUtility TEST_UTIL = new HBaseTestingUtility();

    private static final Logger LOG = LoggerFactory.getLogger(TestMaster_RestartInjected.class);
    ...
    public void testMasterOpsWhileSplitting() throws Exception {
        ...
        RestartFramework.at("after_split_trigger").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_split").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMoveThrowsPleaseHoldException() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_get_regions").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // fake it, set back later
    ...
    public void testBlockingHbkc1WithLockFile() throws IOException {
        ...
        RestartFramework.at("after_second_master_start").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_lock_file_check").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Start a new one.
        TEST_UTIL.getMiniHBaseCluster().startMaster();
        TEST_UTIL.waitFor(30000, () -> TEST_UTIL.getMiniHBaseCluster().getMaster() != null && TEST_UTIL.getMiniHBaseCluster().getMaster().isInitialized());
