...
public class TestClientScannerTimeouts_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestClientScannerTimeouts_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestClientScannerTimeouts_RestartInjected.class);
    ...
    public void setUp() throws Exception {
        ...
        conf.setBoolean(HBASE_CLIENT_USE_SCANNER_TIMEOUT_PERIOD_FOR_NEXT_CALLS, useScannerTimeoutPeriodForNextCalls);
    ...
    public void setup(boolean isSystemTable) throws IOException {
        ...
        String nameAsString = name.getMethodName().replaceAll("[^a-zA-Z0-9]", "_") + "-" + useScannerTimeoutPeriodForNextCalls;
        if (isSystemTable) {
        ...
        RestartFramework.at("after_data_put").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void expectRow(byte[] expected, Result result) {
        assertTrue("Expected row: " + Bytes.toString(expected), Bytes.equals(expected, result.getRow()));
    }

    private void expectNumTries(int expected) {
        assertEquals("Expected tryNumber=" + expected + ", actual=" + RSRpcServicesWithScanTimeout.tryNumber, expected, RSRpcServicesWithScanTimeout.tryNumber);
        // reset for next
    ...
    private void expectRetryOutOfOrderScannerNext(Supplier<ResultScanner> scannerSupplier) throws IOException {
        ...
        assertEquals("Expected to exhaust expectedResults array length=" + expectedResults.length + ", actual index=" + i, expectedResults.length, i);
    ...
    private void expectTimeoutOnNext(int timeout, Supplier<ResultScanner> scannerSupplier) throws IOException {
        ...
        try {
            ...
            assertTrue("Expected CallTimeoutException", e.getCause() instanceof CallTimeoutException || e.getCause() instanceof SocketTimeoutException);
        }
    ...
    private void expectTimeoutOnOpenScanner(int timeout, Supplier<ResultScanner> scannerSupplier) throws IOException {
        ...
        try {
            ...
            assertTrue("Expected SocketTimeoutException or CallTimeoutException, but was " + e.getCause(), e.getCause() instanceof CallTimeoutException || e.getCause() instanceof SocketTimeoutException);
        }
        expectTimeout(start, timeout);
    }

    private void expectTimeoutOnRenewScanner(int timeout, Supplier<ResultScanner> scannerSupplier) throws IOException {
        RSRpcServicesWithScanTimeout.setSleepForTimeout(timeout);
    ...
    private void expectTimeoutOnCloseScanner(int timeout, Supplier<ResultScanner> scannerSupplier) throws IOException {
        RSRpcServicesWithScanTimeout.setSleepForTimeout(timeout);
        ...
        public ScanResponse scan(final RpcController controller, final ScanRequest request) throws ServiceException {
            if (request.hasScannerId()) {
                ...
                if (throwAlways || (!threw && request.hasNextCallSeq() && seqNoToThrowOn == request.getNextCallSeq())) {
                    threw = true;
                    tryNumber++;
                    LOG.info("THROWING exception, tryNumber={}, tableScannerId={}", tryNumber, tableScannerId);
                    throw new ServiceException(new OutOfOrderScannerNextException());
                }
                if (!slept && (request.hasNextCallSeq() && seqNoToSleepOn == request.getNextCallSeq() || sleepOnRenew && request.getRenew())) {
                    try {
                        LOG.info("SLEEPING " + sleepTime);
                        Thread.sleep(sleepTime);
                    } catch (InterruptedException e) {
