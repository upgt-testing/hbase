...
public abstract class WALCorruptionWithMultiPutDueToDanglingByteBufferTestBase_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(WALCorruptionWithMultiPutDueToDanglingByteBufferTestBase_RestartInjected.class);
    ...
    public void test() throws Exception {
        ...
        try (Table table = UTIL.getConnection().getTable(TABLE_NAME)) {
            ...
            for (int i = 1; i <= 100; i++) {
                Put p = new Put(getBytes("row", i)).addColumn(CF, CQ, getBytes("value", i)).setDurability(Durability.ASYNC_WAL);
                puts.add(p);
                if (i % batchSize == 0) {
                    table.put(puts);
                    LOG.info("Wrote batch of {} rows from row {}", batchSize, Bytes.toString(puts.get(0).getRow()));
                    puts.clear();
            ...
            RESUME.countDown();
            RestartFramework.at("after_batch_puts").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("Put a single row to force a WAL sync...");
            table.put(new Put(Bytes.toBytes("row")).addColumn(CF, CQ, Bytes.toBytes("value")));
            RestartFramework.at("after_wal_sync").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_rs_restart").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
