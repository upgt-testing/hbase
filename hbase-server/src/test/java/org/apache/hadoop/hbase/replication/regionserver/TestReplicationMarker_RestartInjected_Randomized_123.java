...
public class TestReplicationMarker_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationMarker_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestReplicationMarker_RestartInjected.class);
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        // 1 sec
        conf1.setLong(REPLICATION_MARKER_CHORE_DURATION_KEY, 1000);
        ...
        RestartFramework.at("after_wait_for_sources").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_cluster1_start").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster2 = utility2.getMiniHBaseCluster();
        waitForReplicationTrackerTableCreation();
        cluster2 = utility2.getMiniHBaseCluster();
        RestartFramework.at("after_cluster2_start").on(cluster2).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Start cluster1
        ...
        RestartFramework.at("after_add_replication_peer").on(cluster1).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_tracker_table_creation").on(cluster2).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private static void waitForReplicationTrackerTableCreation() {
        Waiter.waitFor(conf2, 10000, (Waiter.Predicate) () -> utility2.getAdmin().tableExists(REPLICATION_SINK_TRACKER_TABLE_NAME));
    }
    ...
    public void testReplicationMarkerRow() throws Exception {
        ...
        RestartFramework.at("after_verify_marker_rows").on(cluster2).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_second_sleep").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Force log roll.
        wal1.rollWriter(true);
        RestartFramework.at("after_get_rows").on(cluster2).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_wal_roll").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.DELAYED_CRASH).execute();
        ...
        for (ReplicationSinkTrackerRow desc : list) {
            ...
            assertTrue(walName1ForCluster1.equals(desc.getWalName()) || walName2ForCluster1.equals(desc.getWalName()));
        }
        RestartFramework.at("after_wait_for_tracker_rows").on(cluster2).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_verify_replicated_entries").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.DELAYED_CRASH).execute();
        RestartFramework.at("after_first_sleep").on(cluster1).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_verify_table_existence").on(cluster1).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // This table shouldn't exist on cluster1 since
   ...
   */
    ...
    static class ReplicationSinkTrackerRow {
        ...
        public String toString() {
            return "ReplicationSinkTrackerRow{" + "region_server_name='" + region_server_name + '\'' + ", wal_name='" + wal_name + '\'' + ", timestamp=" + timestamp + ", offset=" + offset + '}';
        }
    }
