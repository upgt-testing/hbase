...
public class TestSplitTableRegionProcedure_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSplitTableRegionProcedure_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSplitTableRegionProcedure_RestartInjected.class);
    ...
    public void testRollbackForSplitTableRegionWithReplica() throws Exception {
        ...
        RegionInfo[] regions = MasterProcedureTestingUtility.createTable(procExec, tableName, null, columnFamilyName1);
        // Refresh after master restart
        procExec = getMasterProcedureExecutor();
        TableDescriptor td = TableDescriptorBuilder.newBuilder(UTIL.getAdmin().getDescriptor(tableName)).setRegionReplication(2).build();
        procExec.submitProcedure(new ModifyTableProcedure(procExec.getEnvironment(), td));
        // Refresh after master restart
        procExec = getMasterProcedureExecutor();
        RestartFramework.at("after_modify_table_submit").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        long procId = procExec.submitProcedure(new SplitTableRegionProcedure(procExec.getEnvironment(), regions[0], HConstants.CATALOG_FAMILY));
        RestartFramework.at("after_wait_split_complete").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Wait for the completion.
        ProcedureTestingUtility.waitProcedure(procExec, procId);
        // Refresh after master restart
        procExec = getMasterProcedureExecutor();
        RestartFramework.at("after_split_submit").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        HBaseTestingUtility.await(2000, () -> {
            try {
                ...
                if (am.getRegionStates().getRegionState(replicaRegion) != null && am.getRegionStates().getRegionState(replicaRegion).isOpened()) {
                    return true;
                }
                return false;
            } catch (Exception e) {
    ...
    public void testSplitTableRegion() throws Exception {
        final TableName tableName = TableName.valueOf(name.getMethodName());
        RestartFramework.at("after_wait_split_complete").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureExecutor<MasterProcedureEnv> procExec = getMasterProcedureExecutor();
        ...
        RestartFramework.at("after_split_submit").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_insert_data").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSplitDetectsModifyTableProcedure() throws Exception {
        ...
        SplitTableRegionProcedure splitProcedure = new SplitTableRegionProcedure(procExec.getEnvironment(), regions[0], HConstants.CATALOG_FAMILY);
