...
public class TestMasterRestartAfterDisablingTable_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterRestartAfterDisablingTable_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestMasterRestartAfterDisablingTable_RestartInjected.class);
    ...
    public void testForCheckingIfEnableAndDisableWorksFineAfterSwitch() throws Exception {
        ...
        StartMiniClusterOption option = StartMiniClusterOption.builder().numMasters(NUM_MASTERS).build();
        ...
        RestartFramework.at("after_verify_regions_online").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = TEST_UTIL.getHBaseCluster();
        RestartFramework.at("after_verify_regions_offline").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        log("Disabling table\n");
        ...
        log("Waiting for no more RIT\n");
        RestartFramework.at("after_master_failover").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TEST_UTIL.waitUntilNoRegionsInTransition(60000);
