...
public class TestMalformedCellFromClient_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestMalformedCellFromClient_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMalformedCellFromClient_RestartInjected.class);
    ...
    public void before() throws Exception {
        TableDescriptor desc = TableDescriptorBuilder.newBuilder(TABLE_NAME).setColumnFamily(ColumnFamilyDescriptorBuilder.of(FAMILY)).setValue(HRegion.HBASE_MAX_CELL_SIZE_KEY, String.valueOf(CELL_SIZE)).build();
    ...
    public void testRegionException() throws InterruptedException, IOException {
        ...
        try (Table table = TEST_UTIL.getConnection().getTable(TABLE_NAME)) {
            ...
            RestartFramework.at("after_successful_get").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testRegionExceptionByAsync() throws Exception {
        ...
        try (AsyncConnection asyncConnection = ConnectionFactory.createAsyncConnection(TEST_UTIL.getConfiguration()).get()) {
            ...
            Cell cell = result.getColumnLatestCell(FAMILY, null);
            assertTrue(Bytes.equals(CellUtil.cloneValue(cell), new byte[10]));
            RestartFramework.at("after_async_get").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testAtomicOperations() throws Exception {
        ...
        try (Table table = TEST_UTIL.getConnection().getTable(TABLE_NAME)) {
            ...
            Cell cell = result.getColumnLatestCell(FAMILY, null);
            assertTrue(Bytes.equals(CellUtil.cloneValue(cell), new byte[10]));
            RestartFramework.at("after_atomic_operation").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private static ClientProtos.MultiRequest createRequest(RowMutations rm, byte[] regionName) throws IOException {
        ...
        ClientProtos.Condition condition = ProtobufUtil.toCondition(rm.getRow(), FAMILY, null, CompareOperator.EQUAL, new byte[10], null, null, false);
    ...
    public void testNonAtomicOperations() throws InterruptedException, IOException {
        ...
        try (Table table = TEST_UTIL.getConnection().getTable(TABLE_NAME)) {
            ...
            RestartFramework.at("after_batch_with_failures").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
