...
public class TestCleanupMetaWAL_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestCleanupMetaWAL_RestartInjected.class);

    private final static HBaseTestingUtility TEST_UTIL = new HBaseTestingUtility();

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCleanupMetaWAL_RestartInjected.class);
    ...
    public void testCleanupMetaWAL() throws Exception {
        ...
        RestartFramework.at("after_meta_move").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        LOG.info("KILL");
        TEST_UTIL.getMiniHBaseCluster().killRegionServer(serverWithMeta.getServerName());
        LOG.info("WAIT");
        RestartFramework.at("before_wal_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_scp_wait").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
