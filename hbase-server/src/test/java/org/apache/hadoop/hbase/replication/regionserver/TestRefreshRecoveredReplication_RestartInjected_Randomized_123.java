...
public class TestRefreshRecoveredReplication_RestartInjected_Randomized_123 extends TestReplicationBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRefreshRecoveredReplication_RestartInjected.class);
    ...
    public void setup() throws Exception {
        ...
        TableDescriptor table = TableDescriptorBuilder.newBuilder(tablename).setColumnFamily(ColumnFamilyDescriptorBuilder.newBuilder(famName).setScope(HConstants.REPLICATION_SCOPE_GLOBAL).build()).build();
        ...
        RestartFramework.at("after_setup_base").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        table2 = UTIL2.getConnection().getTable(tablename);
        RestartFramework.at("after_table_setup").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testReplicationRefreshSource() throws Exception {
        RestartFramework.at("after_replication_completion").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // put some data
        ...
        RestartFramework.at("after_regionserver_abort").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_data_puts").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_wait_for_recovered_peer").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
