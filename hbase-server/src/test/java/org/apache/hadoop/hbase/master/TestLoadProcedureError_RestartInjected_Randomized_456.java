...
public class TestLoadProcedureError_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestLoadProcedureError_RestartInjected.class);
    ...
    public void testLoadError() throws Exception {
        ...
        RestartFramework.at("after_procedure_submit").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ARRIVE.await();
        RestartFramework.at("after_fail_load_clear").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FAIL_LOAD = true;
        ...
        waitNoMaster();
        RestartFramework.at("after_successful_master_init").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_finish_proc_set").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
