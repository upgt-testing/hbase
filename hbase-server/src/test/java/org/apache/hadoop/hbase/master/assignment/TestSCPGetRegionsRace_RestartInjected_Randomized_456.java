...
public class TestSCPGetRegionsRace_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSCPGetRegionsRace_RestartInjected.class);
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_kill_rs").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_future_complete").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Future<?> future = am.moveAsync(new RegionPlan(region, source, dest));
        ARRIVE_REPORT.await();
        RestartFramework.at("after_wait_for_procedures").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ARRIVE_REPORT = null;
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        ...
        RestartFramework.at("after_arrive_report").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RESUME_REPORT.countDown();
        RestartFramework.at("after_get_proc_lock").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // kill the source region server
        ARRIVE_GET = new CountDownLatch(1);
        RestartFramework.at("after_resume_scp").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        future.get();
        RestartFramework.at("after_verify_region_location").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        am = master.getAssignmentManager();
        ...
        RestartFramework.at("after_arrive_get").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.waitFor(60000, () -> finalMaster.getProcedures().stream().allMatch(p -> p.isFinished() || (!(p instanceof ServerCrashProcedure) && !(p instanceof TransitRegionStateProcedure))));
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        am = master.getAssignmentManager();
