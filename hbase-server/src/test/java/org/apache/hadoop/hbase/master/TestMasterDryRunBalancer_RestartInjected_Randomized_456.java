...
public class TestMasterDryRunBalancer_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterDryRunBalancer_RestartInjected.class);
    ...
    public void testDryRunBalancer() throws Exception {
        ...
        cluster = TEST_UTIL.getMiniHBaseCluster();
        int numRegions = 100;
        RestartFramework.at("after_unbalance").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        cluster = TEST_UTIL.getMiniHBaseCluster();
        RestartFramework.at("after_dry_run_balance").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_disable_balancer").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = Mockito.spy(TEST_UTIL.getHBaseCluster().getMaster());
    ...
    private void assertServerContainsAllRegions(ServerName serverName, TableName tableName) throws IOException {
        ...
        assertEquals(numRegions, TEST_UTIL.getMiniHBaseCluster().getRegionServer(serverName).getRegions(tableName).size());
    }

    private void waitForRegionsToSettle(HMaster master) {
        Waiter.waitFor(TEST_UTIL.getConfiguration(), 60_000, () -> master.getAssignmentManager().getRegionStates().getRegionsInTransitionCount() <= 0);
    }
}
