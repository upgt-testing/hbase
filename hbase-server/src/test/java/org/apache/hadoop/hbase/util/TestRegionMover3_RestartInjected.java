...
public class TestRegionMover3_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestRegionMover3_RestartInjected.class);
  ...
  public void testRegionUnloadWithRack() throws Exception {
    ...
    try (RegionMover regionMover = rmBuilder.build()) {
      ...
      RestartFramework.at("after_unload_from_rack")
          .on(cluster)
          .restart("regionserver")
          .withIndex(2)
          .withMode(RestartMode.GRACEFUL)
    ...
    try (RegionMover regionMover = rmBuilder.build()) {
      regionMover.unloadFromRack();
      RestartFramework.at("after_unload_with_custom_rack")
          .on(cluster)
          .restart("regionserver")
          .withIndex(1)
          .withMode(RestartMode.GRACEFUL)
