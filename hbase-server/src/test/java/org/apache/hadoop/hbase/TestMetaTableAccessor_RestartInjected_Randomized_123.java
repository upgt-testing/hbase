...
public class TestMetaTableAccessor_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMetaTableAccessor_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestMetaTableAccessor_RestartInjected.class);
    ...
    public void testGetMergeRegions() throws Exception {
        ...
        try (Admin admin = UTIL.getAdmin()) {
            ...
            admin.mergeRegionsAsync(regions.get(0).getRegionName(), regions.get(1).getRegionName(), false).get(60, TimeUnit.SECONDS);
            RestartFramework.at("after_first_region_merge").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin.mergeRegionsAsync(regions.get(2).getRegionName(), regions.get(3).getRegionName(), false).get(60, TimeUnit.SECONDS);
            RestartFramework.at("after_second_region_merge").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_delete_merge_qualifiers").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAddMergeRegions() throws IOException {
        ...
        for (int i = 0; i < limit; i++) {
            RegionInfo ri = RegionInfoBuilder.newBuilder(tn).setStartKey(previous).setEndKey(Bytes.toBytes(i)).build();
            ris.add(ri);
        }
    ...
    public void testRetrying() throws IOException, InterruptedException {
        ...
        MetaTask writer = new MetaTask(connection, "writer") {
            ...
            void metaTask() throws IOException {
                MetaTableAccessor.addRegionsToMeta(connection, Collections.singletonList(regions.get(0)), 1);
    ...
    public void testScanMetaForTable() throws IOException, InterruptedException {
        final TableName tableName = TableName.valueOf(name.getMethodName());
        LOG.info("Started " + tableName);
        RestartFramework.at("after_first_table_create").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        /**
        ...
        RestartFramework.at("after_second_table_create").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testParseRegionInfoFromRegionName() throws IOException {
        RegionInfo originalRegionInfo = RegionInfoBuilder.newBuilder(TableName.valueOf(name.getMethodName())).setRegionId(999999L).setStartKey(Bytes.toBytes("2")).setEndKey(Bytes.toBytes("3")).setReplicaId(1).build();
        RegionInfo newParsedRegionInfo = MetaTableAccessor.parseRegionInfoFromRegionName(originalRegionInfo.getRegionName());
        assertEquals("Parse TableName error", originalRegionInfo.getTable(), newParsedRegionInfo.getTable());
        assertEquals("Parse regionId error", originalRegionInfo.getRegionId(), newParsedRegionInfo.getRegionId());
        assertTrue("Parse startKey error", Bytes.equals(originalRegionInfo.getStartKey(), newParsedRegionInfo.getStartKey()));
        assertEquals("Parse replicaId error", originalRegionInfo.getReplicaId(), newParsedRegionInfo.getReplicaId());
        assertTrue("We can't parse endKey from regionName only", Bytes.equals(HConstants.EMPTY_END_ROW, newParsedRegionInfo.getEndKey()));
    }
    ...
    public void testMetaReaderGetColumnMethods() {
        ...
        assertArrayEquals(Bytes.toBytes(HConstants.SERVER_QUALIFIER_STR + MetaTableAccessor.META_REPLICA_ID_DELIMITER + "002A"), MetaTableAccessor.getServerColumn(42));
        assertArrayEquals(HConstants.STARTCODE_QUALIFIER, MetaTableAccessor.getStartCodeColumn(0));
        assertArrayEquals(Bytes.toBytes(HConstants.STARTCODE_QUALIFIER_STR + MetaTableAccessor.META_REPLICA_ID_DELIMITER + "002A"), MetaTableAccessor.getStartCodeColumn(42));
        assertArrayEquals(HConstants.SEQNUM_QUALIFIER, MetaTableAccessor.getSeqNumColumn(0));
        assertArrayEquals(Bytes.toBytes(HConstants.SEQNUM_QUALIFIER_STR + MetaTableAccessor.META_REPLICA_ID_DELIMITER + "002A"), MetaTableAccessor.getSeqNumColumn(42));
    }
    ...
    public static void assertMetaLocation(Table meta, byte[] row, ServerName serverName, long seqNum, int replicaId, boolean checkSeqNum) throws IOException {
        ...
        assertTrue(Bytes.equals(result.getValue(HConstants.CATALOG_FAMILY, MetaTableAccessor.getServerColumn(replicaId)), Bytes.toBytes(serverName.getAddress().toString())));
    ...
    public void testMetaLocationForRegionReplicasIsAddedAtRegionSplit() throws IOException {
        long regionId = EnvironmentEdgeManager.currentTime();
        ServerName serverName0 = ServerName.valueOf("foo", 60010, ThreadLocalRandom.current().nextLong());
    ...
    public void testMetaLocationForRegionReplicasIsAddedAtRegionMerge() throws IOException {
        ...
        try (Table meta = MetaTableAccessor.getMetaHTable(connection)) {
            ...
            MetaTableAccessor.addRegionsToMeta(connection, regionInfos, 3);
            MetaTableAccessor.mergeRegions(connection, merged, getMapOfRegionsToSeqNum(parentA, parentB), serverName0, 3);
    ...
    public void testMastersSystemTimeIsUsedInMergeRegions() throws IOException {
        ...
        try (Table meta = MetaTableAccessor.getMetaHTable(connection)) {
            ...
            try {
                ...
                MetaTableAccessor.mergeRegions(connection, mergedRegionInfo, getMapOfRegionsToSeqNum(regionInfoA, regionInfoB), sn, 1);
            } finally {
