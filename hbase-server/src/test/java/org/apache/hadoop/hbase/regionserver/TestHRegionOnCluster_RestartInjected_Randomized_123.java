...
public class TestHRegionOnCluster_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestHRegionOnCluster_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestHRegionOnCluster_RestartInjected.class);
    ...
    public void testDataCorrectnessReplayingRecoveredEdits() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_first_data_put").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            ...
            RestartFramework.at("after_second_data_put").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            TEST_UTIL.waitUntilAllRegionsAssigned(table.getName());
            ...
            RestartFramework.at("after_third_data_put").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            // Kill target server
