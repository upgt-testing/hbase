...
public class TestCoprocessorMetrics_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCoprocessorMetrics_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestCoprocessorMetrics_RestartInjected.class);
    ...
    public void testRegionServerObserver() throws IOException {
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            ...
            RestartFramework.at("after_wal_roll").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        // Find out the MetricRegistry used by the CP using the global registries
    ...
    public void testWALObserver() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            ...
            try (Table table = connection.getTable(TableName.valueOf(name.getMethodName()))) {
                ...
                RestartFramework.at("after_put").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
    ...
    public void testRegionObserverSingleRegion() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            Admin admin = connection.getAdmin()) {
            admin.createTable(new HTableDescriptor(tableName).addFamily(new HColumnDescriptor(foo)).// add the coprocessor for the region
            ...
            try (Table table = connection.getTable(tableName)) {
                ...
                // 2 gets
                table.get(new Get(foo));
                RestartFramework.at("after_get").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    ...
    public void testRegionObserverMultiRegion() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            Admin admin = connection.getAdmin()) {
            admin.createTable(new HTableDescriptor(tableName).addFamily(new HColumnDescriptor(foo)).// add the coprocessor for the region
            addCoprocessor(CustomRegionObserver.class.getName()), // create with
            new byte[][] { foo });
            // 2 regions
            RestartFramework.at("after_table_create").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (Table table = connection.getTable(tableName);
    ...
    public void testRegionObserverMultiTable() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            Admin admin = connection.getAdmin()) {
            admin.createTable(new HTableDescriptor(tableName1).addFamily(new HColumnDescriptor(foo)).// add the coprocessor for the region
            addCoprocessor(CustomRegionObserver.class.getName()));
            ...
            admin.createTable(new HTableDescriptor(tableName2).addFamily(new HColumnDescriptor(foo)).// add the coprocessor for the region
            addCoprocessor(CustomRegionObserver.class.getName()));
    ...
    public void testRegionObserverMultiCoprocessor() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            Admin admin = connection.getAdmin()) {
            admin.createTable(new HTableDescriptor(tableName).addFamily(new HColumnDescriptor(foo)).// add the coprocessor for the region. We add two different coprocessors
            addCoprocessor(CustomRegionObserver.class.getName()).addCoprocessor(CustomRegionObserver2.class.getName()));
            RestartFramework.at("after_table_create").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (Table table = connection.getTable(tableName)) {
                table.get(new Get(foo));
                // 2 gets
    ...
    public void testRegionObserverAfterRegionClosed() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            ...
            try (RegionLocator locator = connection.getRegionLocator(tableName)) {
                ...
                RestartFramework.at("after_region_unassign").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRegionObserverEndpoint() throws IOException, ServiceException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration());
            ...
            try (Table table = connection.getTable(tableName)) {
                ...
                RestartFramework.at("after_endpoint_execution").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
        // Find out the MetricRegistry used by the CP using the global registries
