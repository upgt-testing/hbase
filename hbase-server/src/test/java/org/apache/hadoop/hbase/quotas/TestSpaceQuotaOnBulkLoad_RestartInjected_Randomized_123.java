...
public class TestSpaceQuotaOnBulkLoad_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSpaceQuotaOnBulkLoad_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSpaceQuotaOnBulkLoad_RestartInjected.class);
    ...
    public void testNoBulkLoadsWithNoWrites() throws Exception {
        RestartFramework.at("after_violation_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Put p = new Put(Bytes.toBytes("to_reject"));
        p.addColumn(Bytes.toBytes(SpaceQuotaHelperForTests.F1), Bytes.toBytes("to"), Bytes.toBytes("reject"));
        RestartFramework.at("after_bulk_load_rejection").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TableName tableName = helper.writeUntilViolationAndVerifyViolation(SpaceViolationPolicy.NO_WRITES, p);
        ...
        RpcRetryingCallerFactory factory = new RpcRetryingCallerFactory(TEST_UTIL.getConfiguration(), conn.getConnectionConfiguration());
    ...
    public void testAtomicBulkLoadUnderQuota() throws Exception {
        ...
        RestartFramework.at("after_bulk_load_and_table_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
