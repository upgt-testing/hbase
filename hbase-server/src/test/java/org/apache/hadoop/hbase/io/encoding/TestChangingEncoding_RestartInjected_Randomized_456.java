...
public class TestChangingEncoding_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestChangingEncoding_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestChangingEncoding_RestartInjected.class);
    ...
    public void testChangingEncoding() throws Exception {
        ...
        for (boolean onlineChange : new boolean[] { false, true }) {
            for (DataBlockEncoding encoding : ENCODINGS_TO_ITERATE) {
                ...
                if (iterationCount == 0) {
                    RestartFramework.at("after_encoding_change").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
                writeSomeNewData();
                if (iterationCount == 0) {
