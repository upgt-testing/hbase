...
public class TestFlushWithThroughputController_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFlushWithThroughputController_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestFlushWithThroughputController_RestartInjected.class);
    ...
    private Pair<Double, Long> generateAndFlushData(Table table) throws IOException {
        ...
        for (int i = 0; i < NUM_FLUSHES; i++) {
            ...
            RestartFramework.at("after_put_iteration_" + i).on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_flush_iteration_" + i).on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    ...
    public void testFlushThroughputTuning() throws Exception {
        ...
        RestartFramework.at("after_put_operations").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_initial_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(5000);
