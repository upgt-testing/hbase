...
public class TestAsyncTableAdminApi2_RestartInjected_Randomized_456 extends TestAsyncAdminBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAsyncTableAdminApi2_RestartInjected.class);
    ...
    public void testDisableCatalogTable() throws Exception {
        ...
        RestartFramework.at("after_disable_meta_attempt").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCompactionTimestamps() throws Exception {
        RestartFramework.at("after_table_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_flush").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        createTableWithDefaultConf(tableName);
        ...
        RestartFramework.at("after_multiple_flushes").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
