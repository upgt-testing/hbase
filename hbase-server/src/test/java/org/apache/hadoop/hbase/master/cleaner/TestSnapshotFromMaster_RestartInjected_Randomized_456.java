...
public class TestSnapshotFromMaster_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSnapshotFromMaster_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSnapshotFromMaster_RestartInjected.class);
    ...
    public void testSnapshotHFileArchiving() throws Exception {
        ...
        UTIL.waitFor(60000, 1000, new ExplainingPredicate<Exception>() {
            ...
            public String explainFailure() throws Exception {
                return "Still have some hfiles in the archive, when their snapshot has been deleted: " + getHFiles(archiveDir, fs, TABLE_NAME);
            }
        });
    }
    ...
    private SnapshotDescription createSnapshotWithTtl(final String snapshotName, final long ttl) throws IOException {
        SnapshotTestingUtils.SnapshotMock snapshotMock = new SnapshotTestingUtils.SnapshotMock(UTIL.getConfiguration(), fs, rootDir);
        SnapshotTestingUtils.SnapshotMock.SnapshotBuilder builder = snapshotMock.createSnapshotV2(snapshotName, "test", 0, ttl);
        builder.commit();
        return builder.getSnapshotDescription();
    }
    ...
    public void testAsyncSnapshotWillNotBlockSnapshotHFileCleaner() throws Exception {
        ...
        Future<Void> future = UTIL.getAdmin().snapshotAsync(new org.apache.hadoop.hbase.client.SnapshotDescription(snapshotName, TABLE_NAME, SnapshotType.FLUSH));
