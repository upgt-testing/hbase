 ...
 */
public abstract class AbstractTestDLS_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(AbstractTestDLS_RestartInjected.class);
    ...
    private void startCluster(int numRS) throws Exception {
        ...
        StartMiniClusterOption option = StartMiniClusterOption.builder().numMasters(NUM_MASTERS).numRegionServers(numRS).build();
    ...
    public void testMasterStartsUpWithLogSplittingWork() throws Exception {
        ...
        try (Table ht = installTable(numRegionsToCreate)) {
            ...
            RestartFramework.at("after_regions_online").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_data_verification").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testThreeRSAbort() throws Exception {
        ...
        try (Table table = installTable(numRegionsToCreate)) {
            ...
            RestartFramework.at("after_data_populate").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_kill_region_servers").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_regions_reassigned").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            int rows;
            try {
