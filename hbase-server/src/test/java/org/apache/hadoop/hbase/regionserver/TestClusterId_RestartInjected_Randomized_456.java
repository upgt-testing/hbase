...
public class TestClusterId_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestClusterId_RestartInjected.class);
    ...
    public void testClusterId() throws Exception {
        ...
        Thread.sleep(10000);
        RestartFramework.at("after_hbase_cluster_start").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        StartMiniClusterOption option = StartMiniClusterOption.builder().numMasters(1).numRegionServers(0).build();
        TEST_UTIL.startMiniHBaseCluster(option);
        rst.waitForServerOnline();
        String clusterId = ZKClusterId.readClusterIdZNode(TEST_UTIL.getZooKeeperWatcher());
        RestartFramework.at("after_server_online").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRewritingClusterIdToPB() throws Exception {
        ...
        RestartFramework.at("after_hbase_cluster_start").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_server_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
