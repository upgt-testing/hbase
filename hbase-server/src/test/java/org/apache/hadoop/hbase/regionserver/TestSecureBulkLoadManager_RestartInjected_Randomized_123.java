...
public class TestSecureBulkLoadManager_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSecureBulkLoadManager_RestartInjected.class);
    ...
    public TestSecureBulkLoadManager_RestartInjected_Randomized_123(Boolean useFileBasedSFT) {
    ...
    public void testForRaceCondition() throws Exception {
        ...
        testUtil.getMiniHBaseCluster().getRegionServerThreads().get(0).getRegionServer().getSecureBulkLoadManager().setFsCreatedListener(fsCreatedListener);
        RestartFramework.at("after_data_verification").on(testUtil.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        /// create table
        ...
        RestartFramework.at("after_table_create").on(testUtil.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        laterBulkload.start();
        Threads.shutdown(ealierBulkload);
        Threads.shutdown(laterBulkload);
        RestartFramework.at("after_bulkload_threads_complete").on(testUtil.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_hfile_prepare_dir2").on(testUtil.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Result r = t.get(get1);
        Assert.assertArrayEquals(r.getValue(FAMILY, COLUMN), value1);
        r = t.get(get3);
        RestartFramework.at("after_hfile_prepare_dir1").on(testUtil.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
