...
public class TestRegionServerRejectDuringAbort_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionServerRejectDuringAbort_RestartInjected.class);
    ...
    public void testRejectRequestsOnAbort() throws Exception {
        ...
        assertNotNull("couldn't find a server without meta, but with test table regions", serverWithoutMeta);
        ...
        return () -> {
            try {
                ...
                try (Connection conn = ConnectionFactory.createConnection(conf);
                    Table table = conn.getTableBuilder(TABLE_NAME, null).setRequestAttribute("test", new byte[] { 0 }).build()) {
