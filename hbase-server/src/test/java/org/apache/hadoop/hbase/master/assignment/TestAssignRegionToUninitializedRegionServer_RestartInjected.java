...
public class TestAssignRegionToUninitializedRegionServer_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestAssignRegionToUninitializedRegionServer_RestartInjected.class);
  ...
  public void testMove() throws Exception {
    ...
    RestartFramework.at("before_start_new_rs")
        .on(UTIL.getMiniHBaseCluster())
    ...
    RestartFramework.at("after_rs_arrive_before_report")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_get_servers")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    try {
      ...
      RestartFramework.at("after_first_move_attempt")
          .on(UTIL.getMiniHBaseCluster())
          .restart("master")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_resume_rs")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_new_rs_online")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_second_move")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
