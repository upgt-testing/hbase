...
public class TestMasterFailoverBalancerPersistence_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterFailoverBalancerPersistence_RestartInjected.class);
    ...
    public void testMasterFailoverBalancerPersistence() throws Exception {
        ...
        RestartFramework.at("after_balancer_off_check").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_failover").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = TEST_UTIL.getHBaseCluster();
        active = killActiveAndWaitForNewActive(cluster);
        RestartFramework.at("after_balancer_switch_off").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = TEST_UTIL.getHBaseCluster();
        RestartFramework.at("after_second_failover").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        active = killActiveAndWaitForNewActive(cluster);
        RestartFramework.at("after_balancer_still_on_check").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_balancer_on_check").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = TEST_UTIL.getHBaseCluster();
