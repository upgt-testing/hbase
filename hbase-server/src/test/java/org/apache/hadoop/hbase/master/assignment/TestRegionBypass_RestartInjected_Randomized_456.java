...
public class TestRegionBypass_RestartInjected_Randomized_456 {

    private final static Logger LOG = LoggerFactory.getLogger(TestRegionBypass_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionBypass_RestartInjected.class);
    ...
    public void testBypass() throws IOException, InterruptedException {
        ...
        RestartFramework.at("after_unassign_regions").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (RegionInfo ri : regions) {
        ...
        TEST_UTIL.waitFor(30000, () -> pids.stream().allMatch(pid -> TEST_UTIL.getHBaseCluster().getMaster().getMasterProcedureExecutor().isStarted(pid)));
        RestartFramework.at("after_failed_assign_attempts").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_assign_with_override").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_active_procs_cleared").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_regions").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_override_procs_completed").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_bypass_procedures").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_submit_procedures").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TEST_UTIL.waitFor(30000, () -> TEST_UTIL.getHBaseCluster().getMaster().getMasterProcedureExecutor().getActiveProcIds().isEmpty());
        RestartFramework.at("after_procedures_started").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Now assign with the override flag.
        for (RegionInfo ri : regions) {
            TEST_UTIL.getHbck().assigns(Arrays.<String>asList(ri.getEncodedName()), true);
        }
        TEST_UTIL.waitFor(60000, () -> TEST_UTIL.getHBaseCluster().getMaster().getMasterProcedureExecutor().getActiveProcIds().isEmpty());
        for (RegionInfo ri : regions) {
    ...
    public static class StallingAssignProcedure extends TransitRegionStateProcedure {
        ...
        public StallingAssignProcedure(MasterProcedureEnv env, RegionInfo hri, ServerName assignCandidate, boolean forceNewPlan, TransitionType type) {
            super(env, hri, assignCandidate, forceNewPlan, type);
            init(env);
        }

        private void init(MasterProcedureEnv env) {
            RegionStateNode regionNode = env.getAssignmentManager().getRegionStates().getOrCreateRegionStateNode(getRegion());
