...
public class TestJMXListener_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestJMXListener_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestJMXListener_RestartInjected.class);
    ...
    public static void setupBeforeClass() throws Exception {
        ...
        // new BoundSocketMaker(HBaseTestingUtility::randomFreePort);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            try {
                connector = JMXConnectorFactory.connect(JMXListener.buildJMXServiceURL(CONNECTOR_PORT, CONNECTOR_PORT));
                break;
    ...
    public void testStart() throws Exception {
        JMXConnector connector = JMXConnectorFactory.connect(JMXListener.buildJMXServiceURL(CONNECTOR_PORT, CONNECTOR_PORT));
        RestartFramework.at("after_jmx_connect").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStop() throws Exception {
        ...
        JMXConnector connector = JMXConnectorFactory.newJMXConnector(JMXListener.buildJMXServiceURL(CONNECTOR_PORT, CONNECTOR_PORT), null);
        RestartFramework.at("before_cluster_shutdown").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetRegionServerCoprocessors() throws Exception {
        RestartFramework.at("before_check_coprocessors").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
