...
public class TestAsyncTableRSCrashPublish_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAsyncTableRSCrashPublish_RestartInjected.class);
    ...
    public void test() throws IOException, ExecutionException, InterruptedException {
        ...
        try (AsyncConnection connection = ConnectionFactory.createAsyncConnection(conf).get()) {
            ...
            RestartFramework.at("after_initial_get").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_stop_rs").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            UTIL.waitFor(60000, () -> locator.getRegionLocationInCache(TABLE_NAME, HConstants.EMPTY_START_ROW) == null);
            RestartFramework.at("after_cache_clear").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            connection.getTable(TABLE_NAME).get(new Get(Bytes.toBytes(0))).join();
            assertNotEquals(serverName, locator.getRegionLocationInCache(TABLE_NAME, HConstants.EMPTY_START_ROW).getDefaultRegionLocation().getServerName());
        }
    }
}
