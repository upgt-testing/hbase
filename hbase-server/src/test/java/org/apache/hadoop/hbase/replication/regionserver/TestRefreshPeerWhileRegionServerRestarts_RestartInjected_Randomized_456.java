...
public class TestRefreshPeerWhileRegionServerRestarts_RestartInjected_Randomized_456 extends TestReplicationBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRefreshPeerWhileRegionServerRestarts_RestartInjected.class);
    ...
    public void testRestart() throws Exception {
        ...
        RestartFramework.at("after_set_region_server_impl").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ARRIVE = arrive;
        RestartFramework.at("after_disable_peer_complete").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        ARRIVE.await();
        cluster = UTIL1.getMiniHBaseCluster();
        RestartFramework.at("after_disable_peer_async").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_arrive_latch_await").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_start_region_server").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            UTIL1.waitFor(30000, () -> {
                for (Procedure<?> proc : UTIL1.getMiniHBaseCluster().getMaster().getProcedures()) {
                    if (proc instanceof DisablePeerProcedure) {
                        return ((DisablePeerProcedure) proc).getCurrentStateId() == PeerModificationState.POST_PEER_MODIFICATION_VALUE;
                    }
                }
                return false;
            });
            RestartFramework.at("after_wait_procedure_state").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = UTIL1.getMiniHBaseCluster();
        } finally {
            // let the new region server go
            RESUME.countDown();
            RestartFramework.at("after_resume_latch").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = UTIL1.getMiniHBaseCluster();
        }
        ...
        ReplicationPeer peer = regionServerFuture.get().getReplicationSourceService().getReplicationManager().getReplicationPeers().getPeer(PEER_ID2);
