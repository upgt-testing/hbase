...
public class TestMultiParallel_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMultiParallel_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestMultiParallel_RestartInjected.class);
    ...
    // also used for testing HTable pool size
     ...
     * It inserts one row per region via the batch operation, and then checks the number of active
     * threads.
     * <p/>
     * For HBASE-3553
     */
        ...
        try (Connection connection = ConnectionFactory.createConnection(UTIL.getConfiguration())) {
            ...
            try {
                try (Table t = connection.getTable(TEST_TABLE, executor)) {
                    // creates a Put for every region
                    List<Put> puts = constructPutRequests();
                    t.batch(puts, null);
                    RestartFramework.at("after_batch_put").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBatchWithGet() throws Exception {
        ...
        RestartFramework.at("after_batch_get").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void doTestFlushCommits(boolean doAbort) throws Exception {
        ...
        if (doAbort) {
            ...
            table.put(puts);
            RestartFramework.at("after_abort_recovery").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    ...
    public void testBatchWithDelete() throws Exception {
        ...
        RestartFramework.at("after_batch_delete").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Table table = UTIL.getConnection().getTable(TEST_TABLE);
        // Load some data
        List<Put> puts = constructPutRequests();
        Object[] results = new Object[puts.size()];
    ...
    public void testHTableDeleteWithList() throws Exception {
        ...
        RestartFramework.at("after_delete").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Table table = UTIL.getConnection().getTable(TEST_TABLE);
        // Load some data
        List<Put> puts = constructPutRequests();
        Object[] results = new Object[puts.size()];
    ...
    public void testBatchWithIncrementAndAppend() throws Exception {
        ...
        RestartFramework.at("after_increment_append").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNonceCollision() throws Exception {
        ...
        try {
            ...
            validateResult(result, QUALIFIER, Bytes.toBytes(1L));
            RestartFramework.at("after_increment").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBatchWithMixedActions() throws Exception {
        ...
        get = new Get(KEYS[50]);
        RestartFramework.at("after_mixed_batch").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        get.addColumn(BYTES_FAMILY, qual2);
        r = table.get(get);
        validateResult(r, qual2, val2);
        get = new Get(KEYS[50]);
    ...
    public static class MyMasterObserver implements MasterObserver, MasterCoprocessor {
        ...
        @Override
        public void postBalance(final ObserverContext<MasterCoprocessorEnvironment> ctx, BalanceRequest request, List<RegionPlan> plans) throws IOException {
            if (!plans.isEmpty()) {
