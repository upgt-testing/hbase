...
public class TestInitializeStoreFileTracker_RestartInjected {
  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestInitializeStoreFileTracker_RestartInjected.class);
  ...
  public void testMigrateStoreFileTracker() throws IOException, InterruptedException {
    ...
    RestartFramework.at("after_tables_created")
        .on(HTU.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_tracker_removed_from_descriptors")
        .on(HTU.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_master_restart_ready")
        .on(HTU.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_migration_completed")
        .on(HTU.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
