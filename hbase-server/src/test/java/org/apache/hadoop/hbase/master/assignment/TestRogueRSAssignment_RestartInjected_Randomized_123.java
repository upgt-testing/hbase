...
public class TestRogueRSAssignment_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRogueRSAssignment_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRogueRSAssignment_RestartInjected.class);
    ...
    public static void setupCluster() throws Exception {
        ...
        assertNotNull(admin);
        cluster = UTIL.getHBaseCluster();
        RestartFramework.at("after_get_master").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin = UTIL.getAdmin();
        master = cluster.getMaster();
        assertNotNull(master);
        RestartFramework.at("after_get_cluster").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = UTIL.getHBaseCluster();
    }
    ...
    public void setup() throws IOException {
        RestartFramework.at("after_turn_off_balancer").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Turn off balancer
    ...
    public void tearDown() throws Exception {
        ...
        RestartFramework.at("after_delete_tables").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_turn_on_balancer").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Turn on balancer
    ...
    public void testReportRSWithWrongRegion() throws Exception {
        ...
        RestartFramework.at("after_create_table").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_make_fake_request").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // sending fake request to master
        // TODO: replace YouAreDeadException with appropriate exception as and when necessary
        exception.expect(ServiceException.class);
        RestartFramework.at("after_send_fake_request").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
