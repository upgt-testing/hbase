...
public class TestMasterMetricsWrapper_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterMetricsWrapper_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestMasterMetricsWrapper_RestartInjected.class);
    ...
    public void testInfo() throws IOException {
        ...
        RestartFramework.at("after_decommission_regionserver").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_stop_regionserver").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_metrics_wrapper_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_master_notice_stopped_rs").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_metrics_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = TEST_UTIL.getMiniHBaseCluster().getMaster();
        assertEquals(1, info.getNumDrainingRegionServers());
        assertEquals(master.getServerManager().getOnlineServersList().get(0).toString(), info.getDrainingRegionServers());
    }
    ...
    public void testQuotaSnapshotConversion() {
        ...
        RestartFramework.at("after_quota_wrapper_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
