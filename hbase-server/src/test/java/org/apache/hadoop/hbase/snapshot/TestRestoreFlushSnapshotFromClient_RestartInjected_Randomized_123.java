...
public class TestRestoreFlushSnapshotFromClient_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRestoreFlushSnapshotFromClient_RestartInjected.class);
    ...
    public void setup() throws Exception {
        ...
        RestartFramework.at("after_second_snapshot").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // create Table and disable it
        createTable();
        SnapshotTestingUtils.loadData(UTIL, tableName, 500, FAMILY);
        RestartFramework.at("after_first_data_load").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // insert more data
        SnapshotTestingUtils.loadData(UTIL, tableName, 500, FAMILY);
        RestartFramework.at("after_second_data_load").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_snapshot").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRestoreSnapshot() throws IOException {
        ...
        RestartFramework.at("after_restore_snapshot0").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        admin.disableTable(tableName);
        RestartFramework.at("after_restore_snapshot1").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCloneSnapshot() throws IOException, InterruptedException {
        TableName clonedTableName = TableName.valueOf("clonedtb-" + EnvironmentEdgeManager.currentTime());
        testCloneSnapshot(clonedTableName, snapshotName0, snapshot0Rows);
        testCloneSnapshot(clonedTableName, snapshotName1, snapshot1Rows);
    }

    private void testCloneSnapshot(final TableName tableName, final byte[] snapshotName, int snapshotRows) throws IOException, InterruptedException {
        // create a new table from snapshot
        admin.cloneSnapshot(snapshotName, tableName);
        RestartFramework.at("after_clone_snapshot").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRestoreSnapshotOfCloned() throws IOException, InterruptedException {
        RestartFramework.at("after_first_clone").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TableName clonedTableName = TableName.valueOf("clonedtb-" + EnvironmentEdgeManager.currentTime());
        ...
        RestartFramework.at("after_snapshot_of_clone").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.cloneSnapshot(snapshotName2, clonedTableName);
        RestartFramework.at("after_second_clone").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyRowCount(UTIL, clonedTableName, snapshot0Rows);
        UTIL.deleteTable(clonedTableName);
    }
