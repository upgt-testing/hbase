...
public class TestReportOnlineRegionsRace_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReportOnlineRegionsRace_RestartInjected.class);
    ...
    public static void setUp() throws Exception {
        ...
        UTIL.getConfiguration().setInt(HConstants.REGION_SERVER_HIGH_PRIORITY_HANDLER_COUNT, HConstants.DEFAULT_REGION_SERVER_HIGH_PRIORITY_HANDLER_COUNT);
    ...
    public void testRace() throws Exception {
        ...
        RestartFramework.at("after_resume_rs_report").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_region").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        am = master.getAssignmentManager();
        // Refresh after master restart
        procExec = master.getMasterProcedureExecutor();
        RestartFramework.at("after_setup_latches").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_wait_for_open_region_proc").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Future<byte[]> future = am.moveAsync(new RegionPlan(region, rsn.getRegionLocation(), rsn.getRegionLocation()));
        RestartFramework.at("after_resume_report_state").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        am = master.getAssignmentManager();
        ...
        RestartFramework.at("after_get_lock_entry").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Refresh after master restart
        am = master.getAssignmentManager();
        RestartFramework.at("after_get_assignment_manager").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        procExec = master.getMasterProcedureExecutor();
        // resume the reportRegionStateTransition to finish the CloseRegionProcedure
        RESUME_REPORT_STATE.countDown();
        procExecLock = procExec.getProcExecutionLock();
        RestartFramework.at("after_test_write").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Refresh after master restart
        am = master.getAssignmentManager();
        RestartFramework.at("after_schedule_trsp").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        procExec = master.getMasterProcedureExecutor();
        ...
        // Refresh after master restart
        am = master.getAssignmentManager();
        RestartFramework.at("after_trsp_done").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        procExec = master.getMasterProcedureExecutor();
        RestartFramework.at("after_verify_opening_state").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        RestartFramework.at("after_finish_rs_report").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        am = master.getAssignmentManager();
        RestartFramework.at("after_get_trsp").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        procExec = master.getMasterProcedureExecutor();
        RestartFramework.at("after_arrive_rs_report").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_release_lock_entry").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
