...
public class TestScanEarlyTermination_RestartInjected_Randomized_456 extends SecureTestUtil {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestScanEarlyTermination_RestartInjected.class);
    ...
    public void testEarlyScanTermination() throws Exception {
        ...
        grantOnTable(TEST_UTIL, USER_OTHER.getShortName(), testTable.getTableName(), TEST_FAMILY1, null, Action.READ);
        // Set up test data
        ...
        RestartFramework.at("after_put_data").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_grant").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_grant").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Now grant USER_OTHER access to TEST_FAMILY2:TEST_Q2
        grantOnTable(TEST_UTIL, USER_OTHER.getShortName(), testTable.getTableName(), TEST_FAMILY2, TEST_Q2, Action.READ);
