...
public class TestMasterBalanceThrottling_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterBalanceThrottling_RestartInjected.class);
    ...
    public void testThrottlingByBalanceInterval() throws Exception {
        ...
        RestartFramework.at("after_unbalance").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_balance").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        AtomicBoolean stop = new AtomicBoolean(false);
        Thread checker = startBalancerChecker(master, maxCount, stop);
        master.balance();
        stop.set(true);
        RestartFramework.at("before_balance").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        checker.interrupt();
        checker.join();
