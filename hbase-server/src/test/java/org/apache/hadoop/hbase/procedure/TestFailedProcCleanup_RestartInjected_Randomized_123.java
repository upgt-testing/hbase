...
public class TestFailedProcCleanup_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFailedProcCleanup_RestartInjected.class);
    ...
    public void testFailCreateTable() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_eviction_delay").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    ...
    public void testFailCreateTableAction() throws Exception {
        ...
        try {
            ...
            LOG.debug("Ignoring exception: ", e);
            RestartFramework.at("after_failed_table_create_action").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            Thread.sleep(evictionDelay * 3);
            RestartFramework.at("after_eviction_delay").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
