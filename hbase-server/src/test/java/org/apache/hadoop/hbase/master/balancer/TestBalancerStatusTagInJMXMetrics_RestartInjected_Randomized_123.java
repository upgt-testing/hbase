...
public class TestBalancerStatusTagInJMXMetrics_RestartInjected_Randomized_123 extends BalancerTestBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestBalancerStatusTagInJMXMetrics_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestBalancerStatusTagInJMXMetrics_RestartInjected.class);
    ...
    public void testJmxMetrics() throws Exception {
        RestartFramework.at("after_initial_status_check").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(getStatus(), "true");
        RestartFramework.at("after_balancer_update").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
