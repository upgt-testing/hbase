...
public class TestCloseRegionWhileRSCrash_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestCloseRegionWhileRSCrash_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCloseRegionWhileRSCrash_RestartInjected.class);
    ...
    public static void setUp() throws Exception {
        ...
        if (!srcRs.getRegions(TableName.META_TABLE_NAME).isEmpty()) {
            ...
            RestartFramework.at("after_meta_move").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testRetryBackoff() throws IOException, InterruptedException {
        HRegionServer srcRs = UTIL.getRSForFirstRegionInTable(TABLE_NAME);
        RestartFramework.at("after_move_thread_start").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_procedure_arrive").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_put_verification").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.waitFor(30000, () -> procExec.getProcedures().stream().anyMatch(p -> p instanceof ServerCrashProcedure));
        RestartFramework.at("after_procedure_submit").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_timeout_increase").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_procedure_waiting_timeout").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread t = new Thread(() -> {
            try {
        ...
        RestartFramework.at("after_new_master_start").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_master_connection_close").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
