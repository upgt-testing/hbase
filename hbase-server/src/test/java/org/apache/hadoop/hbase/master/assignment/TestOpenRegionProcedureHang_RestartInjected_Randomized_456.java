...
public class TestOpenRegionProcedureHang_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestOpenRegionProcedureHang_RestartInjected.class);
    ...
    private static final class AssignmentManagerForTest extends AssignmentManager {
        ...
        public ReportRegionStateTransitionResponse reportRegionStateTransition(ReportRegionStateTransitionRequest req) throws PleaseHoldException {
            ...
            if (transition.getTransitionCode() == TransitionCode.OPENED && ProtobufUtil.toTableName(transition.getRegionInfo(0).getTableName()).equals(NAME) && ARRIVE != null) {
                ARRIVE.countDown();
                try {
                    RESUME.await();
                    RESUME = null;
    ...
    public void test() throws InterruptedException, KeeperException, IOException {
        ...
        RestartFramework.at("after_arrive_latch").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_finish_await").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_wait_new_master").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        RestartFramework.at("after_wait_proc_finish").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        UTIL.waitFor(30000, () -> finalProcExec.getProcedures().stream().filter(p -> p instanceof OpenRegionProcedure).map(p -> (OpenRegionProcedure) p).anyMatch(p -> p.region.getTable().equals(NAME)));
        RestartFramework.at("after_set_latches").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_abort_countdown").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        ...
        Thread.sleep(2000);
        RestartFramework.at("after_resume_countdown").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RESUME.countDown();
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        RestartFramework.at("after_get_region_servers").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        ...
        // Refresh after master restart
        procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        RestartFramework.at("after_wait_open_procedure").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        ...
        master = UTIL.getMiniHBaseCluster().getMaster();
        RestartFramework.at("after_close_zk").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        ...
        RestartFramework.at("after_wait_parent_proc_finish").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
