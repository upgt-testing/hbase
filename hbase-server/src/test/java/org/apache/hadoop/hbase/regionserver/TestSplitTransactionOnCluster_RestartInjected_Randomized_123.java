...
public class TestSplitTransactionOnCluster_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSplitTransactionOnCluster_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSplitTransactionOnCluster_RestartInjected.class);
    ...
    public void testRITStateForRollback() throws Exception {
        ...
        try {
            ...
            t.close();
            RestartFramework.at("after_table_create_and_data_insert").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Turn off balancer so it doesn't cut in and mess up our placements.
            this.admin.balancerSwitch(false, true);
            // Turn off the meta scanner so it don't remove parent on us.
            master.setCatalogJanitorEnabled(false);
            RestartFramework.at("after_balancer_and_janitor_config").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // find a splittable region
            final HRegion region = findSplittableRegion(regions);
            assertTrue("not able to find a splittable region", region != null);
            RestartFramework.at("after_find_splittable_region").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_region_online").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
    ...
    public void testSplitFailedCompactionAndSplit() throws Exception {
        final TableName tableName = TableName.valueOf(name.getMethodName());
        RestartFramework.at("after_split").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Create table then get the single region for our new table.
        byte[] cf = Bytes.toBytes("cf");
        RestartFramework.at("after_data_insert").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_compaction_request").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_region_initialize").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSplitCompactWithPriority() throws Exception {
        ...
        RestartFramework.at("after_region_split").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse(compactionContext.get().getRequest().isAfterSplit());
        assertEquals(compactionContext.get().getRequest().getPriority(), 13);
        // Split
        long procId = cluster.getMaster().splitRegion(region.getRegionInfo(), Bytes.toBytes("row4"), 0, 0);
        // wait for the split to complete or get interrupted. If the split completes successfully,
        // the procedure will return true; if the split fails, the procedure would throw exception.
        ProcedureTestingUtility.waitProcedure(cluster.getMaster().getMasterProcedureExecutor(), procId);
        Thread.sleep(3000);
        ...
        compactionContext = hStore2.requestCompaction(Integer.MIN_VALUE + 10, CompactionLifeCycleTracker.DUMMY, null);
        assertTrue(compactionContext.isPresent());
    ...
    private void findRegionToSplit(TableName tableName, String splitRowKey) throws Exception {
        ...
        for (HRegion rg : cluster.getRegions(tableName)) {
            LOG.debug("startKey=" + Bytes.toStringBinary(rg.getRegionInfo().getStartKey()) + ", getEndKey()=" + Bytes.toStringBinary(rg.getRegionInfo().getEndKey()) + ", row=" + splitRowKey);
            if ((rg.getRegionInfo().getStartKey().length == 0 || CellComparator.getInstance().compare(PrivateCellUtil.createFirstOnRow(rg.getRegionInfo().getStartKey()), PrivateCellUtil.createFirstOnRow(toSplitKey)) <= 0) && (rg.getRegionInfo().getEndKey().length == 0 || CellComparator.getInstance().compare(PrivateCellUtil.createFirstOnRow(rg.getRegionInfo().getEndKey()), PrivateCellUtil.createFirstOnRow(toSplitKey)) >= 0)) {
                toSplit = rg;
            }
        }
    ...
    public void testStoreFileReferenceCreationWhenSplitPolicySaysToSkipRangeCheck() throws Exception {
        ...
        try {
            ...
            TableDescriptor htd = TableDescriptorBuilder.newBuilder(tableName).setColumnFamily(ColumnFamilyDescriptorBuilder.of(cf)).setColumnFamily(ColumnFamilyDescriptorBuilder.of(cf1)).setRegionSplitPolicyClassName(CustomSplitPolicy.class.getName()).build();
            admin.createTable(htd);
    ...
    private int ensureTableRegionNotOnSameServerAsMeta(final Admin admin, final RegionInfo hri) throws IOException, MasterNotRunningException, ZooKeeperConnectionException, InterruptedException {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (tableRegionIndex != -1 && tableRegionIndex != metaServerIndex)
                break;
    ...
    private HRegionServer getOtherRegionServer(final MiniHBaseCluster cluster, final HRegionServer notThisOne) {
        for (RegionServerThread rst : cluster.getRegionServerThreads()) {
            ...
            if (hrs.getServerName().equals(notThisOne.getServerName()))
                continue;
