...
public class TestRegionMover3_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionMover3_RestartInjected.class);
    ...
    public void testRegionUnloadWithRack() throws Exception {
        ...
        RestartFramework.at("after_region_move").on(cluster).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        try (RegionMover regionMover = rmBuilder.build()) {
            ...
            RestartFramework.at("after_unload_from_rack").on(cluster).restart("regionserver").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            int newNumRegions0 = hRegionServer0.getNumberOfOnlineRegions();
            int newNumRegions1 = hRegionServer1.getNumberOfOnlineRegions();
        ...
        try (RegionMover regionMover = rmBuilder.build()) {
            regionMover.unloadFromRack();
            RestartFramework.at("after_unload_with_custom_rack").on(cluster).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
            int newNumRegions0 = hRegionServer0.getNumberOfOnlineRegions();
            int newNumRegions1 = hRegionServer1.getNumberOfOnlineRegions();
            int newNumRegions2 = hRegionServer2.getNumberOfOnlineRegions();
