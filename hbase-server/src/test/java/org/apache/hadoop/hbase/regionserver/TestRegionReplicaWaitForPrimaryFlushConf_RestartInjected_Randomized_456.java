...
public class TestRegionReplicaWaitForPrimaryFlushConf_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionReplicaWaitForPrimaryFlushConf_RestartInjected.class);
    ...
    public void testSecondaryReplicaReadEnabled() throws Exception {
        ...
        TableDescriptor tableDescriptor = TableDescriptorBuilder.newBuilder(tableName).setRegionReplication(2).setColumnFamily(ColumnFamilyDescriptorBuilder.of(FAMILY)).setRegionMemStoreReplication(true).build();
        ...
        final ArrayList<Pair<HRegion, HRegionServer>> regionAndRegionServers = new ArrayList<Pair<HRegion, HRegionServer>>(Arrays.asList(null, null));
        RestartFramework.at("after_region_collection").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 2; i++) {
        ...
        RestartFramework.at("after_read_enabled_verification").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HRegionServer secondaryRs = regionAndRegionServers.get(1).getSecond();
        RestartFramework.at("after_executor_verification").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
        ...
        assertFalse(ServerRegionReplicaUtil.isRegionReplicaWaitForPrimaryFlushEnabled(secondaryRegion.conf));
