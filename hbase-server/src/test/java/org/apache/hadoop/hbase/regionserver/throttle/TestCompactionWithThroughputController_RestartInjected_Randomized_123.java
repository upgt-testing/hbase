...
public class TestCompactionWithThroughputController_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCompactionWithThroughputController_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestCompactionWithThroughputController_RestartInjected.class);
    ...
    private HStore prepareData() throws IOException {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (i == 4 || i == 9) {
                RestartFramework.at("after_flush_" + i).on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    ...
    private long testCompactionWithThroughputLimit() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_prepare_data").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(10, store.getStorefilesCount());
            long startTime = EnvironmentEdgeManager.currentTime();
            TEST_UTIL.getAdmin().majorCompact(tableName);
            RestartFramework.at("after_major_compact").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private long testCompactionWithoutThroughputLimit() throws Exception {
        ...
        RestartFramework.at("after_cluster_start_no_limit").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TEST_UTIL.startMiniCluster(1);
        cluster = TEST_UTIL.getMiniHBaseCluster();
        try {
            HStore store = prepareData();
            RestartFramework.at("after_prepare_data_no_limit").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(10, store.getStorefilesCount());
            long startTime = EnvironmentEdgeManager.currentTime();
            TEST_UTIL.getAdmin().majorCompact(tableName);
            RestartFramework.at("after_major_compact_no_limit").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            while (store.getStorefilesCount() != 1) {
                Thread.sleep(20);
            }
            return EnvironmentEdgeManager.currentTime() - startTime;
    ...
    public void testThroughputTuning() throws Exception {
        ...
        try {
            TEST_UTIL.getAdmin().createTable(TableDescriptorBuilder.newBuilder(tableName).setColumnFamily(ColumnFamilyDescriptorBuilder.of(family)).setCompactionEnabled(false).build());
            TEST_UTIL.waitTableAvailable(tableName);
            RestartFramework.at("after_table_create").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            HRegionServer regionServer = TEST_UTIL.getRSForFirstRegionInTable(tableName);
            PressureAwareCompactionThroughputController throughputController = (PressureAwareCompactionThroughputController) regionServer.getCompactSplitThread().getCompactionThroughputController();
            ...
            RestartFramework.at("after_first_flush_cycle").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            TEST_UTIL.flush(tableName);
            RestartFramework.at("after_second_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            regionServer = TEST_UTIL.getRSForFirstRegionInTable(tableName);
            Thread.sleep(2000);
            ...
            TEST_UTIL.flush(tableName);
            RestartFramework.at("after_third_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            regionServer = TEST_UTIL.getRSForFirstRegionInTable(tableName);
            Thread.sleep(2000);
            ...
            assertTrue(regionServer.getCompactSplitThread().getCompactionThroughputController() instanceof NoLimitThroughputController);
        } finally {
            conn.close();
            TEST_UTIL.shutdownMiniCluster();
        }
    ...
    public void testGetCompactionPressureForStripedStore() throws Exception {
        ...
        try {
            ...
            TEST_UTIL.flush(tableName);
            RestartFramework.at("after_fourth_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getMiniHBaseCluster();
