...
public class TestRestartWithEmptyWALDirectory_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRestartWithEmptyWALDirectory_RestartInjected.class);
    ...
    public void testRestart() throws IOException, InterruptedException {
        ...
        RestartFramework.at("after_put").on(UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.getMiniHBaseCluster().stopRegionServer(0).join();
        // let's cleanup the WAL directory
        UTIL.getTestFileSystem().delete(new Path(CommonFSUtils.getWALRootDir(UTIL.getConfiguration()), HConstants.HREGION_LOGDIR_NAME), true);
        // restart the cluster
        UTIL.getMiniHBaseCluster().startMaster();
        UTIL.getMiniHBaseCluster().startRegionServer();
        UTIL.waitTableAvailable(NAME);
        RestartFramework.at("after_table_available").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
