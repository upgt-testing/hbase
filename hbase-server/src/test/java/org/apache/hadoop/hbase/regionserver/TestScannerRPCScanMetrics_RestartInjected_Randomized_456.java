...
public class TestScannerRPCScanMetrics_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestScannerRPCScanMetrics_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestScannerRPCScanMetrics_RestartInjected.class);
    ...
    public void testScannerRPCScanMetrics() throws Exception {
        ...
        RestartFramework.at("after_scan1").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_metric_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // This scan should increment rpc full scan count by 2 (both regions - no stop/start row)
        scanNextIterate(ht, dummyScan);
        RestartFramework.at("after_scan3").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RSRpcServices testClusterRSRPCServices = TEST_UTIL.getMiniHBaseCluster().getRegionServer(0).rpcServices;
