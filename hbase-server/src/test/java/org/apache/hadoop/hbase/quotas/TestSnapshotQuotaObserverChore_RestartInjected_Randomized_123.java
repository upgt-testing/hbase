...
public class TestSnapshotQuotaObserverChore_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSnapshotQuotaObserverChore_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSnapshotQuotaObserverChore_RestartInjected.class);
    ...
    public void testSnapshotsFromTables() throws Exception {
        ...
        RestartFramework.at("after_snapshot_batch2").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_snapshot_batch1").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSnapshotSize() throws Exception {
        ...
        admin.setQuota(QuotaSettingsFactory.limitTableSpace(tn1, SpaceQuotaHelperForTests.ONE_GIGABYTE, SpaceViolationPolicy.NO_INSERTS));
        RestartFramework.at("after_write_flush_compact").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_table_create_quota").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Write some more data, flush it, and then major_compact the table
        helper.writeData(tn1, 256L * SpaceQuotaHelperForTests.ONE_KILOBYTE);
        admin.flush(tn1);
        RestartFramework.at("after_write_flush").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPersistingSnapshotsForNamespaces() throws Exception {
        RestartFramework.at("after_setup").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRemovedSnapshots() throws Exception {
        // Create a table and set a quota
        TableName tn1 = helper.createTableWithRegions(1);
        RestartFramework.at("after_snapshot_delete").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.setQuota(QuotaSettingsFactory.limitTableSpace(tn1, SpaceQuotaHelperForTests.ONE_GIGABYTE, SpaceViolationPolicy.NO_INSERTS));
        // Write some data and flush it
        // 256 KB
        helper.writeData(tn1, 256L * SpaceQuotaHelperForTests.ONE_KILOBYTE);
        ...
        RestartFramework.at("after_write").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBucketingFilesToSnapshots() throws Exception {
        ...
        RestartFramework.at("after_snapshot1_compact").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Major compact the table to force a rewrite
        TEST_UTIL.compact(tn1, true);
        RestartFramework.at("after_snapshot2_compact").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Make sure that the snapshot owns the size
        TEST_UTIL.waitFor(30_000, new Predicate<Exception>() {
            ...
            public boolean evaluate() throws Exception {
                ...
                r.advance();
                Cell c = r.current();
                return closeInSize(lastSeenSize.get(), QuotaTableUtil.parseSnapshotSize(c), SpaceQuotaHelperForTests.ONE_KILOBYTE);
            }
        });
    ...
    void debugFilesForSnapshot(TableName table, String snapshot) throws IOException {
        ...
        SnapshotReferenceUtil.visitReferencedFiles(conf, fs, new Path(snapshotDir, snapshot), new SnapshotVisitor() {

            @Override
            public void storeFile(RegionInfo regionInfo, String familyName, StoreFile storeFile) throws IOException {
                LOG.info("Snapshot={} references file={}, size={}", snapshot, storeFile.getName(), storeFile.getFileSize());
            }
        });
    }
