...
public class TestMasterProcedureEvents_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterProcedureEvents_RestartInjected.class);
    ...
    public void testMasterInitializedEvent() throws Exception {
        ...
        RestartFramework.at("after_set_initialized_false").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testProcedureEventWaitWake(final HMaster master, final ProcedureEvent<?> event, final Procedure<MasterProcedureEnv> proc) throws Exception {
        RestartFramework.at("after_event_woken").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_procedure_submitted").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(false, event.isReady());
        RestartFramework.at("after_procedure_suspended").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_procedure_submit").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(false, event.isReady());
        ...
        assertEquals(true, event.isReady());
        RestartFramework.at("after_procedure_completed").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(0, event.getSuspendedProcedures().size());
