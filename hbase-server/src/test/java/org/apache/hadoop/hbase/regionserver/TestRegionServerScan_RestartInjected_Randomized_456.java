...
public class TestRegionServerScan_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionServerScan_RestartInjected.class);
    ...
    public void testScannWhenRpcCallContextNull() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_table_create").on(TEST_UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            putToTable(table, r0);
            putToTable(table, r1);
            putToTable(table, r2);
            RestartFramework.at("after_data_insert").on(TEST_UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_scan_setup").on(TEST_UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            MyRSRpcServices.inTest = true;
            resultScanner = table.getScanner(scan);
            Result result = resultScanner.next();
            byte[] rowKey = result.getRow();
            assertTrue(Bytes.equals(r0, rowKey));
            RestartFramework.at("after_first_scan_result").on(TEST_UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            result = resultScanner.next();
            rowKey = result.getRow();
            assertTrue(Bytes.equals(r1, rowKey));
            RestartFramework.at("after_second_scan_result").on(TEST_UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            result = resultScanner.next();
            rowKey = result.getRow();
            assertTrue(Bytes.equals(r2, rowKey));
            assertNull(resultScanner.next());
            assertTrue(MyRSRpcServices.exceptionRef.get() == null);
            RestartFramework.at("after_scan_complete").on(TEST_UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
