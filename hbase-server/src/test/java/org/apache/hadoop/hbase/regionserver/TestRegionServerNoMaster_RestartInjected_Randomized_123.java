...
public class TestRegionServerNoMaster_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionServerNoMaster_RestartInjected.class);
    ...
    public static void before() throws Exception {
        ...
        RestartFramework.at("after_put_data").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        stopMasterAndCacheMetaLocation(HTU);
    }

    public static void stopMasterAndCacheMetaLocation(HBaseTestingUtility HTU) throws IOException, InterruptedException {
        ...
        for (JVMClusterUtil.RegionServerThread t : HTU.getMiniHBaseCluster().getRegionServerThreads()) {
            try (RegionLocator locator = t.getRegionServer().getConnection().getRegionLocator(TableName.META_TABLE_NAME)) {
    ...
    public void testCloseByRegionServer() throws Exception {
        closeRegionNoZK();
        RestartFramework.at("after_close_region").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        openRegion(HTU, getRS(), hri);
        RestartFramework.at("after_open_region").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void testMultipleCloseFromMaster() throws Exception {
        RestartFramework.at("after_open_region_multi").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 10; i++) {
        ...
        RestartFramework.at("after_multiple_close").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        checkRegionIsClosed(HTU, getRS(), hri);
        openRegion(HTU, getRS(), hri);
    }
    ...
    public void testCancelOpeningWithoutZK() throws Exception {
        ...
        RestartFramework.at("after_close_attempt").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_open_handler_submit").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_open_region_cancel").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_rit_put").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // The open handler should have removed the region from RIT but kept the region closed
        checkRegionIsClosed(HTU, getRS(), hri);
        RestartFramework.at("after_close_region_cancel").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        openRegion(HTU, getRS(), hri);
    }
    ...
    public void testOpenCloseRegionRPCIntendedForPreviousServer() throws Exception {
        ...
        RestartFramework.at("after_close_rejection").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_region_available_check").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
        ...
        try {
            ...
            RestartFramework.at("after_open_rejection").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            openRegion(HTU, getRS(), hri);
        }
    }
    ...
    public void testInstallShutdownHook() {
        RestartFramework.at("after_shutdown_hook_check").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
