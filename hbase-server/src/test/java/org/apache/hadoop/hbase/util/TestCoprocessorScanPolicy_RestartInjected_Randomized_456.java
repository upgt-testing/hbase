...
public class TestCoprocessorScanPolicy_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCoprocessorScanPolicy_RestartInjected.class);
    ...
    public TestCoprocessorScanPolicy_RestartInjected_Randomized_456(boolean parallelSeekEnable) {
    ...
    public void testBaseCases() throws Exception {
        ...
        Put p = new Put(R);
        RestartFramework.at("after_first_compact").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        p.addColumn(F, Q, now, Q);
        t.put(p);
        p = new Put(R);
        p.addColumn(F, Q, now + 1, Q);
        t.put(p);
        RestartFramework.at("after_version_override").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        p = new Put(R);
        p.addColumn(F, Q, now + 2, Q);
        t.put(p);
        Get g = new Get(R);
        RestartFramework.at("after_initial_puts").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        g.readVersions(10);
        Result r = t.get(g);
        ...
        t.put(p);
        RestartFramework.at("after_second_compact").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // only 2 versions now
        r = t.get(g);
    ...
    public void testTTL() throws Exception {
        ...
        RestartFramework.at("after_ttl_override").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        me.setValue(now);
