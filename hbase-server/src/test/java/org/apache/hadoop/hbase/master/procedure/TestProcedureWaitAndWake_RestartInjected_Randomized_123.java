...
public class TestProcedureWaitAndWake_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestProcedureWaitAndWake_RestartInjected.class);
    ...
    public static final class MyPeerProcedure extends AbstractPeerProcedure<Integer> {
        ...
        @Override
        protected Flow executeFromState(MasterProcedureEnv env, Integer state) throws ProcedureSuspendedException, ProcedureYieldException, InterruptedException {
            if (state.intValue() == 0) {
                setNextState(1);
        ...
        public static final class MySubPeerProcedure extends StateMachineProcedure<MasterProcedureEnv, Integer> implements PeerProcedureInterface {
    ...
    public void testPeerProcedure() {
        RestartFramework.at("after_create_procedures").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureExecutor<MasterProcedureEnv> procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        RestartFramework.at("after_create_barrier").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_wait_second_procedure_finish").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        CyclicBarrier barrier = new CyclicBarrier(2);
        RestartFramework.at("after_wait_first_procedure_finish").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_submit_second_procedure").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        long id2 = procExec.submitProcedure(p2);
        UTIL.waitFor(10000, () -> procExec.isFinished(id1));
        RestartFramework.at("after_submit_first_procedure").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
