...
public class TestIOFencing_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestIOFencing_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestIOFencing_RestartInjected.class);
    ...
    public void doTest(Class<?> regionClass, MemoryCompactionPolicy policy) throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_first_data_load").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            while (compactingRegion.getEarliestFlushTimeForAllStores() <= lastFlushTime || compactingRegion.countStoreFiles() <= 1) {
                ...
                assertTrue("Timed out waiting for the region to flush", EnvironmentEdgeManager.currentTime() - startWaitTime < 30000);
            }
            ...
            RestartFramework.at("after_second_data_load").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin.majorCompact(TABLE_NAME);
            startWaitTime = EnvironmentEdgeManager.currentTime();
            while (newRegion.compactCount.get() == 0) {
                Thread.sleep(1000);
                assertTrue("New region never compacted", EnvironmentEdgeManager.currentTime() - startWaitTime < 180000);
            }
            int count;
            for (int i = 0; ; i++) {
                try {
