...
public class TestMasterRegistry_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestMasterRegistry_RestartInjected.class);
  ...
  public void testMasterAddressParsing() throws Exception {
    ...
    RestartFramework.at("after_master_address_parsing")
  ...
  public void testMasterPortDefaults() throws Exception {
    ...
    assertEquals(HConstants.DEFAULT_MASTER_PORT, sn.getPort());
    RestartFramework.at("after_port_defaults_check")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testRegistryRPCs() throws Exception {
    ...
    for (int numHedgedReqs = 1; numHedgedReqs <= size; numHedgedReqs++) {
      ...
      try (MasterRegistry registry = new MasterRegistry(conf, User.getCurrent())) {
        RestartFramework.at("after_registry_rpc_setup")
            .on(TEST_UTIL.getMiniHBaseCluster())
            .restart("master")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
  ...
  public void testDynamicMasterConfigurationRefresh() throws Exception {
    ...
    try (MasterRegistry registry = new MasterRegistry(conf, User.getCurrent())) {
      RestartFramework.at("after_registry_setup")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_cluster_id_check")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("master")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_masters_refresh")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_active_master_kill")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_master_deregister")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("master")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
