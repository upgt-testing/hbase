...
public class TestCompactionWithThroughputController_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestCompactionWithThroughputController_RestartInjected.class);

  private static final Logger LOG =
    LoggerFactory.getLogger(TestCompactionWithThroughputController_RestartInjected.class);
  ...
  private HStore prepareData() throws IOException {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (i == 4 || i == 9) {
        RestartFramework.at("after_flush_" + i)
            .on(cluster)
            .restart("regionserver")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
  ...
  private long testCompactionWithThroughputLimit() throws Exception {
    ...
    try {
      ...
      RestartFramework.at("after_prepare_data")
          .on(cluster)
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private long testCompactionWithoutThroughputLimit() throws Exception {
    ...
    TEST_UTIL.startMiniCluster(1);
    cluster = TEST_UTIL.getMiniHBaseCluster();
    RestartFramework.at("after_cluster_start_no_limit")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    try {
      HStore store = prepareData();
      RestartFramework.at("after_prepare_data_no_limit")
          .on(cluster)
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      long startTime = EnvironmentEdgeManager.currentTime();
      TEST_UTIL.getAdmin().majorCompact(tableName);
      RestartFramework.at("after_major_compact_no_limit")
          .on(cluster)
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testThroughputTuning() throws Exception {
    ...
    try {
      ...
      RestartFramework.at("after_first_flush_cycle")
          .on(cluster)
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      TEST_UTIL.flush(tableName);
      RestartFramework.at("after_third_flush")
          .on(cluster)
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testGetCompactionPressureForStripedStore() throws Exception {
    ...
    try {
      ...
      TEST_UTIL.flush(tableName);
      RestartFramework.at("after_fourth_flush")
          .on(cluster)
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
