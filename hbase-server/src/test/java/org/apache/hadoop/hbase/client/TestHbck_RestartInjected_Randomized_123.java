...
public class TestHbck_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestHbck_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestHbck_RestartInjected.class);
    ...
    private static final TableName TABLE_NAME = TableName.valueOf(TestHbck_RestartInjected.class.getSimpleName());
    ...
    public void testBypassProcedure() throws Exception {
        RestartFramework.at("after_submit_procedure").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // SuspendProcedure
        final SuspendProcedure proc = new SuspendProcedure();
        long procId = procExec.submitProcedure(proc);
        RestartFramework.at("after_bypass_procedure").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(500);
    ...
    public void testSetTableStateInMeta() throws Exception {
        ...
        RestartFramework.at("after_set_table_state_enabled").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_set_table_state_disabled").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue("Incorrect previous state! expected=DISABLED, found=" + prevState.getState(), prevState.isDisabled());
    }
    ...
    public void testSetRegionStateInMeta() throws Exception {
        ...
        RestartFramework.at("after_restore_region_state").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        final Callable<Void> doTest = () -> {
            ...
            regions.forEach(r -> {
                ...
                assertEquals("state in AM should match requested state ; " + r, expectedState, afterState.getState());
            });
            return null;
        };
    ...
    public void testAssigns() throws Exception {
        ...
        try (Admin admin = TEST_UTIL.getConnection().getAdmin()) {
            List<RegionInfo> regions = admin.getRegions(TABLE_NAME).stream().filter(ri -> ri.getReplicaId() == RegionInfo.DEFAULT_REPLICA_ID).peek(ri -> {
                final RegionState rs = am.getRegionStates().getRegionState(ri.getEncodedName());
                LOG.info("RS: {}", rs);
            }).collect(Collectors.toList());
            List<Long> pids = hbck.unassigns(regions.stream().map(RegionInfo::getEncodedName).collect(Collectors.toList()));
            waitOnPids(pids);
            RestartFramework.at("after_first_unassigns").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            waitOnPids(pids);
            RestartFramework.at("after_assigns").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testScheduleSCP() throws Exception {
        ...
        RestartFramework.at("after_schedule_scp").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_load_table").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRunHbckChore() throws Exception {
        ...
        RestartFramework.at("after_run_hbck_chore").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class FailingMergeAfterMetaUpdatedMasterObserver implements MasterCoprocessor, MasterObserver {
        ...
        @Override
        public void postMergeRegionsCommitAction(final ObserverContext<MasterCoprocessorEnvironment> ctx, final RegionInfo[] regionsToMerge, final RegionInfo mergedRegion) throws IOException {
            latch.countDown();
            throw new IOException("this procedure will fail at here forever");
        }
    }
