...
public abstract class SnapshotWithAclTestBase_RestartInjected_Randomized_123 extends SecureTestUtil {
    ...
    public static void setupBeforeClass() throws Exception {
        ...
        TEST_UTIL.getConfiguration().set(HConstants.SNAPSHOT_RESTORE_FAILSAFE_NAME, "hbase-failsafe-{snapshot.name}-{restore.timestamp}");
    ...
    public void setUp() throws Exception {
        TEST_UTIL.createTable(TableDescriptorBuilder.newBuilder(TEST_TABLE).setColumnFamily(ColumnFamilyDescriptorBuilder.newBuilder(TEST_FAMILY).setMaxVersions(100).build()).setOwner(USER_OWNER).build(), new byte[][] { Bytes.toBytes("s") });
    ...
    protected abstract void cloneSnapshot(String snapshotName, TableName tableName, boolean restoreAcl) throws Exception;

    protected abstract void restoreSnapshot(String snapshotName, boolean takeFailSafeSnapshot, boolean restoreAcl) throws Exception;
    ...
    public void testRestoreSnapshot() throws Exception {
        ...
        RestartFramework.at("after_clone_snapshot").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_load_data").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyAllowed(new AccessReadAction(TEST_TABLE), USER_OWNER, USER_RW);
        RestartFramework.at("after_clone_snapshot").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_restore_snapshot").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyDenied(new AccessReadAction(TEST_TABLE), USER_RO, USER_NONE);
        verifyAllowed(new AccessWriteAction(TEST_TABLE), USER_OWNER, USER_RW);
        verifyDenied(new AccessWriteAction(TEST_TABLE), USER_RO, USER_NONE);
        ...
        RestartFramework.at("after_restore_snapshot").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TEST_UTIL.getAdmin().enableTable(TEST_TABLE);
        verifyAllowed(new AccessReadAction(TEST_TABLE), USER_OWNER, USER_RO, USER_RW);
        verifyDenied(new AccessReadAction(TEST_TABLE), USER_NONE);
        verifyAllowed(new AccessWriteAction(TEST_TABLE), USER_OWNER, USER_RW);
        ...
        int failsafeSnapshotCount = TEST_UTIL.getAdmin().listSnapshots(Pattern.compile("hbase-failsafe-" + snapshotName1 + ".*")).size();
        ...
        // restoring.
        failsafeSnapshotCount = TEST_UTIL.getAdmin().listSnapshots(Pattern.compile("hbase-failsafe-" + snapshotName1 + ".*")).size();
        assertEquals(0, failsafeSnapshotCount);
    }
    ...
    public void testDeleteSnapshot() throws Exception {
        ...
        List<SnapshotDescription> snapshotDescriptions = TEST_UTIL.getAdmin().listSnapshots(Pattern.compile(testSnapshotName));
        assertEquals(1, snapshotDescriptions.size());
        RestartFramework.at("after_snapshot_create").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        List<SnapshotDescription> snapshotsAfterDelete = TEST_UTIL.getAdmin().listSnapshots(Pattern.compile(testSnapshotName));
