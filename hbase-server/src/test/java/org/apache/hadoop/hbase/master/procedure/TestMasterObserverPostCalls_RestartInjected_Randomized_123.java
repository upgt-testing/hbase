...
public class TestMasterObserverPostCalls_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterObserverPostCalls_RestartInjected.class);
    ...
    public static class MasterObserverForTest implements MasterCoprocessor, MasterObserver {
        ...
        @Override
        public void postModifyNamespace(ObserverContext<MasterCoprocessorEnvironment> ctx, NamespaceDescriptor desc) {
            postHookCalls.incrementAndGet();
        }
        ...
        @Override
        public void postModifyTable(ObserverContext<MasterCoprocessorEnvironment> ctx, TableName tn, TableDescriptor td) {
            postHookCalls.incrementAndGet();
        }
    ...
    public void testPostDeleteNamespace() throws IOException {
        ...
        RestartFramework.at("after_disable_table_delete").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_namespace_delete").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final TableName tn1 = TableName.valueOf(ns, "table1");
        admin.createNamespace(NamespaceDescriptor.create(ns).build());
        RestartFramework.at("after_create_table_delete").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_observer_delete").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Refresh after master restart
        observer = master.getMasterCoprocessorHost().findCoprocessor(MasterObserverForTest.class);
        try {
        ...
        RestartFramework.at("after_delete_table_delete").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPostModifyNamespace() throws IOException {
        ...
        RestartFramework.at("after_modify_namespace").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Refresh after master restart
        observer = master.getMasterCoprocessorHost().findCoprocessor(MasterObserverForTest.class);
        RestartFramework.at("after_get_observer_modify").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
        ...
        postCount = observer.postHookCalls.get();
        RestartFramework.at("after_create_namespace_modify").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPostCreateNamespace() throws IOException {
        RestartFramework.at("after_create_namespace").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Admin admin = UTIL.getAdmin();
        ...
        int postCount = observer.postHookCalls.get();
        assertEquals("Expected 1 invocation of postModifyNamespace", preCount + 1, postCount);
        RestartFramework.at("after_get_observer_create_ns").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPostCreateTable() throws IOException {
        ...
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        RestartFramework.at("after_get_observer_create_table").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        observer = master.getMasterCoprocessorHost().findCoprocessor(MasterObserverForTest.class);
        // Validate that the post hook is called
        int preCount = observer.postHookCalls.get();
        RestartFramework.at("after_create_table").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPostModifyTable() throws IOException {
        ...
        RestartFramework.at("after_modify_table").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        observer = master.getMasterCoprocessorHost().findCoprocessor(MasterObserverForTest.class);
    ...
    public void testPostDisableTable() throws IOException {
        ...
        // Refresh after master restart
        observer = master.getMasterCoprocessorHost().findCoprocessor(MasterObserverForTest.class);
        RestartFramework.at("after_create_table_disable").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Validate that the post hook is called
        int preCount = observer.postHookCalls.get();
    ...
    public void testPostDeleteTable() throws IOException {
        ...
        admin.createTable(td);
        admin = UTIL.getAdmin();
        RestartFramework.at("after_delete_table_test").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = UTIL.getMiniHBaseCluster().getMaster();
        // Refresh after master restart
        observer = master.getMasterCoprocessorHost().findCoprocessor(MasterObserverForTest.class);
        ...
        observer = master.getMasterCoprocessorHost().findCoprocessor(MasterObserverForTest.class);
        int postCount = observer.postHookCalls.get();
        RestartFramework.at("after_create_table_delete_test").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals("Expected 1 invocation of postDeleteTable", preCount + 1, postCount);
        RestartFramework.at("after_disable_table_delete_test").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Then, validate that it's not called when the call fails
        preCount = observer.postHookCalls.get();
        try {
