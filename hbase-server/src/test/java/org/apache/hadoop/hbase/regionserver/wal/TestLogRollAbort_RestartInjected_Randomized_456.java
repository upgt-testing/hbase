...
public class TestLogRollAbort_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestLogRollAbort_RestartInjected.class);
    ...
    public void testRSAbortWithUnflushedEdits() throws Exception {
        ...
        try {
            ...
            table.put(p);
            RestartFramework.at("after_first_put").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            server = TEST_UTIL.getRSForFirstRegionInTable(tableName);
            log = server.getWAL(null);
            log.sync();
            RestartFramework.at("after_wal_sync").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            server = TEST_UTIL.getRSForFirstRegionInTable(tableName);
            log = server.getWAL(null);
            p = new Put(Bytes.toBytes("row2002"));
            p.addColumn(HConstants.CATALOG_FAMILY, Bytes.toBytes("col"), Bytes.toBytes(2002));
            table.put(p);
            RestartFramework.at("after_second_put").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            server = TEST_UTIL.getRSForFirstRegionInTable(tableName);
            log = server.getWAL(null);
            dfsCluster.restartDataNodes();
            LOG.info("Restarted datanodes");
            RestartFramework.at("after_datanode_restart").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            server = TEST_UTIL.getRSForFirstRegionInTable(tableName);
            log = server.getWAL(null);
            try {
                log.rollWriter(true);
    ...
    public void testLogRollAfterSplitStart() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_wal_create").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            int total = 20;
            ...
            RestartFramework.at("after_wal_append").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Send the data to HDFS datanodes and close the HDFS writer
            log.sync();
            RestartFramework.at("after_wal_sync_split").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ((AbstractFSWAL<?>) log).replaceWriter(((FSHLog) log).getOldPath(), null, null);
            RestartFramework.at("after_writer_replace").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_dir_rename").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LOG.debug("Processing the old log files.");
            WALSplitter.split(HBASELOGDIR, rsSplitDir, OLDLOGDIR, fs, conf, wals);
            RestartFramework.at("after_wal_split").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
