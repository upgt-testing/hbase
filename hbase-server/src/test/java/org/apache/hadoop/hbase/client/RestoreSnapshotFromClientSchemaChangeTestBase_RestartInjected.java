...
public class RestoreSnapshotFromClientSchemaChangeTestBase_RestartInjected
  ...
  public void testRestoreSchemaChange() throws Exception {
    ...
    RestartFramework.at("after_first_restore")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
        .execute();
    admin.enableTable(tableName);
    RestartFramework.at("after_first_enable")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_second_restore")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
        .execute();
    admin.enableTable(tableName);
    RestartFramework.at("after_second_enable")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
