...
public class TestRegionReplicaFailover_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionReplicaFailover_RestartInjected.class);
    ...
    public void testSecondaryRegionWithEmptyRegion() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(HTU.getConfiguration());
            ...
            RestartFramework.at("after_replica_ready").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Get get = new Get(row);
    ...
    public void testSecondaryRegionWithNonEmptyRegion() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(HTU.getConfiguration());
            ...
            RestartFramework.at("after_disable_table").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            HTU.getAdmin().enableTable(htd.getTableName());
            RestartFramework.at("after_enable_table").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPrimaryRegionKill() throws Exception {
        try (Connection connection = ConnectionFactory.createConnection(HTU.getConfiguration());
            ...
            RestartFramework.at("after_replication_verify").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertTrue(aborted);
            RestartFramework.at("after_primary_abort").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // wal replication is async, we have to wait until the replication catches up, or we timeout
            verifyNumericRowsWithTimeout(table, fam, 0, 1000, 0, 30000);
            verifyNumericRowsWithTimeout(table, fam, 0, 1000, 1, 30000);
            verifyNumericRowsWithTimeout(table, fam, 0, 1000, 2, 30000);
            RestartFramework.at("after_recovery_verify").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
     ...
     * wal replication is async, we have to wait until the replication catches up, or we timeout
     */
    private void verifyNumericRowsWithTimeout(final Table table, final byte[] f, final int startRow, final int endRow, final int replicaId, final long timeout) throws Exception {
        try {
    ...
    public void testSecondaryRegionKill() throws Exception {
        try (Connection connection = ConnectionFactory.createConnection(HTU.getConfiguration());
            ...
            assertTrue(aborted);
            RestartFramework.at("after_secondary_abort").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_recovery_verify").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        }
        // restart the region server
        HTU.getMiniHBaseCluster().startRegionServer();
    }
    ...
    public void testSecondaryRegionKillWhilePrimaryIsAcceptingWrites() throws Exception {
        try (Connection connection = ConnectionFactory.createConnection(HTU.getConfiguration());
            ...
            HTU.loadNumericRows(table, fam, 0, 1000);
            RestartFramework.at("after_initial_load").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin.flush(table.getName());
            RestartFramework.at("after_flush").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            HTU.loadNumericRows(table, fam, 1000, 2000);
            RestartFramework.at("after_second_load").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            loader.join();
            assertNull(ex.get());
            RestartFramework.at("after_concurrent_abort").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_final_verify").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        // restart the region server
        HTU.getMiniHBaseCluster().startRegionServer();
    }
    ...
    public void testLotsOfRegionReplicas() throws IOException {
        ...
        try (Connection connection = ConnectionFactory.createConnection(HTU.getConfiguration());
            ...
            RestartFramework.at("after_get_operations").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
