...
public class TestShutdownWhileWALBroken_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestShutdownWhileWALBroken_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestShutdownWhileWALBroken_RestartInjected.class);
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_table_create").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try (Table table = UTIL.getConnection().getTable(TABLE_NAME)) {
            UTIL.loadTable(table, CF);
        }
        RestartFramework.at("after_table_load").on(UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_region_count").on(UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_session_expire").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_rs_identification").on(UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.expireSession(toKillRSThread.getRegionServer().getZooKeeper(), false);
        RestartFramework.at("after_failover_complete").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
