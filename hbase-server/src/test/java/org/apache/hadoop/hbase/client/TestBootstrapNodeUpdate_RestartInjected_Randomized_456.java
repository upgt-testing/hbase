...
public class TestBootstrapNodeUpdate_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestBootstrapNodeUpdate_RestartInjected.class);
    ...
    public void testUpdate() throws Exception {
        ...
        ServerName masterInConf = ServerName.valueOf(activeMasterServerName.getHostname(), activeMasterServerName.getPort(), -1);
        RestartFramework.at("after_master_bootstrap").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        UTIL.waitFor(15000, () -> !REGISTRY.getParsedServers().contains(masterInConf) && !REGISTRY.getParsedServers().contains(activeMasterServerName));
        RestartFramework.at("after_switch_to_regionservers").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Set<ServerName> parsedServers = REGISTRY.getParsedServers();
        assertEquals(2, parsedServers.size());
        RestartFramework.at("before_final_verification").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // now kill one region server
        ServerName serverToKill = parsedServers.iterator().next();
        RestartFramework.at("after_regionserver_kill").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
