...
public class TestMergeTableRegionsWhileRSCrash_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMergeTableRegionsWhileRSCrash_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestMergeTableRegionsWhileRSCrash_RestartInjected.class);
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_no_regions_in_transition").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_merge_submit").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_data_write").on(UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_regions").on(UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        int count = 0;
        Result result = null;
