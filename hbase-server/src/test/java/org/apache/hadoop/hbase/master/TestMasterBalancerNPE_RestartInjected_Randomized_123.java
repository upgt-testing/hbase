...
public class TestMasterBalancerNPE_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterBalancerNPE_RestartInjected.class);
    ...
    public void setupConfiguration() {
        ...
        TEST_UTIL.getConfiguration().set(HConstants.HBASE_MASTER_LOADBALANCER_CLASS, MyLoadBalancer.class.getName());
    }

    @After
    public void shutdown() throws Exception {
        HMaster.setDisableBalancerChoreForTest(false);
        TEST_UTIL.getConfiguration().set(HConstants.HBASE_MASTER_LOADBALANCER_CLASS, LoadBalancerFactory.getDefaultLoadBalancerClass().getName());
    ...
    public void testBalancerNPE() throws Exception {
        ...
        RestartFramework.at("after_get_region_info").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_setup_spies").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        Mockito.doAnswer((InvocationOnMock invocation) -> {
            ...
            Map<ServerName, List<RegionInfo>> regionServerNameToRegionInfos = (Map<ServerName, List<RegionInfo>>) invocation.getArgument(1);
        ...
        }).when(spiedLoadBalancer).balanceTable(Mockito.eq(HConstants.ENSEMBLE_TABLE_NAME), Mockito.anyMap());
        ...
        try {
            ...
            RestartFramework.at("after_enable_balance").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Refresh after master restart
            master = TEST_UTIL.getMiniHBaseCluster().getMaster();
            // Refresh after master restart
            assignmentManager = master.getAssignmentManager();
