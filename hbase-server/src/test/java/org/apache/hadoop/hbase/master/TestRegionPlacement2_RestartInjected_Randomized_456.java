...
public class TestRegionPlacement2_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionPlacement2_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRegionPlacement2_RestartInjected.class);
    ...
    public void testFavoredNodesPresentForRoundRobinAssignment() throws HBaseIOException {
        ...
        RestartFramework.at("after_remove_primary").on(TEST_UTIL.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_balancer_init").on(TEST_UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_add_back_primary").on(TEST_UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_assignment").on(TEST_UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFavoredNodesPresentForRandomAssignment() throws HBaseIOException {
        ...
        RestartFramework.at("after_first_random_assignment").on(TEST_UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<RegionInfo> regions = new ArrayList<>(1);
        RegionInfo region = RegionInfoBuilder.newBuilder(TableName.valueOf(name.getMethodName())).build();
        regions.add(region);
        ...
        List<ServerName> favoredNodesNow = ((FavoredNodeLoadBalancer) balancer).getFavoredNodes(region);
        RestartFramework.at("after_remove_all_favored_nodes").on(TEST_UTIL.getHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(favoredNodesNow.size() == FavoredNodeAssignmentHelper.FAVORED_NODES_NUM);
        assertTrue(!favoredNodesNow.contains(favoredNodesAfter.get(PRIMARY)) && !favoredNodesNow.contains(favoredNodesAfter.get(SECONDARY)) && !favoredNodesNow.contains(favoredNodesAfter.get(TERTIARY)));
    }
