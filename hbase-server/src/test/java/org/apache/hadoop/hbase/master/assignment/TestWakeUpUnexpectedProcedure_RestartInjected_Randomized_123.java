...
public class TestWakeUpUnexpectedProcedure_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestWakeUpUnexpectedProcedure_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestWakeUpUnexpectedProcedure_RestartInjected.class);
    ...
    private static final class AMForTest extends AssignmentManager {
        ...
        public ReportRegionStateTransitionResponse reportRegionStateTransition(ReportRegionStateTransitionRequest req) throws PleaseHoldException {
            ...
            if (rst.getTransitionCode() == TransitionCode.OPENED && ProtobufUtil.toTableName(rst.getRegionInfo(0).getTableName()).equals(NAME)) {
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_arrive_report").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_verify_single_region").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_arrive_exec_proc").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
