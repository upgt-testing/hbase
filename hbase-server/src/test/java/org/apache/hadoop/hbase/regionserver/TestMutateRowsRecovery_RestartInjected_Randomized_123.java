...
public class TestMutateRowsRecovery_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMutateRowsRecovery_RestartInjected.class);
    ...
    public void MutateRowsAndCheckPostKill() throws IOException, InterruptedException {
        ...
        try {
            ...
            rm.add(p1);
            hTable.mutateRow(rm);
            RestartFramework.at("after_row_mutation").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Add a put
            Put p2 = new Put(row1);
            ...
            RestartFramework.at("after_rs_load_report").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_rs_kill").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin = connection.getAdmin();
