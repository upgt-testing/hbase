...
public class TestSlowLogAccessor_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSlowLogAccessor_RestartInjected.class);
    ...
    public static void setup() throws Exception {
        ...
        cluster = HBASE_TESTING_UTILITY.getHBaseCluster();
    }
    ...
    public void testSlowLogRecords() throws Exception {
        ...
        RestartFramework.at("after_first_batch_records").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_slowlog_table_ready").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_all_records_added").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Assert.assertNotEquals(-1, HBASE_TESTING_UTILITY.waitFor(3000, () -> getSlowLogPayloads(request).size() == 14));
        Assert.assertNotEquals(-1, HBASE_TESTING_UTILITY.waitFor(3000, () -> getTableCount(connection) == 14));
    }
    ...
    public void testHigherSlowLogs() throws Exception {
        Connection connection = waitForSlowLogTableCreation();
        RestartFramework.at("after_async_records_submit").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        namedQueueRecorder.clearNamedQueue(NamedQueuePayload.NamedQueueEvent.SLOW_LOG);
