...
public class TestCompactionFileNotFound_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCompactionFileNotFound_RestartInjected.class);
    ...
    public void testSplitAfterRefresh() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_flush_1").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Put putb = new Put(ROW_B);
            putb.addColumn(TEST_FAMILY, qualifierCol1, bytes2);
            table.put(putb);
            admin.flush(TEST_TABLE);
            RestartFramework.at("after_flush_2").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Put putc = new Put(ROW_C);
            putc.addColumn(TEST_FAMILY, qualifierCol1, bytes3);
            table.put(putc);
            admin.flush(TEST_TABLE);
            RestartFramework.at("after_flush_3").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin.compact(TEST_TABLE);
            while (admin.getCompactionState(TEST_TABLE) != CompactionState.NONE) {
                Thread.sleep(1000);
            }
            RestartFramework.at("after_compact").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            table.put(putb);
            ...
            RestartFramework.at("after_store_refresh").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCompactionAfterRefresh() throws Exception {
        RestartFramework.at("after_table_create_2").on(util.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Admin admin = util.getAdmin();
        table = util.createTable(TEST_TABLE, TEST_FAMILY);
        admin = util.getAdmin();
        try {
            ...
            table.put(puta);
            admin.flush(TEST_TABLE);
            RestartFramework.at("after_flush_1_test2").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Put putb = new Put(ROW_B);
            putb.addColumn(TEST_FAMILY, qualifierCol1, bytes2);
            table.put(putb);
            admin.flush(TEST_TABLE);
            RestartFramework.at("after_flush_2_test2").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Put putc = new Put(ROW_C);
            putc.addColumn(TEST_FAMILY, qualifierCol1, bytes3);
            table.put(putc);
            admin.flush(TEST_TABLE);
            RestartFramework.at("after_flush_3_test2").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin.compact(TEST_TABLE);
            while (admin.getCompactionState(TEST_TABLE) != CompactionState.NONE) {
                Thread.sleep(1000);
            }
            RestartFramework.at("after_compact_test2").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            table.put(putb);
            HRegion hr1 = (HRegion) util.getRSForFirstRegionInTable(TEST_TABLE).getRegionByEncodedName(admin.getTableRegions(TEST_TABLE).get(0).getEncodedName());
            // Refresh store files post compaction, this should not open already compacted files
            hr1.refreshStoreFiles(true);
            RestartFramework.at("after_store_refresh_test2").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Archive the store files and try another compaction to see if all is good
            for (HStore store : hr1.getStores()) {
                store.closeAndArchiveCompactedFiles();
            }
            RestartFramework.at("after_archive_compacted_files").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
                hr1.compact(false);
                RestartFramework.at("after_second_compact").on(util.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            } catch (IOException e) {
