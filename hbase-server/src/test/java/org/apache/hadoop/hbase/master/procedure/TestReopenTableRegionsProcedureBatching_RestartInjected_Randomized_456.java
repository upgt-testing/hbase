...
public class TestReopenTableRegionsProcedureBatching_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReopenTableRegionsProcedureBatching_RestartInjected.class);
    ...
    public void testSmallMaxBatchSize() throws IOException {
        ...
        Set<StuckRegion> stuckRegions = regions.stream().map(r -> stickRegion(finalAm, finalProcExec, r)).collect(Collectors.toSet());
        RestartFramework.at("after_procedure_waiting").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ReopenTableRegionsProcedure proc = new ReopenTableRegionsProcedure(TABLE_NAME, BACKOFF_MILLIS_PER_RS, REOPEN_BATCH_SIZE_MAX);
        ...
        RestartFramework.at("after_stick_regions").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDefaultMaxBatchSize() throws IOException {
        RestartFramework.at("after_get_regions_default").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        AssignmentManager am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        RestartFramework.at("after_stick_regions_default").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureExecutor<MasterProcedureEnv> procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        List<RegionInfo> regions = UTIL.getAdmin().getRegions(TABLE_NAME);
        assertTrue(2 <= regions.size());
        RestartFramework.at("after_procedure_waiting_default").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        // Refresh after master restart
        procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        ...
        procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        UTIL.waitFor(10000, () -> proc.getState() == ProcedureState.WAITING_TIMEOUT);
        RestartFramework.at("after_submit_procedure_default").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNegativeBatchSizeDoesNotBreak() throws IOException {
        AssignmentManager am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        ProcedureExecutor<MasterProcedureEnv> procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        RestartFramework.at("after_stick_regions_negative").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<RegionInfo> regions = UTIL.getAdmin().getRegions(TABLE_NAME);
        assertTrue(2 <= regions.size());
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        // Refresh after master restart
        procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        RestartFramework.at("after_submit_procedure_negative").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final AssignmentManager finalAm = am;
        final ProcedureExecutor<MasterProcedureEnv> finalProcExec = procExec;
        Set<StuckRegion> stuckRegions = regions.stream().map(r -> stickRegion(finalAm, finalProcExec, r)).collect(Collectors.toSet());
        ReopenTableRegionsProcedure proc = new ReopenTableRegionsProcedure(TABLE_NAME, BACKOFF_MILLIS_PER_RS, -100);
        procExec.submitProcedure(proc);
        // Refresh after master restart
        am = UTIL.getMiniHBaseCluster().getMaster().getAssignmentManager();
        // Refresh after master restart
        ...
        // the first batch should be small
        confirmBatchSize(1, stuckRegions, proc);
        RestartFramework.at("after_procedure_waiting_negative").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureSyncWait.waitForProcedureToComplete(procExec, proc, 60_000);
        RestartFramework.at("after_get_regions_negative").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // other batches should also be small
        assertTrue(proc.getBatchesProcessed() >= regions.size());
        // all regions should only be opened once
        assertEquals(proc.getRegionsReopened(), regions.size());
    ...
    public void testBatchSizeDoesNotOverflow() {
        ReopenTableRegionsProcedure proc = new ReopenTableRegionsProcedure(TABLE_NAME, BACKOFF_MILLIS_PER_RS, Integer.MAX_VALUE);
