...
public class TestRegionReplicasAreDistributed_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionReplicasAreDistributed_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRegionReplicasAreDistributed_RestartInjected.class);
    ...
    public static void before() throws Exception {
        ...
        Thread.sleep(3000);
        final TableName tableName = TableName.valueOf(TestRegionReplicasAreDistributed_RestartInjected.class.getSimpleName());
    ...
    public void testRegionReplicasCreatedAreDistributed() throws Exception {
        try {
            ...
            RestartFramework.at("after_table_disable").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("Disabled the table " + table.getName());
            LOG.info("enabling the table " + table.getName());
            HTU.getAdmin().enableTable(table.getName());
            RestartFramework.at("after_table_enable").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
