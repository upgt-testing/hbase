...
public class TestNamespaceCommands_RestartInjected_Randomized_456 extends SecureTestUtil {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestNamespaceCommands_RestartInjected.class);

    private static HBaseTestingUtility UTIL = new HBaseTestingUtility();

    private static MiniHBaseCluster cluster;

    private static final Logger LOG = LoggerFactory.getLogger(TestNamespaceCommands_RestartInjected.class);
    ...
    // TODO: WE DO NOT GIVE ANY PERMS TO THIS USER
    ...
    public static void beforeClass() throws Exception {
        ...
        RestartFramework.at("after_global_group_grants").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_acl_table_available").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_namespace_creation").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_namespace_grants").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        if (ACCESS_CONTROLLER == null) {
        ...
        RestartFramework.at("after_access_controller_setup").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_global_user_grants").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAclTableEntries() throws Exception {
        ...
        try {
            ListMultimap<String, UserPermission> perms = PermissionStorage.getNamespacePermissions(conf, TEST_NAMESPACE);
            ...
            assertEquals(TEST_NAMESPACE, ((NamespacePermission) namespacePerms.get(0).getPermission()).getNamespace());
    ...
    public void testCreateTableWithNamespace() throws Exception {
        ...
        verifyAllowed(createTable, SUPERUSER, USER_GLOBAL_CREATE, USER_NS_CREATE, USER_GROUP_CREATE, USER_GLOBAL_ADMIN, USER_NS_ADMIN, USER_GROUP_ADMIN);
        verifyDenied(createTable, USER_GLOBAL_WRITE, USER_GLOBAL_READ, USER_GLOBAL_EXEC, USER_NS_WRITE, USER_NS_READ, USER_NS_EXEC, USER_TABLE_CREATE, USER_TABLE_WRITE, USER_GROUP_READ, USER_GROUP_WRITE);
    }
}
