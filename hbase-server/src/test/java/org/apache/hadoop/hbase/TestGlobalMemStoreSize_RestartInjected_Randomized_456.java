...
public class TestGlobalMemStoreSize_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestGlobalMemStoreSize_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestGlobalMemStoreSize_RestartInjected.class);
    ...
    public void testGlobalMemStore() throws Exception {
        ...
        RestartFramework.at("after_memstore_check").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_region_assignment").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = TEST_UTIL.getHBaseCluster();
        int numRegions = -1;
        ...
        for (HRegionServer server : getOnlineRegionServers()) {
            ...
            while (server.getRegionServerAccounting().getGlobalMemStoreDataSize() != 0 && timeout < EnvironmentEdgeManager.currentTime()) {
                Threads.sleep(10);
            }
