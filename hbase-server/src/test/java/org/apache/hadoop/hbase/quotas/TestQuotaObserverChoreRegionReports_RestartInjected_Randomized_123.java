...
public class TestQuotaObserverChoreRegionReports_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestQuotaObserverChoreRegionReports_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestQuotaObserverChoreRegionReports_RestartInjected.class);
    ...
    public void testReportExpiration() throws Exception {
        Configuration conf = TEST_UTIL.getConfiguration();
        RestartFramework.at("after_report_received").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        TableDescriptor tableDesc = TableDescriptorBuilder.newBuilder(tn).addColumnFamily(ColumnFamilyDescriptorBuilder.of(FAM1)).build();
    ...
    public void testMissingReportsRemovesQuota() throws Exception {
        ...
        TableDescriptor tableDesc = TableDescriptorBuilder.newBuilder(tn).addColumnFamily(ColumnFamilyDescriptorBuilder.of(FAM1)).build();
        TEST_UTIL.getAdmin().createTable(tableDesc);
        RestartFramework.at("after_violation").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Set a quota
        ...
        RestartFramework.at("after_region_unassign").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.flush(tn);
