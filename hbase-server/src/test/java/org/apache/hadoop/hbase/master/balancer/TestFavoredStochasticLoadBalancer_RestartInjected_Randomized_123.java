...
public class TestFavoredStochasticLoadBalancer_RestartInjected_Randomized_123 extends BalancerTestBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFavoredStochasticLoadBalancer_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestFavoredStochasticLoadBalancer_RestartInjected.class);
    ...
    private void checkFavoredNodeAssignments(TableName tableName, FavoredNodesManager fnm, RegionStates regionStates) throws IOException {
        for (RegionInfo hri : admin.getTableRegions(tableName)) {
            ...
            assertNotNull("Region: " + hri.getEncodedName() + " not on FN, current: " + host + " FN list: " + fnm.getFavoredNodes(hri), FavoredNodesPlan.getFavoredServerPosition(fnm.getFavoredNodes(hri), host));
        }
    }
    ...
    private void stopServersAndWaitUntilProcessed(List<ServerName> currentFN) throws Exception {
        ...
        assertEquals("Not all servers killed", SLAVES - currentFN.size(), cluster.getLiveRegionServerThreads().size());
    }
