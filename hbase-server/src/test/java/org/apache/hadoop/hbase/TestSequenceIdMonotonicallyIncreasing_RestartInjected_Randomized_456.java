...
public class TestSequenceIdMonotonicallyIncreasing_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSequenceIdMonotonicallyIncreasing_RestartInjected.class);
    ...
    private long getMaxSeqId(HRegionServer rs, RegionInfo region) throws IOException {
        ...
        long maxSeqId = -1L;
        try (WALStreamReader reader = NoEOFWALStreamReader.create(UTIL.getTestFileSystem(), walFile, UTIL.getConfiguration())) {
            for (; ; ) {
    ...
    public void testSplit() throws IOException, InterruptedException, ExecutionException, TimeoutException {
        ...
        RestartFramework.at("before_region_split").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMerge() throws IOException, InterruptedException, ExecutionException, TimeoutException {
        ...
        RestartFramework.at("before_region_merge").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
