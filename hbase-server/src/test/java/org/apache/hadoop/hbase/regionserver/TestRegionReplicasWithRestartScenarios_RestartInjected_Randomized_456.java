...
public class TestRegionReplicasWithRestartScenarios_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionReplicasWithRestartScenarios_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRegionReplicasWithRestartScenarios_RestartInjected.class);
    ...
    public void testRegionReplicasCreated() throws Exception {
        ...
        RestartFramework.at("after_replica_distribution_verification").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testWhenRestart() throws Exception {
        ...
        RestartFramework.at("after_new_regionserver_start").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Make a copy because this is actual instance from HRegionServer
        Collection<HRegion> regionsOnStoppedServer = new ArrayList<HRegion>(stopRegionServer.getOnlineRegionsLocalContext());
        HTU.getHBaseCluster().stopRegionServer(serverName);
        HTU.getHBaseCluster().waitForRegionServerToStop(serverName, 60000);
        RestartFramework.at("after_final_replica_distribution_verification").on(HTU.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HTU.waitTableAvailable(this.tableName);
        RestartFramework.at("after_regionserver_stop_and_table_available").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        for (Region region : copyOfRegion) {
            ...
            for (Region actualRegion : onlineRegions3) {
                if (regionInfoForReplica.equals(RegionReplicaUtil.getRegionInfoForDefaultReplica(actualRegion.getRegionInfo()))) {
                    i++;
                    if (i > 1) {
                        LOG.warn("Duplicate found {} and {}", actualRegion.getRegionInfo(), region.getRegionInfo());
