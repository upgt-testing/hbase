...
public class TestAsyncClusterAdminApi_RestartInjected_Randomized_456 extends TestAsyncAdminBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAsyncClusterAdminApi_RestartInjected.class);
    ...
    public void testGetMasterInfoPort() throws Exception {
        ...
        RestartFramework.at("after_get_master_info_port").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void testRegionServerOnlineConfigChange() throws Exception {
        RestartFramework.at("after_rs_config_update").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        replaceHBaseSiteXML();
    ...
    public void testMasterOnlineConfigChange() throws Exception {
        ...
        RestartFramework.at("after_backup_master_config_update").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.updateConfiguration(master).join();
        RestartFramework.at("after_master_config_update").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAllClusterOnlineConfigChange() throws IOException {
        ...
        RestartFramework.at("after_cluster_config_update_rs").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cluster_config_update").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Check the configuration of the Masters
        TEST_UTIL.getMiniHBaseCluster().getMasterThreads().forEach(thread -> {
            Configuration conf = thread.getMaster().getConfiguration();
            assertEquals(1000, conf.getInt("hbase.custom.config", 0));
    ...
    public void testRollWALWALWriter() throws Exception {
        ...
        RestartFramework.at("after_flush_regions").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // flush all regions
        for (HRegion r : regionServer.getOnlineRegionsLocalContext()) {
            r.flush(true);
        }
        RestartFramework.at("after_roll_wal").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetRegionLoads() throws Exception {
        ...
        RestartFramework.at("after_create_load_tables").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_get_cluster_metrics").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetRegionServers() throws Exception {
        ...
        RestartFramework.at("after_recommission_rs").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_decommission_rs").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
