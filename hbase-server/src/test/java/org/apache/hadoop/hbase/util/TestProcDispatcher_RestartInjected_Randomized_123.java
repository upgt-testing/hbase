...
public class TestProcDispatcher_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestProcDispatcher_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestProcDispatcher_RestartInjected.class);
    ...
    public static void setUpBeforeClass() throws Exception {
        TEST_UTIL.getConfiguration().set(HBASE_MASTER_RSPROC_DISPATCHER_CLASS, RSProcDispatcher.class.getName());
    ...
    public void testRetryLimitOnConnClosedErrors() throws Exception {
        ...
        HRegion region0 = !hRegionServer0.getRegions().isEmpty() ? hRegionServer0.getRegions().get(0) : hRegionServer1.getRegions().get(0);
        ...
        TEST_UTIL.waitFor(5000, 1000, () -> {
            ...
            LOG.info("Num of successfully completed procedures: {} , num of all procedures: {}", master.getMasterProcedureExecutor().getProcedures().stream().filter(masterProcedureEnvProcedure -> masterProcedureEnvProcedure.getState() == ProcedureProtos.ProcedureState.SUCCESS).count(), master.getMasterProcedureExecutor().getProcedures().size());
