...
public class TestRollingRestart_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRollingRestart_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRollingRestart_RestartInjected.class);
    ...
    public void testBasicRollingRestart() throws Exception {
        ...
        RestartFramework.at("after_restart_active_master").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // catalogs
        numRegions += 1;
        ...
        RestartFramework.at("after_stop_active_master").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = TEST_UTIL.getHBaseCluster();
        ...
        cluster = TEST_UTIL.getHBaseCluster();
        RestartFramework.at("after_restart_backup_master").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_stop_backup_master").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_add_regionserver").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (RegionServerThread rst : regionServers) {
            ...
            expectedNumRS--;
            assertEquals(expectedNumRS, cluster.getRegionServerThreads().size());
            RestartFramework.at("after_rs_shutdown_" + num).on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            log("Waiting for no more RIT");
            TEST_UTIL.waitUntilNoRegionsInTransition(60000);
            RestartFramework.at("after_rs_restart_" + num).on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            log("Verifying there are " + numRegions + " assigned on cluster");
            assertRegionsAssigned(cluster, regions);
            assertEquals(expectedNumRS, cluster.getRegionServerThreads().size());
            num++;
    ...
    private boolean isDeadServerSCPExecuted(ServerName serverName) throws IOException {
        return TEST_UTIL.getMiniHBaseCluster().getMaster().getProcedures().stream().anyMatch(p -> p instanceof ServerCrashProcedure && ((ServerCrashProcedure) p).getServerName().equals(serverName));
    }

    private void waitForRSShutdownToStartAndFinish(MasterThread activeMaster, ServerName serverName) throws InterruptedException, IOException {
