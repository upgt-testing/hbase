...
public class TestSpaceQuotaBasicFunctioning_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSpaceQuotaBasicFunctioning_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestSpaceQuotaBasicFunctioning_RestartInjected.class);
    ...
    public void testNoInsertsWithPut() throws Exception {
        RestartFramework.at("before_test").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNoCompactions() throws Exception {
        Put p = new Put(Bytes.toBytes("to_reject"));
        RestartFramework.at("after_compaction_rejection_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        p.addColumn(Bytes.toBytes(SpaceQuotaHelperForTests.F1), Bytes.toBytes("to"), Bytes.toBytes("reject"));
        RestartFramework.at("before_test").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_violation_verification").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final TableName tn = helper.writeUntilViolationAndVerifyViolation(SpaceViolationPolicy.NO_WRITES_COMPACTIONS, p);
        try {
