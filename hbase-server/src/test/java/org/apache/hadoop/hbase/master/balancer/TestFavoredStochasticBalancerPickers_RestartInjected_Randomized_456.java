...
public class TestFavoredStochasticBalancerPickers_RestartInjected_Randomized_456 extends BalancerTestBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFavoredStochasticBalancerPickers_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestFavoredStochasticBalancerPickers_RestartInjected.class);
    ...
    public void testPickers() throws Exception {
        ...
        RestartFramework.at("after_new_rs_start").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_balancer_setup").on(this.cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_region_moves").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
