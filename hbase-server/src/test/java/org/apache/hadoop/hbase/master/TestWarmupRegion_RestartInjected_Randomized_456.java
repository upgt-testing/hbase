...
public class TestWarmupRegion_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestWarmupRegion_RestartInjected.class);
    ...
    public void testWarmup() throws Exception {
        ...
        RegionInfo info = region.getRegionInfo();
        runwarmup();
        RestartFramework.at("after_region_moves").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_warmup").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 10; i++) {
    ...
    public void testWarmupAndClose() throws IOException {
        ...
        RestartFramework.at("after_get_region_info").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HRegion warmedUpRegion = warmupHRegion(info, htd, rs.getWAL(info), rs.getConfiguration(), rs, null);
        RestartFramework.at("after_warmup").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        rs = TEST_UTIL.getMiniHBaseCluster().getRegionServer(0);
