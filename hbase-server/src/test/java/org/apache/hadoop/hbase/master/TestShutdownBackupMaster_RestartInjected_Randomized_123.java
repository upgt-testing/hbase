...
public class TestShutdownBackupMaster_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestShutdownBackupMaster_RestartInjected.class);
    ...
    public void testShutdownWhileBecomingActive() throws InterruptedException {
        ...
        RestartFramework.at("after_assert_masters").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_find_masters").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
