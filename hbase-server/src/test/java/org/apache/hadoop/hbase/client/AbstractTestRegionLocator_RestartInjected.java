...
public abstract class AbstractTestRegionLocator_RestartInjected {
  ...
  public void testGetRegionLocation() throws IOException {
    for (int i = 0; i <= SPLIT_KEYS.length; i++) {
      ...
      if (i == SPLIT_KEYS.length / 2) {
        RestartFramework.at("during_region_location_lookup")
            .on(UTIL.getMiniHBaseCluster())
            .restart("regionserver")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
  ...
  public void testGetRegionLocations() throws IOException {
    for (int i = 0; i <= SPLIT_KEYS.length; i++) {
      ...
      if (i == SPLIT_KEYS.length / 2) {
        RestartFramework.at("during_region_locations_lookup")
            .on(UTIL.getMiniHBaseCluster())
            .restart("regionserver")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
  ...
  public void testGetAllRegionLocations() throws IOException {
    ...
    RestartFramework.at("after_get_all_locations")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMeta() throws IOException {
    ...
    RestartFramework.at("after_meta_start_end_keys")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_meta_region_locations")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
