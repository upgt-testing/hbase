...
public class TestCoprocessorStop_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCoprocessorStop_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestCoprocessorStop_RestartInjected.class);

    private static HBaseTestingUtility UTIL = new HBaseTestingUtility();

    private static final String MASTER_FILE = "master" + EnvironmentEdgeManager.currentTime();

    private static final String REGIONSERVER_FILE = "regionserver" + EnvironmentEdgeManager.currentTime();
    ...
    public void testStopped() throws Exception {
        ...
        RestartFramework.at("after_get_cluster_rs").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = UTIL.getHBaseCluster();
        cluster = UTIL.getHBaseCluster();
        LOG.info("shutdown hbase cluster...");
        cluster.shutdown();
        RestartFramework.at("after_get_cluster").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
