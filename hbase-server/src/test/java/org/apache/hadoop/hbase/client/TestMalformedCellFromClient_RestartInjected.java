...
public class TestMalformedCellFromClient_RestartInjected {
  private static final Logger LOG = LoggerFactory.getLogger(TestMalformedCellFromClient_RestartInjected.class);
  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestMalformedCellFromClient_RestartInjected.class);
  ...
  public void testRegionException() throws InterruptedException, IOException {
    ...
    try (Table table = TEST_UTIL.getConnection().getTable(TABLE_NAME)) {
      ...
      RestartFramework.at("after_successful_get")
        .on(TEST_UTIL.getMiniHBaseCluster())
  ...
  public void testRegionExceptionByAsync() throws Exception {
    ...
    try (AsyncConnection asyncConnection =
      ...
      RestartFramework.at("after_async_get")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testAtomicOperations() throws Exception {
    ...
    try (Table table = TEST_UTIL.getConnection().getTable(TABLE_NAME)) {
      ...
      RestartFramework.at("after_atomic_operation")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testNonAtomicOperations() throws InterruptedException, IOException {
    ...
    try (Table table = TEST_UTIL.getConnection().getTable(TABLE_NAME)) {
      ...
      RestartFramework.at("after_batch_with_failures")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
