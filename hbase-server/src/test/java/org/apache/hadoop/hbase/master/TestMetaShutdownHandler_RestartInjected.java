...
public class TestMetaShutdownHandler_RestartInjected {
  private static final Logger LOG = LoggerFactory.getLogger(TestMetaShutdownHandler_RestartInjected.class);
  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestMetaShutdownHandler_RestartInjected.class);
  ...
  public void testExpireMetaRegionServer() throws Exception {
    ...
    RestartFramework.at("after_get_meta_server")
        .on(cluster)
        .restart("master")
        .withIndex(0)
    ...
    if (
      ...
      RestartFramework.at("after_move_meta")
          .on(cluster)
    ...
    RestartFramework.at("after_get_meta_state")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_delete_zk_node")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_rs_stopped")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_server_offline")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_no_rit")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_meta_reassigned")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
