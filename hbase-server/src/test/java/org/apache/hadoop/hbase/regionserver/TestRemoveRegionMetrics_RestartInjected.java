...
public class TestRemoveRegionMetrics_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestRemoveRegionMetrics_RestartInjected.class);
  ...
  public void testMoveRegion() throws IOException, InterruptedException {
    ...
    for (int i = 0; i < 30; i++) {
      ...
      if (i == 15) {
        RestartFramework.at("after_mid_put")
            .on(cluster)
            .restart("regionserver")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_loop_complete")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
