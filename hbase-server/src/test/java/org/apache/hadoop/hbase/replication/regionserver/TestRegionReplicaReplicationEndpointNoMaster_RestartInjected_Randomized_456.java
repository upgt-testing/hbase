...
public class TestRegionReplicaReplicationEndpointNoMaster_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionReplicaReplicationEndpointNoMaster_RestartInjected.class);

    private static final int NB_SERVERS = 2;

    private static TableName tableName = TableName.valueOf(TestRegionReplicaReplicationEndpointNoMaster_RestartInjected.class.getSimpleName());
    ...
    public static void beforeClass() throws Exception {
        ...
        StartMiniClusterOption option = StartMiniClusterOption.builder().numAlwaysStandByMasters(1).numRegionServers(NB_SERVERS).numDataNodes(NB_SERVERS).build();
    ...
    public void testReplayCallable() throws Exception {
        ...
        RestartFramework.at("after_replay").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HTU.verifyNumericRows(region, f, 0, 1000);
        HTU.deleteNumericRows(table, f, 0, 1000);
        RestartFramework.at("after_data_load").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void replicateUsingCallable(ClusterConnection connection, Queue<Entry> entries) throws IOException, RuntimeException {
        ...
        while ((entry = entries.poll()) != null) {
            ...
            RpcRetryingCallerFactory factory = RpcRetryingCallerFactory.instantiate(connection.getConfiguration(), connection.getConnectionConfiguration(), connection.getConnectionMetrics());
    ...
    public void testReplayCallableWithRegionMove() throws Exception {
        ...
        openRegion(HTU, rs0, hriSecondary);
        RestartFramework.at("after_first_data_load").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ClusterConnection connection = (ClusterConnection) ConnectionFactory.createConnection(HTU.getConfiguration());
        // load some data to primary
        HTU.loadNumericRows(table, f, 0, 1000);
        RestartFramework.at("after_second_data_load").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Assert.assertEquals(1000, entries.size());
        // replay the edits to the secondary using replay callable
        replicateUsingCallable(connection, entries);
        Region region = rs0.getRegion(hriSecondary.getEncodedName());
        ...
        RestartFramework.at("after_region_move").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_replay").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_replay").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRegionReplicaReplicationEndpointReplicate() throws Exception {
        ...
        openRegion(HTU, rs0, hriSecondary);
        RestartFramework.at("after_endpoint_data_load").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ClusterConnection connection = (ClusterConnection) ConnectionFactory.createConnection(HTU.getConfiguration());
        ...
        RestartFramework.at("after_endpoint_start").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        replicator.replicate(new ReplicateContext().setEntries(Lists.newArrayList(entries)).setWalGroupId(fakeWalGroupId));
        RestartFramework.at("after_endpoint_replicate").on(HTU.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Region region = rs0.getRegion(hriSecondary.getEncodedName());
        HTU.verifyNumericRows(region, f, 0, 1000);
        HTU.deleteNumericRows(table, f, 0, 1000);
        closeRegion(HTU, rs0, hriSecondary);
