...
public class TestFailedMetaReplicaAssigment_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFailedMetaReplicaAssigment_RestartInjected.class);
    ...
    public void testFailedReplicaAssignment() throws InterruptedException {
        ...
        TEST_UTIL.waitFor(30000, () -> {
            RegionStateNode metaReplicaRegionNode = finalAm.getRegionStates().getOrCreateRegionStateNode(metaReplicaHri);
        ...
        RestartFramework.at("after_failed_replica_check").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_replica_assigned").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_master_init").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BrokenMetaReplicaMaster extends HMaster {
        ...
        @Override
        public AssignmentManager createAssignmentManager(MasterServices master, MasterRegion masterRegion) {
    ...
    public static class BrokenMasterMetaAssignmentManager extends AssignmentManager {
        ...
        public BrokenMasterMetaAssignmentManager(final MasterServices master, MasterRegion masterRegion) {
        ...
        public TransitRegionStateProcedure[] createAssignProcedures(List<RegionInfo> hris) {
            ...
            for (RegionInfo hri : hris) {
                if (hri.isMetaRegion() && hri.getReplicaId() == 2) {
                    ...
                    regionNode.lock();
                    try {
                        procs.add(regionNode.setProcedure(new BrokenTransitRegionStateProcedure(master.getMasterProcedureExecutor().getEnvironment(), hri)));
                    } finally {
                        regionNode.unlock();
                    }
                } else {
