...
public class TestReplicationStatusBothNormalAndRecoveryLagging_RestartInjected_Randomized_123 extends TestReplicationBase {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationStatusBothNormalAndRecoveryLagging_RestartInjected.class);

    @Test
    public void testReplicationStatusBothNormalAndRecoveryLagging() throws Exception {
        RestartFramework.at("after_source_cluster_restart").on(UTIL1.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_batch_puts").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(10000);
        RestartFramework.at("after_second_batch_puts").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_server_name").on(UTIL1.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
