...
public abstract class TestReplicationSourceManager_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationSourceManager_RestartInjected.class);

    protected static final Logger LOG = LoggerFactory.getLogger(TestReplicationSourceManager_RestartInjected.class);
    ...
    protected static void setupZkAndReplication() throws Exception {
        ...
        replication.initialize(new DummyServer(), fs, logDir, oldLogDir, new WALFactory(conf, "test", null));
    ...
    public void testLogRoll() throws Exception {
        ...
        wal.sync();
        int logNumber = 0;
        RestartFramework.at("after_log_cleanup").on(utility.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        wal.rollWriter();
        RestartFramework.at("after_manager_init").on(utility.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        manager.logPositionAndCleanOldLogs(manager.getSources().get(0), new WALEntryBatch(0, manager.getSources().get(0).getCurrentPath()));
        wal.appendData(hri, new WALKeyImpl(hri.getEncodedNameAsBytes(), test, EnvironmentEdgeManager.currentTime(), mvcc, scopes), edit);
        wal.sync();
    ...
    public void testCleanupFailoverQueues() throws Exception {
        ...
        Server s1 = new DummyServer("dummyserver1.example.org");
        ReplicationPeers rp1 = ReplicationFactory.getReplicationPeers(s1.getFileSystem(), s1.getZooKeeper(), s1.getConfiguration());
        rp1.init();
    ...
    public void testPeerRemovalCleanup() throws Exception {
        ...
        final ReplicationPeerConfig peerConfig = new ReplicationPeerConfig().setClusterKey(utility.getZkCluster().getAddress().toString() + ":/hbase");
        try {
    ...
    public void testSameWALPrefix() throws IOException {
        Set<String> latestWalsBefore = manager.getLastestPath().stream().map(Path::getName).collect(Collectors.toSet());
        ...
        Set<String> latestWals = manager.getLastestPath().stream().map(Path::getName).filter(n -> !latestWalsBefore.contains(n)).collect(Collectors.toSet());
