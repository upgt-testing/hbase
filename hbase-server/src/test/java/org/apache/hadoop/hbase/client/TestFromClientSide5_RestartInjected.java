...
public class TestFromClientSide5_RestartInjected extends FromClientSideBase {
  private static final Logger LOG = LoggerFactory.getLogger(TestFromClientSide5_RestartInjected.class);

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestFromClientSide5_RestartInjected.class);

  @Rule
  public TableNameTestRule name = new TableNameTestRule();

  // To keep the child classes happy.
  TestFromClientSide5_RestartInjected() {
  }

  public TestFromClientSide5_RestartInjected(Class<? extends ConnectionRegistry> registry, int numHedgedReqs)
    throws Exception {
  ...
  public void testRowMutations() throws Exception {
    ...
    try (Table t = TEST_UTIL.createTable(tableName, FAMILY)) {
      ...
      RestartFramework.at("after_row_mutations")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testBatchAppendWithReturnResultFalse() throws Exception {
    ...
    try (Table table = TEST_UTIL.createTable(tableName, FAMILY)) {
      ...
      RestartFramework.at("after_batch_append")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private List<Result> doAppend(final boolean walUsed) throws IOException {
    ...
    try (Table t = TEST_UTIL.createTable(TABLENAME, FAMILY)) {
      ...
      t.put(put_0);
      t.put(put_1);
      RestartFramework.at("after_append_and_puts")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testCheckAndPut() throws IOException {
    ...
    try (Table table = TEST_UTIL.createTable(name.getTableName(), FAMILY)) {
      ...
      assertTrue(ok);
      RestartFramework.at("after_check_and_put")
          .on(TEST_UTIL.getMiniHBaseCluster())
          .restart("regionserver")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
