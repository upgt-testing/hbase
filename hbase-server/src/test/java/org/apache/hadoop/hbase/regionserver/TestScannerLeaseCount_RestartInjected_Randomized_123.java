...
public class TestScannerLeaseCount_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestScannerLeaseCount_RestartInjected.class);
    ...
    public void itIncreasesScannerCount() throws Exception {
        ...
        RestartFramework.at("before_scanner_create").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_scanner_operations").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void itDoesNotIncreaseScannerLeaseCount() throws Exception {
        SHOULD_THROW = true;
        RestartFramework.at("before_throttled_scanner_create").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try (ResultScanner scanner = TABLE.getScanner(SCAN)) {
        ...
        RestartFramework.at("after_throttle_verification").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    private static class ScannerTrackingRSRpcServicesForTest extends RSRpcServices {
        ...
        @Override
        RegionScannerContext checkQuotaAndGetRegionScannerContext(ClientProtos.ScanRequest request, ClientProtos.ScanResponse.Builder builder) throws IOException {
    ...
    private static boolean isTestScan(ClientProtos.ScanRequest request) {
        ...
        return scan.getAttributeList().stream().anyMatch(nbp -> nbp.getName().equals(SCAN_IDENTIFIER_NAME) && Bytes.equals(nbp.getValue().toByteArray(), SCAN_IDENTIFIER));
    }
}
