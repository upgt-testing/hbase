...
public class TestTableSnapshotScanner_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestTableSnapshotScanner_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestTableSnapshotScanner_RestartInjected.class);
    ...
    public void setupCluster() throws Exception {
        ...
        StartMiniClusterOption option = StartMiniClusterOption.builder().numRegionServers(NUM_REGION_SERVERS).numDataNodes(NUM_REGION_SERVERS).createRootDir(true).build();
    ...
    public void testNoDuplicateResultsWhenSplitting() throws Exception {
        ...
        try {
            ...
            Table table = UTIL.getConnection().getTable(tableName);
            UTIL.loadTable(table, FAMILIES);
            RestartFramework.at("after_data_load").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // split to 2 regions
            admin.split(tableName, Bytes.toBytes("eee"));
            blockUntilSplitFinished(UTIL, tableName, 2);
            RestartFramework.at("after_region_split").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path rootDir = CommonFSUtils.getRootDir(UTIL.getConfiguration());
            FileSystem fs = rootDir.getFileSystem(UTIL.getConfiguration());
            SnapshotTestingUtils.createSnapshotAndValidate(admin, tableName, Arrays.asList(FAMILIES), null, snapshotName, rootDir, fs, true);
            RestartFramework.at("after_snapshot").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // load different values
            byte[] value = Bytes.toBytes("after_snapshot_value");
            UTIL.loadTable(table, FAMILIES, value);
            // cause flush to create new files in the region
            admin.flush(tableName);
            RestartFramework.at("after_flush").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            table.close();
    ...
    public void testScanLimit() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_create_snapshot").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path restoreDir = UTIL.getDataTestDirOnTestFS(snapshotName);
            // limit the scan
            ...
            while (true) {
                ...
                count++;
                if (count == 50) {
                    RestartFramework.at("during_scan").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
            }
    ...
    public void testScanMetricsByRegionForSingleRegion() throws Exception {
        ...
        Assert.assertNotNull(scanMetrics);
        Map<ScanMetricsRegionInfo, Map<String, Long>> scanMetricsByRegion = scanMetrics.collectMetricsByRegion();
    ...
    public void testScannerWithRestoreScanner() throws Exception {
        ...
        try {
            ...
            scanner0.close();
            RestartFramework.at("after_first_scan").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // restore snapshot.
            RestoreSnapshotHelper.copySnapshotForScanner(conf, fs, rootDir, restoreDir, snapshotName);
            RestartFramework.at("after_restore_snapshot").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMergeRegion() throws Exception {
        ...
        try (Admin admin = UTIL.getAdmin()) {
            ...
            RestartFramework.at("after_region_merge").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // snapshot
            ...
            // scan snapshot
            try (TableSnapshotScanner scanner = new TableSnapshotScanner(conf, UTIL.getDataTestDirOnTestFS(snapshotName), snapshotName, new Scan(bbb, yyy))) {
                verifyScanner(scanner, bbb, yyy);
            }
        } catch (Exception e) {
