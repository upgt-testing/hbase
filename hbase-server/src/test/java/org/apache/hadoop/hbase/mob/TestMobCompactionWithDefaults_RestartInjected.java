...
public class TestMobCompactionWithDefaults_RestartInjected {
  private static final Logger LOG = LoggerFactory.getLogger(TestMobCompactionWithDefaults_RestartInjected.class);
  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestMobCompactionWithDefaults_RestartInjected.class);
  ...
  public TestMobCompactionWithDefaults_RestartInjected(Boolean useFileBasedSFT) {
  ...
  public void baseTestMobFileCompaction() throws InterruptedException, IOException {
    ...
    RestartFramework.at("after_load_three_times")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_mob_compact")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_cleanup")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMobFileCompactionAfterSnapshotClone() throws InterruptedException, IOException {
    ...
    loadAndFlushThreeTimes(rows, table, famStr);
    RestartFramework.at("after_load_three_times_clone")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_hlinks_check")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_mob_compact_clone")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_mob_file_check_clone")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_cleanup_clone")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMobFileCompactionAfterSnapshotCloneAndFlush()
    ...
    loadAndFlushThreeTimes(rows, table, famStr);
    RestartFramework.at("after_first_load_flush")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    LOG.debug("Taking snapshot and cloning table {}", table);
    admin.snapshot(TestMobUtils.getTableName(test), table);
    RestartFramework.at("after_snapshot_flush_test")
        .on(HTU.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
        .execute();
    admin.cloneSnapshot(TestMobUtils.getTableName(test), clone);
    RestartFramework.at("after_clone_snapshot_flush_test")
        .on(HTU.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    assertEquals("Should have 3 hlinks per region in MOB area from snapshot clone", 3 * numRegions,
      getNumberOfMobFiles(clone, famStr));
    RestartFramework.at("after_hlinks_check_flush")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_second_load_flush")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
        .execute();
    mobCompact(admin.getDescriptor(clone), familyDescriptor);
    RestartFramework.at("after_mob_compact_flush_test")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_mob_file_check_flush")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_cleanup_flush")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  protected void mobCompact(TableDescriptor tableDescriptor,
    ...
    enableCompactions();
    RestartFramework.at("after_enable_compactions")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_mob_compact_impl")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
