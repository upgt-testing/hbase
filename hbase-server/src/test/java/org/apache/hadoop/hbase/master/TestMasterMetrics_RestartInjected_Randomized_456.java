...
public class TestMasterMetrics_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterMetrics_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestMasterMetrics_RestartInjected.class);
                ...
                public RegionServerStartupResponse regionServerStartup(RpcController controller, RegionServerStartupRequest request) throws ServiceException {
                    ...
                    new HashMap<>(serverManager.getOnlineServers()).forEach((sn, sm) -> {
                        if (sm.getLastReportTimestamp() <= 0) {
                            try {
                                serverManager.regionServerReport(sn, ServerMetricsBuilder.newBuilder(sn).setVersionNumber(sm.getVersionNumber()).setVersion(sm.getVersion()).setLastReportTimestamp(EnvironmentEdgeManager.currentTime()).build());
    ...
    public void testClusterRequests() throws Exception {
        ...
        request.setLoad(sl);
        RestartFramework.at("after_second_report").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        master.getMasterRpcServices().regionServerReport(null, request.build());
        RestartFramework.at("after_first_report").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        if (tablesOnMaster) {
            metricsHelper.assertCounter("cluster_requests", expectedRequestNumber, masterSource);
        } else {
            metricsHelper.assertCounterGt("cluster_requests", expectedRequestNumber, masterSource);
    ...
    public void testDefaultMasterMetrics() throws Exception {
        ...
        RestartFramework.at("after_verify_all_metrics").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_verify_server_metrics").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        metricsHelper.assertGauge("oldWALsDirSize", master.getMasterWalManager().getOldWALsDirSize(), masterSource);
    }
    ...
    public void testDefaultMasterProcMetrics() throws Exception {
        ...
        RestartFramework.at("after_verify_wal_metrics").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
