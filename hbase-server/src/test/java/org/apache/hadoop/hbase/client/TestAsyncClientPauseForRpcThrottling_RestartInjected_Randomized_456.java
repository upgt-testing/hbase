...
public class TestAsyncClientPauseForRpcThrottling_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAsyncClientPauseForRpcThrottling_RestartInjected.class);
    ...
    public static final class ThrottlingRSRpcServicesForTest extends RSRpcServices {
        ...
        private void maybeThrottle() throws ServiceException {
            if (THROTTLE.get()) {
                THROTTLE.set(false);
                throw new ServiceException(new RpcThrottlingException("number of requests exceeded - wait " + TimeUnit.NANOSECONDS.toMillis(WAIT_INTERVAL_NANOS) + "ms"));
            }
        }
    }
    ...
    public static void setUp() throws Exception {
        assertTrue("The MAX_MULTIPLIER_EXPECTATION must be less than HConstants.RETRY_BACKOFF[RETRY_COUNT] " + "in order for our tests to adequately verify that we aren't " + "multiplying throttled pauses based on the retry count.", MAX_MULTIPLIER_EXPECTATION < HConstants.RETRY_BACKOFF[RETRY_COUNT]);
        ...
        UTIL.getMiniHBaseCluster().getConfiguration().setClass(HConstants.REGION_SERVER_IMPL, ThrottlingRegionServerForTest.class, HRegionServer.class);
        ...
        cluster = UTIL.getMiniHBaseCluster();
        UTIL.getAdmin().move(UTIL.getAdmin().getRegions(TABLE_NAME).get(0).getEncodedNameAsBytes(), regionServer.getServerName());
        RestartFramework.at("after_table_create_and_data_load").on(cluster).restart("regionserver").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void itDoesNotWaitForThrottledGetExceedingTimeout() throws Exception {
        AsyncTable<AdvancedScanResultConsumer> table = CONN.getTableBuilder(TABLE_NAME).setOperationTimeout(1, TimeUnit.MILLISECONDS).build();
        boolean isThrottled = true;
        THROTTLE.set(isThrottled);
        assertTime(() -> {
    ...
    public void itDoesNotMultiplyThrottledGetWait() throws Exception {
        ...
        AsyncTable<AdvancedScanResultConsumer> table = CONN.getTableBuilder(TABLE_NAME).setOperationTimeout(1, TimeUnit.MINUTES).setMaxRetries(RETRY_COUNT + 1).setRetryPause(1, TimeUnit.NANOSECONDS).build();
        assertTimeBetween(() -> {
            table.get(new Get(Bytes.toBytes(0))).get();
            return null;
        ...
        assertTime(() -> {
            ...
            try (AsyncBufferedMutator mutator = CONN.getBufferedMutator(TABLE_NAME)) {
                for (int i = 100; i < 110; i++) {
                    futures.add(mutator.mutate(new Put(Bytes.toBytes(i)).addColumn(FAMILY, QUALIFIER, Bytes.toBytes(i))));
                }
            }
    ...
    public void itDoesNotWaitForThrottledBatchExceedingTimeout() throws Exception {
        ...
        assertTime(() -> {
            List<CompletableFuture<?>> futures = new ArrayList<>();
            try (AsyncBufferedMutator mutator = CONN.getBufferedMutatorBuilder(TABLE_NAME).setOperationTimeout(1, TimeUnit.MILLISECONDS).build()) {
                for (int i = 100; i < 110; i++) {
                    futures.add(mutator.mutate(new Put(Bytes.toBytes(i)).addColumn(FAMILY, QUALIFIER, Bytes.toBytes(i))));
                }
            }
            assertThrows(ExecutionException.class, () -> CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).get());
            return null;
        }, WAIT_INTERVAL_NANOS, false);
    }
    ...
    public void itDoesNotMultiplyThrottledBatchWait() throws Exception {
        ...
        assertTimeBetween(() -> {
            List<CompletableFuture<?>> futures = new ArrayList<>();
            try (AsyncBufferedMutator mutator = CONN.getBufferedMutatorBuilder(TABLE_NAME).setOperationTimeout(1, TimeUnit.MINUTES).setMaxRetries(RETRY_COUNT + 1).setRetryPause(1, TimeUnit.NANOSECONDS).build()) {
                for (int i = 100; i < 110; i++) {
                    futures.add(mutator.mutate(new Put(Bytes.toBytes(i)).addColumn(FAMILY, QUALIFIER, Bytes.toBytes(i))));
                }
            }
    ...
    public void itDoesNotWaitForThrottledScanExceedingTimeout() throws Exception {
        AsyncTable<AdvancedScanResultConsumer> table = CONN.getTableBuilder(TABLE_NAME).setScanTimeout(1, TimeUnit.MILLISECONDS).build();
        boolean isThrottled = true;
        THROTTLE.set(isThrottled);
        assertTime(() -> {
