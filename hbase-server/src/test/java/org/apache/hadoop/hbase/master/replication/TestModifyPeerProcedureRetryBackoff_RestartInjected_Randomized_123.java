...
public class TestModifyPeerProcedureRetryBackoff_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestModifyPeerProcedureRetryBackoff_RestartInjected.class);
    ...
    public static class TestModifyPeerProcedure extends ModifyPeerProcedure {
        ...
        @Override
        protected <T extends Procedure<MasterProcedureEnv>> void addChildProcedure(@SuppressWarnings("unchecked") T... subProcedure) {
            // Make it a no-op
        }
    ...
    public void test() throws IOException, InterruptedException {
        RestartFramework.at("after_pre_peer_modification_backoff").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureExecutor<MasterProcedureEnv> procExec = UTIL.getMiniHBaseCluster().getMaster().getMasterProcedureExecutor();
        RestartFramework.at("after_serial_peer_set_peer_enabled_backoff").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_serial_peer_reopen_regions_backoff").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        long procId = procExec.submitProcedure(new TestModifyPeerProcedure("1"));
        RestartFramework.at("after_update_peer_storage_backoff").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertBackoffIncrease();
        RestartFramework.at("after_submit_proc").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // SERIAL_PEER_SET_PEER_ENABLED
        assertBackoffIncrease();
        RestartFramework.at("after_post_peer_modification_backoff").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // No retry for SERIAL_PEER_ENABLE_PEER_REFRESH_PEER_ON_RS
        // POST_PEER_MODIFICATION
        assertBackoffIncrease();
        RestartFramework.at("after_serial_peer_update_last_pushed_seq_id_backoff").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_get_proc_exec").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
