...
public class TestReplicationSource_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationSource_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestReplicationSource_RestartInjected.class);
    ...
    public void testDefaultSkipsMetaWAL() throws IOException {
        ...
        when(peerConfig.getReplicationEndpointImpl()).thenReturn(DoNothingReplicationEndpoint.class.getName());
    ...
    public void testLogMoving() throws Exception {
        ...
        writer.close();
        WALStreamReader reader = WALFactory.createStreamReader(FS, logPath, TEST_UTIL.getConfiguration());
    ...
    public void testTerminateClearsBuffer() throws Exception {
        ReplicationSource source = new ReplicationSource();
        ReplicationSourceManager mockManager = new ReplicationSourceManager(null, null, conf, null, null, null, null, null, null, mock(MetricsReplicationGlobalSourceSource.class));
        ReplicationPeer mockPeer = mock(ReplicationPeer.class);
        Mockito.when(mockPeer.getPeerBandwidth()).thenReturn(0L);
        Configuration testConf = HBaseConfiguration.create();
        source.init(testConf, null, mockManager, null, mockPeer, Mockito.mock(Server.class), "testPeer", null, p -> OptionalLong.empty(), mock(MetricsSource.class));
    ...
    public void testServerShutdownRecoveredQueue() throws Exception {
        try {
            ...
            RestartFramework.at("after_peer_cluster_start").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_add_replication_peer").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_wait_for_sources").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_disable_peer").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Stopping serverA
            // It's queues should be claimed by the only other alive server i.e. serverB
            cluster.stopRegionServer(serverA.getServerName());
            RestartFramework.at("after_stop_server_a").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Waiter.waitFor(conf, 20000, new Waiter.Predicate<Exception>() {
            ...
            RestartFramework.at("after_wait_for_manager_b_old_sources").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final HRegionServer serverC = cluster.startRegionServer().getRegionServer();
            RestartFramework.at("after_start_server_c").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            serverC.waitForServerOnline();
            RestartFramework.at("after_server_c_online").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Waiter.waitFor(conf, 20000, new Waiter.Predicate<Exception>() {
            ...
            RestartFramework.at("after_wait_for_manager_c_sources").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_stop_server_b").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Waiter.waitFor(conf, 20000, (Waiter.Predicate<Exception>) () -> managerC.getOldSources().size() == 2);
            RestartFramework.at("after_wait_for_manager_c_two_queues").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin.enableReplicationPeer(peerId);
            RestartFramework.at("after_enable_peer").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRecoveredReplicationSourceShipperGetPosition() throws Exception {
        ...
        when(storage.getWALPosition(Mockito.eq(serverName), Mockito.any(), Mockito.any())).thenReturn(1001L);
        when(storage.getWALPosition(Mockito.eq(deadServer), Mockito.any(), Mockito.any())).thenReturn(-1L);
        Configuration conf = new Configuration(TEST_UTIL.getConfiguration());
        conf.setInt("replication.source.maxretriesmultiplier", -1);
   ...
   */
    ...
    private MetricsReplicationSourceSource getSourceMetrics(String sourceId) {
        MetricsReplicationSourceFactory factory = CompatibilitySingletonFactory.getInstance(MetricsReplicationSourceFactory.class);
