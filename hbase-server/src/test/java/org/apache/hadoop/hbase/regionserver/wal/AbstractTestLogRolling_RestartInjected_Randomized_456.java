 ...
 */
public abstract class AbstractTestLogRolling_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(AbstractTestLogRolling_RestartInjected.class);
    ...
    public AbstractTestLogRolling_RestartInjected_Randomized_456() {
        this.server = null;
        this.tableName = null;
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        // Slow sync executor.
        EXECUTOR = Executors.newSingleThreadScheduledExecutor(new ThreadFactoryBuilder().setNameFormat("Slow-sync-%d").setDaemon(true).setUncaughtExceptionHandler(Threads.LOGGING_EXCEPTION_HANDLER).build());
    }
    ...
    protected final AbstractFSWAL<?> getWALAndRegisterSlowSyncHook(RegionInfo region) throws IOException {
    ...
    protected final void checkSlowSync(AbstractFSWAL<?> log, Table table, int slowSyncLatency, int writeCount, boolean slowSync) throws Exception {
    ...
    public void testLogRolling() throws Exception {
        ...
        RestartFramework.at("after_flush_all_regions").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_write_data").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_log_roll").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCompactionRecordDoesntBlockRolling() throws Exception {
        ...
        try (Table t = TEST_UTIL.getConnection().getTable(TableName.META_TABLE_NAME);
            ...
            RestartFramework.at("after_initial_flushes").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            server = TEST_UTIL.getRSForFirstRegionInTable(table.getName());
            ...
            admin.flush(table.getName());
            RestartFramework.at("after_final_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            server = TEST_UTIL.getRSForFirstRegionInTable(table.getName());
            doPut(table, 1);
