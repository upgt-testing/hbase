...
public class TestReplicationStatus_RestartInjected_Randomized_123 extends TestReplicationBase {

    private static final Logger LOG = LoggerFactory.getLogger(TestReplicationStatus_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestReplicationStatus_RestartInjected.class);
    ...
    public void testReplicationStatus() throws Exception {
        ...
        RestartFramework.at("after_failover_metrics_check").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_rs_start").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        Threads.sleep(10000);
        RestartFramework.at("after_rs_stop").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_insert_rows").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_metrics_check").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_disable_peer").on(UTIL1.getHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
    private List<ReplicationLoadSource> waitOnMetricsReport(int greaterThan, ServerName serverName) throws Exception {
        UTIL1.waitFor(30000, 1000, new Waiter.ExplainingPredicate<Exception>() {

            @Override
            public boolean evaluate() throws Exception {
                List<ReplicationLoadSource> list = hbaseAdmin.getClusterMetrics(EnumSet.of(Option.LIVE_SERVERS)).getLiveServerMetrics().get(serverName).getReplicationLoadSourceList();
