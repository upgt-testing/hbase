...
public class TestSplitOrMergeStatus_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSplitOrMergeStatus_RestartInjected.class);
    ...
    public void testSplitSwitch() throws Exception {
        ...
        RestartFramework.at("after_region_split").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_table_load").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.split(t.getName());
        ...
        assertTrue(originalCount < count);
        RestartFramework.at("after_split_switch_enable").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_split_switch_disable").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.close();
    }
    ...
    public void testMergeSwitch() throws Exception {
        RestartFramework.at("after_region_merge").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final TableName tableName = TableName.valueOf(name.getMethodName());
        Table t = TEST_UTIL.createTable(tableName, FAMILY);
        TEST_UTIL.loadTable(t, FAMILY, false);
        Admin admin = TEST_UTIL.getAdmin();
        ...
        RestartFramework.at("after_merge_switch_disable").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_region_split").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_merge_switch_enable").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        admin.close();
    }

    @Test
    public void testMultiSwitches() throws Exception {
        Admin admin = TEST_UTIL.getAdmin();
        ...
        RestartFramework.at("after_switches_disable").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSplitRegionReplicaRitRecovery() throws Exception {
        ...
        RestartFramework.at("after_data_insert").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_split_procedure").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_disable_procedure").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Wait the completion
        ProcedureTestingUtility.waitProcedure(procExec, procId2);
        RestartFramework.at("after_delete_procedure").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void insertData(final TableName tableName, int startRow, int rowCount) throws IOException {
