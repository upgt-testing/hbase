...
public class TestMasterFileSystemWithWALDir_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterFileSystemWithWALDir_RestartInjected.class);
    ...
    public void testFsUriSetProperly() throws Exception {
        ...
        RestartFramework.at("after_get_master_filesystem").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        cluster = UTIL.getMiniHBaseCluster();
        RestartFramework.at("after_verify_filesystem_uris").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = cluster.getMaster();
        fs = master.getMasterFileSystem();
    }
