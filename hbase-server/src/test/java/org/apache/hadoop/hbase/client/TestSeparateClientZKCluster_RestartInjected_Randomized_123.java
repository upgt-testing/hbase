...
public class TestSeparateClientZKCluster_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestSeparateClientZKCluster_RestartInjected.class);
    ...
    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestSeparateClientZKCluster_RestartInjected.class);
    ...
    public void testMasterSwitch() throws Exception {
        ...
        try (Admin admin = conn.getAdmin()) {
            ...
            TEST_UTIL.waitFor(30000, () -> cluster.getMaster() != null && cluster.getMaster().isInitialized());
            LOG.info("Got master {}", cluster.getMaster().getServerName());
            RestartFramework.at("after_master_switch").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMetaRegionMove() throws Exception {
        ...
        try (Admin admin = conn.getAdmin();
            ...
            RestartFramework.at("after_meta_move").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_user_region_move").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            put = new Put(row);
    ...
    public void testMetaMoveDuringClientZkClusterRestart() throws Exception {
        ...
        try (Admin admin = conn.getAdmin();
            ...
            RestartFramework.at("after_meta_server_stop").on(TEST_UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            conn = TEST_UTIL.getConnection();
    ...
    public void testChangeMetaReplicaCount() throws Exception {
        ...
        try (RegionLocator locator = TEST_UTIL.getConnection().getRegionLocator(TableName.META_TABLE_NAME)) {
            ...
            RestartFramework.at("after_increase_replicas_to_3").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_decrease_replicas_to_2").on(TEST_UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            admin = TEST_UTIL.getAdmin();
