...
public class TestRegionMergeTransactionOnCluster_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestRegionMergeTransactionOnCluster_RestartInjected.class);

  private static final Logger LOG =
    LoggerFactory.getLogger(TestRegionMergeTransactionOnCluster_RestartInjected.class);
  ...
  public void testWholesomeMerge() throws Exception {
    ...
    try {
      ...
      RestartFramework.at("after_table_create_and_load")
        .on(cluster)
      ...
      RestartFramework.at("after_first_merge")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_second_merge")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_row_verification")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RegionInfo hri = ThreadLocalRandom.current().nextBoolean()
      ...
      RestartFramework.at("after_assign_attempt")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_unassign_attempt")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMergeAndRestartingMaster() throws Exception {
    ...
    try {
      ...
      RestartFramework.at("after_merge_with_restart_simulation")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testCleanMergeReference() throws Exception {
    ...
    try {
      ...
      RestartFramework.at("after_compaction")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      for (RegionServerThread rs : regionServerThreads) {
        CompactedHFilesDischarger cleaner =
          new CompactedHFilesDischarger(100, null, rs.getRegionServer(), false);
        cleaner.chore();
        Thread.sleep(1000);
      }

      RestartFramework.at("after_cleaner_chore")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_catalog_janitor")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMerge() throws Exception {
    ...
    try {
      ...
      RestartFramework.at("after_region_offline")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_merge_attempts")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMergeWithReplicas() throws Exception {
    ...
    try {
      ...
      RestartFramework.at("after_table_create_with_replicas")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_initial_region_collection")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_merge_replicas")
        .on(cluster)
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_replica_verification")
        .on(cluster)
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  private PairOfSameType<RegionInfo> mergeRegionsAndVerifyRegionNum(HMaster master,
    TableName tablename, int regionAnum, int regionBnum, int expectedRegionNum) throws Exception {
    PairOfSameType<RegionInfo> mergedRegions =
  ...
  private PairOfSameType<RegionInfo> requestMergeRegion(HMaster master, TableName tablename,
    ...
    ADMIN.mergeRegionsAsync(regionA.getEncodedNameAsBytes(), regionB.getEncodedNameAsBytes(),
      false);
  ...
  private void waitAndVerifyRegionNum(HMaster master, TableName tablename, int expectedRegionNum)
    ...
    while (EnvironmentEdgeManager.currentTime() < timeout) {
      ...
      if (
        tableRegionsInMetaSize == expectedRegionNum && tableRegionsInMasterSize == expectedRegionNum
      ) {
        break;
      }
      Thread.sleep(250);
