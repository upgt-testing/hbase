...
public class TestRegionServerNoMaster_RestartInjected {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestRegionServerNoMaster_RestartInjected.class);
  ...
  public static void before() throws Exception {
    ...
    table.put(p);
    RestartFramework.at("after_put_data")
      .on(HTU.getMiniHBaseCluster())
  ...
  public static void closeRegion(HBaseTestingUtility HTU, HRegionServer rs, HRegionInfo hri)
    ...
    AdminProtos.CloseRegionRequest crr =
      ProtobufUtil.buildCloseRegionRequest(rs.getServerName(), hri.getRegionName());
  ...
  public void testCloseByRegionServer() throws Exception {
    closeRegionNoZK();
    RestartFramework.at("after_close_region")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_open_region")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
  ...
  public void testMultipleCloseFromMaster() throws Exception {
    ...
    checkRegionIsClosed(HTU, getRS(), hri);
    RestartFramework.at("after_multiple_close")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    openRegion(HTU, getRS(), hri);
    RestartFramework.at("after_open_region_multi")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
  ...
  public void testCancelOpeningWithoutZK() throws Exception {
    ...
    closeRegionNoZK();
    checkRegionIsClosed(HTU, getRS(), hri);
    RestartFramework.at("after_close_region_cancel")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_rit_put")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    Assert.assertEquals(Boolean.FALSE,
      getRS().getRegionsInTransitionInRS().get(hri.getEncodedNameAsBytes()));
    RestartFramework.at("after_close_attempt")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_open_handler_submit")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    openRegion(HTU, getRS(), hri);
    RestartFramework.at("after_open_region_cancel")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
  ...
  public void testOpenCloseRegionRPCIntendedForPreviousServer() throws Exception {
    ...
    RestartFramework.at("after_region_available_check")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    try {
      CloseRegionRequest request =
        ProtobufUtil.buildCloseRegionRequest(earlierServerName, regionName);
    ...
    RestartFramework.at("after_close_rejection")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
    ...
    try {
      AdminProtos.OpenRegionRequest orr =
        RequestConverter.buildOpenRegionRequest(earlierServerName, hri, null);
      ...
      RestartFramework.at("after_open_rejection")
        .on(HTU.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testInstallShutdownHook() {
    ...
    RestartFramework.at("after_shutdown_hook_check")
      .on(HTU.getMiniHBaseCluster())
      .restart("regionserver")
      .withIndex(0)
      .withMode(RestartMode.GRACEFUL)
