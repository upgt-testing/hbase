...
public class TestAlwaysStandByHMaster_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestAlwaysStandByHMaster_RestartInjected.class);
    ...
    public void testAlwaysStandBy() throws Exception {
        ...
        RestartFramework.at("after_start_new_master").on(testUtil.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_master_verification").on(testUtil.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        newActive = testUtil.getMiniHBaseCluster().getMaster();
        assertTrue(testUtil.getMiniHBaseCluster().waitForActiveAndReadyMaster(5000));
        RestartFramework.at("after_stop_master").on(testUtil.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_standby_verification").on(testUtil.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
