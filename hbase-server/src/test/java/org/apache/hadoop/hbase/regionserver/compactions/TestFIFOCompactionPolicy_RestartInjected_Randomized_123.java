...
public class TestFIFOCompactionPolicy_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestFIFOCompactionPolicy_RestartInjected.class);
    ...
    public void testPurgeExpiredFiles() throws Exception {
        ...
        RestartFramework.at("after_major_compact_submit").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_major_compact_complete").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_store_file_count_check").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFIFOCompactionPolicyExpiredEmptyHFiles() throws Exception {
        ...
        RestartFramework.at("after_second_put_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_put_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_major_compact_complete").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        put = new Put(Bytes.toBytes("row3")).addColumn(family, qualifier, ts, Bytes.toBytes("value1"));
        RestartFramework.at("after_third_put_flush").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        table.put(put);
        RestartFramework.at("after_second_major_compact_complete").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TEST_UTIL.getAdmin().flush(tableName);
        Assert.assertEquals(2, store.getStorefilesCount());
        TEST_UTIL.getAdmin().majorCompact(tableName);
        RestartFramework.at("after_first_major_compact_submit").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TEST_UTIL.waitFor(testWaitTimeoutMs, (Waiter.Predicate<Exception>) () -> store.getStorefilesCount() == 1);
        RestartFramework.at("after_second_major_compact_submit").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Assert.assertEquals(1, store.getStorefilesCount());
