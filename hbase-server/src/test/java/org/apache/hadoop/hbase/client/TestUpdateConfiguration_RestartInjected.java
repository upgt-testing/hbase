...
public class TestUpdateConfiguration_RestartInjected extends AbstractTestUpdateConfiguration {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestUpdateConfiguration_RestartInjected.class);
  ...
  public void testMasterOnlineConfigChange() throws IOException {
    LOG.debug("Starting the test {}", name.getMethodName());
    replaceHBaseSiteXML();
    RestartFramework.at("after_config_replace")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testAllOnlineConfigChange() throws IOException {
    ...
    RestartFramework.at("after_all_config_update")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testAllCustomOnlineConfigChange() throws IOException {
    LOG.debug("Starting the test {}", name.getMethodName());
    replaceHBaseSiteXML();
    RestartFramework.at("after_custom_config_replace")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    Admin admin = TEST_UTIL.getAdmin();
    admin.updateConfiguration();
    RestartFramework.at("after_all_custom_config_update")
        .on(TEST_UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
