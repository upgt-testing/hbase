...
public class TestMasterCoprocessorExceptionWithAbort_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestMasterCoprocessorExceptionWithAbort_RestartInjected.class);
    ...
    public void testExceptionFromCoprocessorWhenCreatingTable() throws IOException {
        ...
        assertTrue(HMaster.getLoadedCoprocessors().contains(TestMasterCoprocessorExceptionWithAbort_RestartInjected.BuggyMasterObserver.class.getName()));
        ...
        RestartFramework.at("after_zk_watcher_setup").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
