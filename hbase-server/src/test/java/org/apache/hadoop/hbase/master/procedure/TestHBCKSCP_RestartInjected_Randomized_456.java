...
public class TestHBCKSCP_RestartInjected_Randomized_456 extends TestSCPBase {

    private static final Logger LOG = LoggerFactory.getLogger(TestHBCKSCP_RestartInjected.class);

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestHBCKSCP_RestartInjected.class);
    ...
    public TestHBCKSCP_RestartInjected_Randomized_456(final int replicas, final HBCKSCPScheduler hbckscpScheduler, final RegionSelector regionSelector) {
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_get_regions").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final ServerName rsServerName = cluster.getRegionServerThreads().stream().map(JVMClusterUtil.RegionServerThread::getRegionServer).map(HRegionServer::getServerName).filter(sn -> !sn.equals(metaServer)).findAny().orElseThrow(() -> new NoSuchElementException("Cannot locate a region server that is not hosting meta."));
        RestartFramework.at("after_verify_region_open").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        final RegionInfo rsRI = regions.stream().peek(info -> LOG.debug("{}", info)).filter(regionSelector::regionFilter).findAny().orElseThrow(regionSelector::regionFilterFailure);
        RestartFramework.at("after_verify_region_on_new_server").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final int replicaId = rsRI.getReplicaId();
        RestartFramework.at("after_hbckscp_complete").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_schedule_hbckscp").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Result r = MetaTableAccessor.getRegionResult(master.getConnection(), rsRI);
        // Assert region is OPEN.
        assertEquals(RegionState.State.OPEN.toString(), Bytes.toString(r.getValue(HConstants.CATALOG_FAMILY, MetaTableAccessor.getRegionStateColumn(replicaId))));
        ...
        // Refresh after master restart
        master = cluster.getMaster();
        RestartFramework.at("after_kill_regionserver").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_find_target_server").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = cluster.getMaster();
        ProcedureTestingUtility.waitProcedure(master.getMasterProcedureExecutor(), pid);
        RestartFramework.at("after_table_load").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_verify_meta_references").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh after master restart
        master = cluster.getMaster();
        ...
        assertNotNull(cluster.getRegionServer(serverName));
        RestartFramework.at("after_remove_from_master").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private boolean searchMeta(HMaster master, ServerName sn) throws IOException {
        List<Pair<RegionInfo, ServerName>> ps = MetaTableAccessor.getTableRegionsAndLocations(master.getConnection(), null);
    ...
    private static class ScheduleServerCrashProcedure extends HBCKSCPScheduler {
        ...
        public long scheduleHBCKSCP(ServerName rsServerName, HMaster master) throws ServiceException {
            MasterProtos.ScheduleServerCrashProcedureResponse response = master.getMasterRpcServices().scheduleServerCrashProcedure(null, MasterProtos.ScheduleServerCrashProcedureRequest.newBuilder().addServerName(ProtobufUtil.toServerName(rsServerName)).build());
    ...
    private static class PrimaryNotMetaRegionSelector extends RegionSelector {
        ...
        boolean regionFilter(final RegionInfo info) {
            return !Objects.equals(TableName.META_TABLE_NAME, info.getTable()) && Objects.equals(RegionInfo.DEFAULT_REPLICA_ID, info.getReplicaId());
        }
