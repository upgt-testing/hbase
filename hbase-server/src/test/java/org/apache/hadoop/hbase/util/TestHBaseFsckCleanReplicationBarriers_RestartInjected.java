...
public class TestHBaseFsckCleanReplicationBarriers_RestartInjected {
  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestHBaseFsckCleanReplicationBarriers_RestartInjected.class);
  ...
  public void testCleanReplicationBarrierWithDeletedTable() throws Exception {
    ...
    RestartFramework.at("after_barriers_written")
    ...
    RestartFramework.at("after_barrier_scan")
        .on(UTIL.getMiniHBaseCluster())
    ...
    boolean cleaned = HbckTestingUtil.cleanReplicationBarrier(UTIL.getConfiguration(), tableName);
    assertTrue(cleaned);
    RestartFramework.at("after_clean_barriers")
        .on(UTIL.getMiniHBaseCluster())
        .restart("master")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testCleanReplicationBarrierWithExistTable() throws Exception {
    ...
    RestartFramework.at("after_barriers_added")
        .on(UTIL.getMiniHBaseCluster())
        .restart("regionserver")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
