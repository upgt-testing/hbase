...
public class TestRaceBetweenSCPAndDTP_RestartInjected_Randomized_456 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRaceBetweenSCPAndDTP_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRaceBetweenSCPAndDTP_RestartInjected.class);
    ...
    public static void setUp() throws Exception {
        ...
        RestartFramework.at("after_balancer_switch").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.waitTableAvailable(NAME);
        RestartFramework.at("after_table_available").on(UTIL.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private boolean wasExecuted(Procedure<?> proc) {
        ...
        return proc.wasExecuted() || proc.getState() == ProcedureState.WAITING_TIMEOUT || proc.getState() == ProcedureState.WAITING;
    }
    ...
    public void testRace() throws Exception {
        ...
        RestartFramework.at("after_dtp_started").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_all_scps_finished").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cdl.await();
        ...
        RestartFramework.at("after_scp_at_latch_point").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        UTIL.waitFor(60000, () -> procExec.getProcedures().stream().filter(p -> p instanceof DisableTableProcedure).map(p -> (DisableTableProcedure) p).filter(p -> p.getTableName().equals(NAME)).anyMatch(this::wasExecuted));
        RestartFramework.at("after_rs_stop_scp_triggered").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_dtp_executed").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RESUME_GET_REPLICATION_PEER_MANAGER.countDown();
        RestartFramework.at("after_dtp_completed").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_resume_latch_released").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // make sure the DTP can finish
        future.get();
        // also make sure all SCPs are finished
        UTIL.waitFor(60000, () -> procExec.getProcedures().stream().filter(p -> p instanceof ServerCrashProcedure).allMatch(Procedure::isFinished));
    }
}
