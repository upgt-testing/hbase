...
public class TestWALEventTracker_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestWALEventTracker_RestartInjected.class);
    ...
    public void waitForWalEventTrackerTableCreation() {
        Waiter.waitFor(CONF, 10000, (Waiter.Predicate) () -> TEST_UTIL.getAdmin().tableExists(WAL_EVENT_TRACKER_TABLE_NAME));
    }
    ...
    public void testWALRolling() throws Exception {
        ...
        RestartFramework.at("after_wal_roll").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_wal_tracker_table_ready").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<WALEventTrackerPayload> walEventsList = getRows(hostName, connection);
        RestartFramework.at("after_wal_events_retrieved").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        compareEvents(hostName, wal1Path.getName(), walEventsList, new ArrayList<>(Arrays.asList(WALEventTrackerListener.WalState.ROLLING.name(), WALEventTrackerListener.WalState.ROLLED.name())), false);
        // There should be only 1 event for wal2Name which is current wal, with ACTIVE state
        compareEvents(hostName, wal2Path.getName(), walEventsList, new ArrayList<>(Arrays.asList(WALEventTrackerListener.WalState.ACTIVE.name())), true);
    ...
    private void checkWALRolledEventHasSize(List<WALEventTrackerPayload> walEvents, String walName, long actualSize) {
        ...
        for (WALEventTrackerPayload event : walEvents) {
            if (walName.equals(event.getWalName()) && WALEventTrackerListener.WalState.ROLLED.name().equals(event.getState())) {
