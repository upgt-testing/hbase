...
public class TestCellACLs_RestartInjected_Randomized_456 extends SecureTestUtil {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestCellACLs_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestCellACLs_RestartInjected.class);
    ...
    public static void setupBeforeClass() throws Exception {
        ...
        RestartFramework.at("after_acl_table_enabled").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_user_creation").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        cluster = TEST_UTIL.getMiniHBaseCluster();
        RestartFramework.at("after_coprocessor_setup").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void setUp() throws Exception {
        ...
        RestartFramework.at("after_table_enabled").on(cluster).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCoveringCheck() throws Exception {
        ...
        grantOnTable(TEST_UTIL, USER_OTHER.getShortName(), testTable.getTableName(), TEST_FAMILY, null, Action.READ);
        // Grant read access to GROUP
        grantOnTable(TEST_UTIL, AuthUtil.toGroupEntry(GROUP), testTable.getTableName(), TEST_FAMILY, null, Action.READ);
