...
public class TestRegionServerCoprocessorExceptionWithAbort_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestRegionServerCoprocessorExceptionWithAbort_RestartInjected.class);

    private static final Logger LOG = LoggerFactory.getLogger(TestRegionServerCoprocessorExceptionWithAbort_RestartInjected.class);
    ...
    public void testExceptionFromCoprocessorDuringPut() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_regions_assigned").on(cluster).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster = TEST_UTIL.getHBaseCluster();
    ...
    public static class BuggyRegionObserver extends SimpleRegionObserver {
        ...
        @SuppressWarnings("null")
        @Override
        public void prePut(final ObserverContext<RegionCoprocessorEnvironment> c, final Put put, final WALEdit edit, final Durability durability) {
