...
public class TestLockProcedure_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestLockProcedure_RestartInjected.class);
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TestLockProcedure_RestartInjected.class);
    ...
    public static void setupCluster() throws Exception {
        ...
        RestartFramework.at("after_table2_create").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UTIL.getAdmin().createNamespace(NamespaceDescriptor.create(namespace).build());
        RestartFramework.at("after_table1_create").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_namespace_create").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testUpdateHeartbeatAndUnlockForTable() throws Exception {
        ...
        RestartFramework.at("after_lock_release").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(HEARTBEAT_TIMEOUT / 2);
        sendHeartbeatAndCheckLocked(procId, true);
        Thread.sleep(HEARTBEAT_TIMEOUT / 2);
        sendHeartbeatAndCheckLocked(procId, true);
        RestartFramework.at("after_table_lock_acquired").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_heartbeat_checks").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(HEARTBEAT_TIMEOUT / 2);
        sendHeartbeatAndCheckLocked(procId, true);
        releaseLock(procId);
    ...
    public void testAbort() throws Exception {
        ...
        final long procId = queueLock(lock);
        assertTrue(awaitForLocked(procId, 2000));
        RestartFramework.at("after_abort").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(procExec.abort(procId));
        sendHeartbeatAndCheckLocked(procId, false);
        ProcedureTestingUtility.waitProcedure(procExec, procId);
        RestartFramework.at("after_lock_acquired_before_abort").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureTestingUtility.assertProcNotFailed(procExec, procId);
    }
    ...
    public void testUpdateHeartbeatAndUnlockForNamespace() throws Exception {
        ...
        Thread.sleep(HEARTBEAT_TIMEOUT / 2);
        sendHeartbeatAndCheckLocked(procId, true);
        RestartFramework.at("after_namespace_lock_acquired").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_namespace_lock_release").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(HEARTBEAT_TIMEOUT / 2);
        sendHeartbeatAndCheckLocked(procId, true);
        releaseLock(procId);
        sendHeartbeatAndCheckLocked(procId, false);
        RestartFramework.at("after_namespace_heartbeat_checks").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureTestingUtility.waitProcedure(procExec, procId);
        ProcedureTestingUtility.assertProcNotFailed(procExec, procId);
    }
    ...
    public void testTimeout() throws Exception {
        ...
        sendHeartbeatAndCheckLocked(procId, true);
        Thread.sleep(HEARTBEAT_TIMEOUT / 2);
        RestartFramework.at("before_timeout_expiry").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        sendHeartbeatAndCheckLocked(procId, true);
        Thread.sleep(4 * HEARTBEAT_TIMEOUT);
        sendHeartbeatAndCheckLocked(procId, false);
        ProcedureTestingUtility.waitProcedure(procExec, procId);
        RestartFramework.at("after_lock_acquired_before_timeout").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ProcedureTestingUtility.assertProcNotFailed(procExec, procId);
    }
    ...
    public void testMultipleLocks() throws Exception {
        ...
        RestartFramework.at("after_all_locks_queued").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_table1_lock_released").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        sendHeartbeatAndCheckLocked(table2ProcId, false);
        RestartFramework.at("after_namespace_lock_released").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(awaitForLocked(regions2ProcId, 2000));
        RestartFramework.at("after_region_locks_released").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_table2_lock_released").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testLatch() throws Exception {
        ...
        RestartFramework.at("after_local_lock_latch_acquired").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testLocalLockTimeout() throws Exception {
        ...
        RestartFramework.at("before_local_lock_timeout").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_local_lock_acquired").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        releaseLock(lockProc.getProcId());
        ProcedureTestingUtility.waitProcedure(procExec, lockProc.getProcId());
        ProcedureTestingUtility.assertProcNotFailed(procExec, lockProc.getProcId());
    }
    ...
    private void testRemoteLockRecovery(LockRequest lock) throws Exception {
        ...
        RestartFramework.at("after_remote_lock_acquired").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        Thread.sleep(HEARTBEAT_TIMEOUT / 2);
        sendHeartbeatAndCheckLocked(procId, true);
        RestartFramework.at("after_proc_executor_recovery").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testLocalMasterLockRecovery() throws Exception {
        ProcedureTestingUtility.setKillAndToggleBeforeStoreUpdate(procExec, true);
        RestartFramework.at("after_local_master_lock_recovery").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        CountDownLatch latch = new CountDownLatch(1);
        LockProcedure lockProc = new LockProcedure(UTIL.getConfiguration(), TableName.valueOf("table"), LockType.EXCLUSIVE, "desc", latch);
        procExec.submitProcedure(lockProc);
        assertTrue(latch.await(2000, TimeUnit.MILLISECONDS));
        ...
        assertEquals(true, procExec.isRunning());
        ProcedureTestingUtility.waitProcedure(procExec, lockProc.getProcId());
        RestartFramework.at("after_local_master_lock_acquired").on(UTIL.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
