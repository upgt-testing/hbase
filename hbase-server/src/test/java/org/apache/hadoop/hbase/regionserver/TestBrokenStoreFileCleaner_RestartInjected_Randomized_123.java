...
public class TestBrokenStoreFileCleaner_RestartInjected_Randomized_123 {

    @ClassRule
    public static final HBaseClassTestRule CLASS_RULE = HBaseClassTestRule.forClass(TestBrokenStoreFileCleaner_RestartInjected.class);
    ...
    public void setUp() throws Exception {
        ...
        RestartFramework.at("after_cluster_start").on(testUtil.getMiniHBaseCluster()).restart("master").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_cluster_start_rs").on(testUtil.getMiniHBaseCluster()).restart("regionserver").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testJunkFileTTL() throws Exception {
        ...
        BrokenStoreFileCleaner cleaner = new BrokenStoreFileCleaner(15000000, 0, rs, rs.getConfiguration(), rs);
        cleaner.chore();
    ...
    public void testWhenRegionIsClosing() throws Exception {
        ...
        BrokenStoreFileCleaner cleaner = new BrokenStoreFileCleaner(15000000, 0, rs, rs.getConfiguration(), mockedServer);
        cleaner.chore();
        // verify no storefile got deleted
